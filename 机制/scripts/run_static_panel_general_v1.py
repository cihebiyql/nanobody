...
def main() -> int:
    ...
    if len(manifest) < 2 or len(manifest) % 2:
        raise ValueError(f"static manifest must have an even positive job count, got {len(manifest)}")
    expected_candidates = len(manifest) // 2
    ...
    if missing or failed or len(rows) != len(manifest):
        status = {
            "state": "PARTIAL",
            "finished_at": utcnow(),
            "total": len(manifest),
    ...
    if set(candidate_counts.values()) != {2} or len(candidate_counts) != expected_candidates:
    ...
    receipt = {
        "schema_version": "pvrig.static_review.generalized.v1",
    ...
    print(json.dumps({"candidates": expected_candidates, "jobs": len(manifest), "failed": 0}))
    return 0
