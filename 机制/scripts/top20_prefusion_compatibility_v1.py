...
def main():
    ...
    final=sorted(read_tsv(a.final50),key=lambda r:int(r['final_rank']))[:20]; rows={r['candidate_id']:r for r in final}
    manifest=read_tsv(a.manifest)
    if len(final)!=20 or len(manifest)!=160:raise ValueError((len(final),len(manifest)))
    by=defaultdict(list)
    for m in manifest:by[m['candidate_id']].append(m)
    if set(by)!=set(rows) or set(map(len,by.values()))!={8}:raise ValueError('manifest is not Top20×8')
    poses=[]
