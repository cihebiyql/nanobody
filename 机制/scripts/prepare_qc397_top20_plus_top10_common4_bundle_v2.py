#!/usr/bin/env python3
"""Extract frozen common4 poses for Final-rank Top20 plus current Top10 add-ons."""
...
def main():
    ...
    base=all_final[:20]; base_ids={r['candidate_id'] for r in base}
    final=base+[r for r in all_final if r['candidate_id'] in top10_rank and r['candidate_id'] not in base_ids]
    for r in final:
        r['top10_rank']=top10_rank.get(r['candidate_id'],'')
        r['fusion_panel_membership']='FINAL_RANK_TOP20' if r['candidate_id'] in base_ids else 'CURRENT_TOP10_DIVERSITY_ADDON'
    ...
    ):raise ValueError('fusion panel invalid or current Top10 not fully covered')
