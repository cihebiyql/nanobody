...
def main():
    ...
    final=sorted(read_tsv(a.final50),key=lambda r:int(r['final_rank']))
    ...
    current_top10={r['candidate_id'] for r in final if r.get('top10_rank')}
    primary_top20={r['candidate_id'] for r in final if r.get('fusion_panel_membership')=='FINAL_RANK_TOP20'}
    if (
        len(final)<20
        or len(primary_top20)!=20
        or len(current_top10)!=10
        or set(struct)!=ids
        or set(fusion)!=ids
    ):raise ValueError('fusion-panel sidecar membership mismatch')
    grades=[]
    for row in final:
