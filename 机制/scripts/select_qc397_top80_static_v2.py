#!/usr/bin/env python3
"""Select the Top80 from any complete two-conformation static-review panel.

This is the generalized, auditable counterpart of the frozen legacy Top200→Top80
selector.  It deliberately retains its channel quotas, diversity constraints and
rank semantics.  Rosetta/PRODIGY values are joined for diagnostics only and never
alter cross-candidate rank.
"""
...
def aggregate_static(rows: list[dict[str, str]]) -> dict[str, str]:
    if len(rows) != 2 or {row["conformation"] for row in rows} != {"8x6b", "9e6y"}:
        raise ValueError("candidate static evidence must contain exactly 8X6B and 9E6Y")
    output: dict[str, str] = {
        "static_pose_count": "2",
    ...
    for name in metric_names:
        ...
        output[f"static_median_{name}"] = f"{statistics.median(finite):.9g}" if finite else ""
        output[f"static_range_{name}"] = f"{min(finite):.9g},{max(finite):.9g}" if finite else ""
...
def main() -> int:
    ...
    parser.add_argument("--candidates", type=Path, required=True,
                        help="unified, hard-gated candidate TSV")
    ...
    if receipt.get("state") != "STATIC_COMPLETE":
        raise ValueError("static receipt is incomplete")
    expected_candidates = receipt.get("candidates")
    expected_jobs = receipt.get("jobs")
    if not isinstance(expected_candidates, int) or expected_candidates < 80:
        raise ValueError("static receipt candidate count is invalid for Top80")
    if expected_jobs != 2 * expected_candidates:
        raise ValueError("static receipt does not prove a two-pose panel")
    if receipt.get("method_roles") != METHOD_ROLES:
        raise ValueError("static method roles are not the frozen descriptive-only roles")

    candidates = read_tsv(args.candidates)
    metrics = read_tsv(args.static_metrics)
    if len(candidates) != expected_candidates or len(metrics) != expected_jobs:
        raise ValueError("candidate/static metric count mismatches static receipt")
    candidate_ids = [row.get("candidate_id", "") for row in candidates]
    if len(set(candidate_ids)) != expected_candidates or not all(candidate_ids):
        raise ValueError("candidate table does not contain unique nonempty candidate IDs")
    ...
    if set(by_candidate) != set(candidate_ids):
        raise ValueError("static metrics candidate set does not match candidate table")
    ...
    for row in candidates:
        ...
        if not math.isfinite(score):
            raise ValueError(f"candidate lacks a finite pre-static ranking score: {candidate_id}")
        copied["post_static_selection_score"] = f"{score:.9f}"
        copied["static_selection_policy"] = (
            "technical completeness plus pre-static shared geometry ordinal; "
            "Rosetta/PRODIGY values are not cross-candidate rank inputs"
        )
        merged.append(copied)
    missing = [row["candidate_id"] for row in merged if not row.get("cdr3", "")]
    if missing:
        raise ValueError(f"{len(missing)} candidates lack CDR3")
    ...
    def allowed(row: dict[str, str]) -> bool:
        return (
            ...
            and all(direct_cdr3_identity(row, chosen) < 0.80 for chosen in selected)
        )
    ...
    def add(row: dict[str, str], channel: str) -> None:
        ...
        copied["top80_selection_reason"] = (
            ...
            "selected by pre-static shared geometry ordinal under frozen diversity quotas"
        )
    ...
    for channel, target_count in CHANNEL_TARGETS:
        pool = [
            ...
            or (channel == "STRUCTURAL_RESERVE" and row.get("selection_channel") == "QUOTA_SAFE_BACKFILL")
        ]
        before = len(selected)
        for row in pool:
    ...
    exclusions = [
        {
            "candidate_id": row["candidate_id"],
            "unified_qc397_geometry_rank": row.get("unified_qc397_geometry_rank", ""),
        ...
        for row in merged if row["candidate_id"] not in selected_set
    ]

    args.out.mkdir(parents=True, exist_ok=True)
    tsv_path = args.out / "top80_post_static.tsv"
    fasta_path = args.out / "top80_post_static.fasta"
    exclusions_path = args.out / "qc397_to_top80_exclusions.tsv"
    write_tsv(tsv_path, selected)
    write_fasta(fasta_path, selected)
    write_tsv(exclusions_path, exclusions)
    top80_receipt = {
        "schema_version": "pvrig.qc397.top80.post_static.v2",
        "state": "TOP80_COMPLETE",
        "count": 80,
        "source_pool_size": expected_candidates,
        "channel_counts": dict(Counter(row["top80_selection_channel"] for row in selected)),
        ...
        "rank_policy": "shared common4 geometry ordinal; calibrated static methods do not change cross-candidate rank",
        "input_hashes": {
        ...
        "claim_boundary": "Computational shortlist only; static values are not experimental binding, Kd, IC50, expression or purity.",
    }
    receipt_path = args.out / "TOP80_COMPLETE.json"
    receipt_path.write_text(json.dumps(top80_receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8")
