...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fixed-pose-audit-receipt", required=True, type=Path)
...
def main() -> None:
    ...
    assert joined_receipt["instability_index_source"] == "vhh_eval.tsv"
    assert joined_receipt["instability_index_complete_count"] == 50
    assert epitope_receipt["state"] == "COMPLETE"
    assert epitope_receipt["candidates"] == 50
    fixed_pose_audit = read_tsv(args.fixed_pose_audit_tsv)
    fixed_pose_receipt = json.loads(
        args.fixed_pose_audit_receipt.read_text(encoding="utf-8")
    )
    assert len(fixed_pose_audit) == 15
    assert fixed_pose_receipt["state"] == "COMPLETE"
    assert fixed_pose_receipt["records"] == 15
    assert fixed_pose_receipt["verdict_counts"] == {
        "PASS_ALL_THREE_CDRS_REDESIGNED": 15
    }
    assert all(
        row["audit_verdict"] == "PASS_ALL_THREE_CDRS_REDESIGNED"
        for row in fixed_pose_audit
    )
    ...
    required_files = [
        ...
        args.fixed_pose_audit_tsv,
        args.fixed_pose_audit_receipt,
    ]
    ...
    receipt = {
        "schema_version": "qc397_final50_submission_freeze_p0p1_v1_1",
        ...
        "p1": {
            ...
            "instability_index_source": joined_receipt[
                "instability_index_source"
            ],
            "instability_index_complete_count": joined_receipt[
                "instability_index_complete_count"
            ],
            ...
            "fixed_pose_all_three_cdrs_redesigned": "PASS_15_OF_15",
        },
        "assertions": {
