...
def main() -> int:
    ...
    if not tasks:
        raise ValueError(f"empty task shard: {args.tasks}")
    ...
    for task in tasks:
        ...
        if parent["sequence_aa"] != task["parent_sequence"]:
            raise ValueError(f"parent sequence mismatch: {task['task_id']}")
        if GEN.sha256_text(parent["sequence_aa"]) != task["parent_sequence_sha256"]:
            raise ValueError(f"parent sequence SHA256 mismatch: {task['task_id']}")
        candidate = GEN.generate_candidate(task, parent, donors)
        candidate.update(
            GEN.fast_qc(
