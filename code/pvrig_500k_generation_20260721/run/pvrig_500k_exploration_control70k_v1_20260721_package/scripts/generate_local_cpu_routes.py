...
LOCAL_ROUTES = {
    ...
    "natural_cdr_donor",
    "profile_diversified_exploration_control",
}
...
def diversified_mutate(cdr: str, rng: random.Random) -> str:
    """Broader CDR exploration control; preserves length and existing cysteines."""
    mutable = [index for index, aa in enumerate(cdr) if aa != "C"]
    if not mutable:
        return cdr
    mutation_count = max(2, round(len(cdr) * 0.30))
    positions = rng.sample(mutable, min(mutation_count, len(mutable)))
    result = list(cdr)
    alphabet = sorted(AA - {"C"})
    for position in positions:
        choices = [aa for aa in alphabet if aa != result[position]]
        result[position] = rng.choice(choices)
    return "".join(result)
...
def generate_candidate(
    ...
    for region_index, region in enumerate(regions):
        ...
        elif task["route_id"] == "profile_diversified_exploration_control":
            new = diversified_mutate(old, rng)
            donor_ids.append(f"{region}:profile_diversified_v1")
        else:
