#!/usr/bin/env python3
"""Fail-closed contract checks for the Top200 public-seed completion campaign."""
...
def validate_ready(
    ...
    if jobs_per_shard is not None and jobs_per_shard not in (
        ...
        raise RuntimeError("READY jobs_per_shard is not 2x212")
    anchors = {
        "archive": _nested(
...
def validate_project(
    ...
    if observed != master_job_ids:
        raise RuntimeError("two shards are not an exact partition of the master manifest")
    ...
    cfg_lock_path = project_root / "config/TWO_SEED_CFG_LOCK.json"
    cfg_lock = json.loads(cfg_lock_path.read_text())
    if cfg_lock.get("status") != "LOCKED":
        raise RuntimeError("TWO_SEED_CFG_LOCK status is not LOCKED")
    if {str(seed) for seed in cfg_lock.get("seeds", [])} != EXPECTED_SEED_VALUES:
        raise RuntimeError("TWO_SEED_CFG_LOCK seeds mismatch")
    if set(cfg_lock.get("conformations", [])) != EXPECTED_CONFORMATIONS:
        raise RuntimeError("TWO_SEED_CFG_LOCK conformations mismatch")
    ...
    if observed_pairs != expected_pairs:
        raise RuntimeError("TWO_SEED_CFG_LOCK does not contain the exact 2x2 matrix")
...
def audit_results(
    ...
    marker_count = sum(
        ...
        if (result_root / "markers" / f"top200_seed_completion_shard_{index}.done").is_file()
    )
    complete = (
    ...
    payload = {
        "schema_version": "pvrig.top200_seed_completion.technical_audit.v1",
