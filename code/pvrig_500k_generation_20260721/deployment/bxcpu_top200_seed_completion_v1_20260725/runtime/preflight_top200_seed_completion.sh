...
EXTERNAL_MANIFEST="${PVRIG_TOP200_MANIFEST_PATH:-$HOME/${PROJECT_NAME}.manifest.tsv}"
...
SMOKE_JOB="${PVRIG_TOP200_SMOKE_JOB_ID:-}"
if [[ -z "$SMOKE_JOB" ]]; then
    SMOKE_JOB=$(
...
# The actual 2-task array has already been accepted by Slurm before this
...
payload={
    "schema_version":"pvrig.top200_seed_completion.bxcpu_preflight.v1",
    "status":"PASS_RUNTIME_PACKAGE_2_SHARDS_AND_ONE_JOB_SMOKE",
    "runtime":{"haddock":"2025.11.0","numpy":"2.0.1"},
    "expected_candidates":106,
    "expected_jobs":424,
    "expected_shards":2,
    "expected_jobs_per_shard":212,
    "node_layout":{"nodes":2,"cpus_per_node":64,"concurrent_jobs_per_node":16,"cpus_per_job":4},
    "slurm_array_validation":"accepted_dependency_gated_array_plus_static_2x64_layout; no duplicate sbatch --test-only",
