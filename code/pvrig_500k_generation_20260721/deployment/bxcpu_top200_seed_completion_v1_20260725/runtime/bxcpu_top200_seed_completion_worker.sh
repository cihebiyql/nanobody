...
JOBS_PER_SHARD=212
JOB_CPUS=4
NODE_CONCURRENCY="${PVRIG_TOP200_NODE_CONCURRENCY:-16}"
EXPECTED_JOBS_PER_SHARD="${PVRIG_TOP200_EXPECTED_JOBS_PER_SHARD:-$JOBS_PER_SHARD}"

[[ "$SHARD_INDEX" =~ ^[1-2]$ ]] || pvrig_die "shard index must be 1..2"
[[ "$NODE_CONCURRENCY" == 16 ]] ||
...
[[ "$EXPECTED_JOBS_PER_SHARD" == "$JOBS_PER_SHARD" ]] ||
    pvrig_die "expected jobs per shard must be $JOBS_PER_SHARD"
for name in \
...
if [[ "${PVRIG_TOP200_KEEP_LOCAL_WORK:-0}" != 1 ]]; then
...
marker="$PUBLISH_ROOT/markers/top200_seed_completion_shard_${SHARD_INDEX}.done"
