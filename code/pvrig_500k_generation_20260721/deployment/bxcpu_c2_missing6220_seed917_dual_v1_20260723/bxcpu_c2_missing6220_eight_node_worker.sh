...
PROJECT_NAME=pvrig_c2_only_missing6220_seed917_dual_handoff_v1_20260723
BUNDLE_ARCHIVE="$HOME/${PROJECT_NAME}.tar.zst"
EXPECTED_ARCHIVE_SHA256="${PVRIG_C2_ARCHIVE_SHA256:?sealed archive SHA256 is required}"
PUBLISH_ROOT="${PVRIG_C2_PUBLISH_ROOT:-$HOME/${PROJECT_NAME}_bxcpu_results}"
SHARD_INDEX="${SLURM_ARRAY_TASK_ID:-1}"
SHARD_COUNT=8
NODE_CONCURRENCY="${PVRIG_C2_NODE_CONCURRENCY:-16}"
...
EXPECTED_RECEIPT_SHA256="${PVRIG_C2_BUNDLE_RECEIPT_SHA256:?sealed bundle receipt SHA256 is required}"
EXPECTED_MANIFEST_SHA256="${PVRIG_C2_MANIFEST_SHA256:?sealed manifest SHA256 is required}"
[[ $(sha256sum "$LOCAL_PROJECT/DEPLOYMENT_BUNDLE_RECEIPT.json" | awk '{print $1}') == "$EXPECTED_RECEIPT_SHA256" ]] || {
    echo "bundle receipt SHA256 mismatch" >&2; exit 65;
}
[[ $(sha256sum "$LOCAL_PROJECT/manifests/docking_jobs.tsv" | awk '{print $1}') == "$EXPECTED_MANIFEST_SHA256" ]] || {
    echo "job manifest SHA256 mismatch" >&2; exit 65;
}
"$LOCAL_ENV/bin/python" - "$LOCAL_PROJECT/DEPLOYMENT_BUNDLE_RECEIPT.json" <<'PY_GATE'
import json, sys
...
assert d["candidates"] == 6220 and d["jobs"] == 12440
assert d["shard_sizes"] == [1555] * 8
assert d["seed"] == 917 and set(d["conformations"]) == {"8x6b", "9e6y"}
assert d["docking_started"] is False
...
PY
    ...
    if [[ "$rc" == 0 ]]; then
        ...
        rm -f "$tmp_compact"
        "$LOCAL_ENV/bin/python" "$HOME/.local/share/bxcpu_c2_missing6220_seed917_dual_v1_20260723/compact_run_evidence.py" \
...
marker="$PUBLISH_ROOT/markers/c2_missing6220_12440_shard_${SHARD_INDEX}.done"
