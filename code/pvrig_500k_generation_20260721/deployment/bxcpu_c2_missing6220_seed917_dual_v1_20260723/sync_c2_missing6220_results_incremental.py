...
CAMPAIGNS = {
    "c2_missing6220_seed917": {
        "expected": 12440,
        "remote": "pvrig_c2_only_missing6220_seed917_dual_handoff_v1_20260723_bxcpu_results",
    },
}
LOCAL_ROOT = pathlib.Path(
    os.environ.get(
        "PVRIG_BXCPU_SYNC_LOCAL_ROOT",
        "/mnt/d/work/抗体/node1/pvrig_c2_missing6220_seed917_bxcpu_incremental_spool_20260723",
    )
)
NODE1_ROOT = os.environ.get(
    "PVRIG_BXCPU_SYNC_NODE1_ROOT",
    "/data1/qlyu/projects/pvrig_c2_missing6220_seed917_dual_docking_results_v1_20260723",
)
...
def relay_and_verify(
    ...
    archive = LOCAL_ROOT / "state" / f"{campaign}_{batch_id}.tar.gz"
    partial = archive.with_name(archive.name + ".partial")
    partial.unlink(missing_ok=True)
    archive.unlink(missing_ok=True)
    run(
