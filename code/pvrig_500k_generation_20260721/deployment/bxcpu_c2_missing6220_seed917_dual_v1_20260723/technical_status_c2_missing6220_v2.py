#!/usr/bin/env python3
"""Fail-closed terminal audit V2 for the frozen C2-only 12,440-job campaign."""
...
def audit(*, manifest: pathlib.Path, publish_root: pathlib.Path, expected_count: int,
    ...
    return {
        "schema_version": "pvrig_c2_missing6220_12440_technical_status_v2",
        "status": status,
        ...
        "mismatched_job_id_statuses": mismatched_job_ids,
        "missing_job_ids": missing_jobs,
        "updated_at_utc": datetime.now(timezone.utc).isoformat(),
        "overlap1280_reuse_authorized": False,
        "claim_boundary": (
            "Technical completion of the latest C2-only missing6220 seed917 dual-receptor "
            "campaign only; FAILED states are technical NA, never biological negatives; no "
            "old overlap1280 result is counted or reused."
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("--expected-count", type=int, default=12_440)
    parser.add_argument("--expected-manifest-sha256")
    parser.add_argument("--output", type=pathlib.Path, required=True)
    args = parser.parse_args()
    payload = audit(
        manifest=args.manifest, publish_root=args.publish_root,
