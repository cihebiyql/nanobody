...
PREDECESSOR_AUDIT_DEFAULT=11943297
PREDECESSOR_AUDIT_OVERRIDE_FILE="$PUBLISH_ROOT/markers/CURRENT_AUDIT_OVERRIDE_ID"
PREDECESSOR_AUDIT="${PVRIG_C2_EXTRA_PREDECESSOR_AUDIT:-}"
if [[ -z "$PREDECESSOR_AUDIT" && -s "$PREDECESSOR_AUDIT_OVERRIDE_FILE" ]]; then
    PREDECESSOR_AUDIT=$(cat "$PREDECESSOR_AUDIT_OVERRIDE_FILE")
fi
PREDECESSOR_AUDIT="${PREDECESSOR_AUDIT:-$PREDECESSOR_AUDIT_DEFAULT}"
