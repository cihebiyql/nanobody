...
NODE_CONCURRENCY="${PVRIG_C2_EXTRA_NODE_CONCURRENCY:-16}"

[[ "$SHARD_INDEX" =~ ^[1-8]$ ]] || { echo "shard index must be 1..8" >&2; exit 64; }
[[ "$NODE_CONCURRENCY" == 16 ]] || { echo "worker requires 16 concurrent jobs" >&2; exit 64; }
...
"$LOCAL_ENV/bin/python" - "$LOCAL_PROJECT/HANDOFF_RECEIPT.json" "$MANIFEST" <<'PY_GATE'
import csv,json,sys
r=json.load(open(sys.argv[1]))
rows=list(csv.DictReader(open(sys.argv[2]),delimiter="\t"))
assert r["status"]=="READY_FOR_EXTERNAL_DOCKING_SUBMISSION"
assert r["counts"]["candidates"]==4220 and r["counts"]["jobs"]==16880
assert r["protocol"]["seeds"]==[42,3047]
assert set(r["protocol"]["conformations"])=={"8x6b","9e6y"}
assert len(rows)==16880 and len({x["job_id"] for x in rows})==16880
assert {x["seed"] for x in rows}=={"42","3047"}
assert {x["conformation"] for x in rows}=={"8x6b","9e6y"}
PY_GATE

SHARD_ZERO=$((SHARD_INDEX - 1))
SAFE_MANIFEST=$(printf '%s/manifests/shards_recommended_8/shard_%02d.tsv' "$LOCAL_PROJECT" "$SHARD_ZERO")
mapfile -t JOB_IDS < <(awk -F'\t' 'NR>1{gsub(/\r$/, "", $1); print $1}' "$SAFE_MANIFEST")
[[ ${#JOB_IDS[@]} -eq 2110 ]] || {
    echo "unexpected shard size ${#JOB_IDS[@]} expected 2110" >&2
    exit 65
}
...
marker="$PUBLISH_ROOT/markers/seed42_3047_shard_${SHARD_INDEX}.done"
printf 'array_job=%s shard=%s assigned=%s skipped=%s failures=%s completed_at=%s\n' \
    "${SLURM_ARRAY_JOB_ID:-manual}" "$SHARD_INDEX" "${#JOB_IDS[@]}" \
