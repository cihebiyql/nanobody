...
def render_restraints(cdr_residues: list[str], core_hash: str) -> str:
    anchors = anchor_positions()
    lines = [
        f"! protocol_core_sha256={core_hash}",
        "! VHH CDR residues (source chain H, runtime chain A) to 12 UniProt-numbered PVRIG AIR anchors (chain T)",
...
def render_restraints_from_job(job: dict[str, str]) -> str:
    residues = [value.strip() for value in job["cdr_residues"].split(",") if value.strip()]
