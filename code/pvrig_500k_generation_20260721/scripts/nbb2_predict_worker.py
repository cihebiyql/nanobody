...
def retryable_prediction_failure(exc: Exception) -> bool:
    message = str(exc)
    return isinstance(exc, ValueError) and (
        "No template found for residue" in message
        or message in {"PDB_SEQUENCE_MISMATCH", "TOO_FEW_ATOMS"}
    )
...
def main() -> None:
    ...
    parser.add_argument("--retryable-attempts", type=int, default=3)
    ...
    with manifest_partial.open("w", newline="") as handle:
        ...
        for candidate_id, sequence in records:
            ...
            try:
                if args.retryable_attempts < 1:
                    raise ValueError("retryable-attempts must be positive")
                for attempt in range(1, args.retryable_attempts + 1):
                    try:
                        ...
                        if pdb.exists() and (
                            pdb.stat().st_size < 10_000
                            or str(exc) in {"PDB_SEQUENCE_MISMATCH", "TOO_FEW_ATOMS"}
                        ):
                            pdb.unlink(missing_ok=True)
                        if attempt >= args.retryable_attempts or not retryable_prediction_failure(exc):
                            raise
            except Exception as exc:
                status = "TECHNICAL_NA"
