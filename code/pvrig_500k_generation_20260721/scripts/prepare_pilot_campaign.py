...
def main() -> int:
    ...
    parents = read_csv(args.parents)
    if len(parents) != 200:
        raise ValueError(f"expected exactly 200 parents, found {len(parents)}")
    if len({row["sequence_aa"] for row in parents}) != 200:
        raise ValueError("Top200 contains duplicate sequences")
    if len({row["cluster_id"] for row in parents}) != 200:
    ...
    if not required.issubset(parents[0]):
        raise ValueError(f"parent table missing fields: {sorted(required - set(parents[0]))}")
    ...
    summary = {
        ...
        "new_parent_raw_task_fraction": new_parent_tasks / len(tasks),
    ...
    checksum_paths = sorted(path for path in campaign.rglob("*") if path.is_file())
