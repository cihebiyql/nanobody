...
# N is omitted in this production route because the predeclared QC rejects
# newly introduced N-X-S/T motifs.  The CX exploratory arm passed 0/32 in the
# matched smoke, while CXN passed 30/32; retaining it would waste most CPU.
OMIT_AAS = ("CXN",)
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "omit_aas": OMIT_AAS[index % len(OMIT_AAS)],
    ...
    summary = {
        ...
        "omit_aas_counts": dict(sorted(Counter(str(row["omit_aas"]) for row in rows).items())),
