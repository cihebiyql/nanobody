...
这里区分三种状态：`task manifest 已物化`、`generator 已验证可执行`、`有效序列已冻结`。五条路线的 task manifest 可以先全部物化，但未通过部署 smoke 的路线必须保持 BLOCKED；不能把“已有 25k 任务定义”写成“已有 25k 可执行生成器”或“已经生成 25k 序列”。
...
本 campaign 不直接使用该错序字符串做序列替换，而是用原始抗体序列中 FR3 末端 conserved-C 后、按已编号 CDR3 长度恢复 sequence-order CDR3，并把原表和恢复值都保留用于审计。本轮 finalizer 已明确实现 IMGT CDR2 60/61 与 CDR3 111/112 的两侧 insertion sequence-order，并有 scrambled-header 回归测试。旧表不能静默覆盖。

## 14. 2026-07-21 启动后的已验证进度

- conservative CDR redesign：5,000 条有效冻结；
- natural CDR donor：5,000 条有效冻结；
- 本地两路线合计 10,000 条，exact unique=10,000，ANARCI/IMGT、FR/CDR、Cys、阳性 CDR 相似性与 CDR3-family cap 门禁通过；
- Node1 RFantibody 路线已在 `/data1/qlyu/projects/pvrig_500k_rfantibody_pilot_v1_20260721` 使用 6 张 GPU 生成；
- fixed-pose/AntiFold 和 de novo/disagreement 仍保持 BLOCKED，不把未验证的任务定义写成已生成序列。

本地有效集与终态证据位于：

```text
pvrig_500k_generation_20260721/run/pvrig_500k_generation_pilot25k_v1_20260721/qc/local_cpu_routes_effective.tsv
pvrig_500k_generation_20260721/run/pvrig_500k_generation_pilot25k_v1_20260721/status/LOCAL_CPU_ANARCI_TERMINAL.json
...
因此本轮 RFantibody 输出会保留原始 `patch_id=P1...P6`；在进入 25k 合并 release 前另建显式映射/分层表。未经结构几何核验，不允许把 P1–P6 静默改名为 A/B/C，也不允许宣称已满足正式 500k 的 A/B/C 40/35/25 配额。
