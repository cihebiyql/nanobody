...
所以，“运行官方权重进行设计”是可复现的；“从零严格重训出完全相同的 RFdiffusion 和 RF2 权重”目前并不完全可复现。
...
若使用自己的 scaffold，应先做 Chothia 编号并转换为 HLT。当前 README 给出的转换工具名是：

```bash
python scripts/util/chothia_to_HLT.py -inpdb mychothia.pdb -outpdb myHLT.pdb
```

实际运行前应以当前仓库 `scripts/util/` 中的文件名为准，因为历史版本曾出现大小写或命名差异。
