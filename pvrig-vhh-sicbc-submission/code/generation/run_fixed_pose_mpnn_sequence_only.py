...
def generate(args: argparse.Namespace, task: dict[str, str]) -> list[dict[str, object]]:
    ...
    try:
        ...
        # RFantibody keeps the full input path (without the .pdb suffix) as
        # the feature name, so the fixed-position dictionary must use it too.
        fixed_positions = {str(feature_pdb)[:-4]: features.fixed_res}
        ...
        for start in range(0, requested, args.batch_size):
            ...
            omit_aas = task.get("omit_aas", "CX")
            settings = mpnn_util.set_default_args(count, omit_AAs=list(omit_aas), allow_x=False)
