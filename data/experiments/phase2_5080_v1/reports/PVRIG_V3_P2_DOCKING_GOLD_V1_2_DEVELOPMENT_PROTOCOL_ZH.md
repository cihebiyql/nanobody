...
→ 52-run V1.1 emref reuse/rerun regression cohort PASS
...
V1.1 sync audit 已证明这 52 个 run 的 `emref` 输出数为 50 x 10 + 2 x 9，都满足 fixed Top-8 的数量要求。因此 V1.2 **不默认重跑 HADDOCK**，而是先对已有 `4_emref` 阶段做闭包回收：

1. 核对原始 config、monomer、receptor、restraint、seed 与 V1.1 manifest 哈希；
2. 核对 `4_emref/io.json` 及其引用的全部 pose 文件存在、非空、可解析且哈希闭包；
3. 若某 run 的 input/stage/coordinate 闭包全部 PASS，直接以 `execution_mode=REUSE_HASH_CLOSED_EMREF` 进入 V1.2 development postprocess；
4. 只有原始 emref 坐标缺失、损坏或哈希/输入闭包失败时，才以完全冻结的 V1.2 development package 重跑该 run，并记录 `execution_mode=RERUN_HADDOCK_REQUIRED`；
5. 回收/重跑选择只依赖输入与文件完整性，不得依赖遮挡、tier 或候选身份。

这不是对 V1.1 final 的“回填”：V1.2 对所有 run 都使用同一 emref Top-8 语义，且新产物只进入 development namespace。

52-run failure-regression cohort 必须：

1. 使用同一个 V1.2 development schema、全局配置、固定 K 和 scorer/rules 哈希；
2. 全部 52/52 run 产生恰好 8 个 canonical emref pose；
3. 不得因 candidate、receptor 或 seed-role 改变容差、K 或排序法；
4. 无 flexref/rigidbody/final 跨 stage 补 pose；所有 canonical pose 仅来自 emref；
5. 生成独立 V1.2 completion/postprocess audit；
6. 仍然标记 `development_only`，不得成为 formal holdout。

不得将这种事后回收或重跑写回 V1.1 Gold。
...
除 Pilot64 外，V1.2 必须纳入两个已有但严格 leakage-excluded 的校准 cohort：

| cohort | cases | 当前 `6_seletopclusts` pose rows | V1.2 角色 |
| --- | ---: | ---: | --- |
| patent success anchors | 11 | 109 | 已知成功家族、success anchor、family sensitivity |
| mutant/perturbation panel | 36 | 357 | 几何敏感性、局部扰动和 parser/plumbing controls |

36-case mutant panel 包含 7 个 base reference、7 个 conservative CDR3 change、7 个 aromatic-to-alanine change、7 个 center multi-alanine scan、7 个 conservative framework control，以及 1 个 family CDR3 stability-delta control。其中 7 条是 exact-known-positive，29 条是 near-known-positive；全部都是 leakage/robustness controls，不是新设计候选。

**任何 mutant 都不能因“设计意图是破坏”、alanine scan 或 center perturbation 而被预设为负样本。**它的 V1.2 几何标签必须完全来自 fixed Top-8 + ATOM-only 重算；如果仍保留 blocker-like 几何，必须保留实际计算结果，不得为了制造正负平衡而翻转标签。

Pilot64 中的 21 个 matched controls 和 32 个 Teacher500 stratified rows 必须与上述 11+36 核心校准面板分开：它们是 **development-distribution cohorts**，用于检查分布外推、生成器/来源偏差和规则的稳定性，不得与 36-mutant panel 合并成“实验负例”。
...
不再要求“至少 2 个 final cluster 才有 label”。如果 Top-8 收敛到一个几何盆地，该现象应作为低多样性/高采样集中度证据进入聚合报告，而不是丢弃整个候选；它本身也不应被自动解释为“高确定性”。
...
4. 现有 11-positive 的 109 个对齐 pose 和 36-mutant 的 357 个对齐 pose 都来自旧 `6_seletopclusts` 可变分母；它们只可用于验证 ATOM-only scorer 的敏感性、record inventory 和数值闭包。
5. 不得使用这 109+357 个旧 final aligned poses 冻结 V1.2 的阈值、R_gold 或 tier rules。

### Step 4：使用 V1.2 canonical pose 重建 calibration table

校准表必须从 fixed Top-8 + ATOM-only 全流程重建，而不能只替换旧表中的几个列。核心 threshold/rules freeze cohort 是 **11 个 success anchors + 36 个 mutant/perturbation controls**。

对这 47 个 case：

1. 先查找并闭包审计它们原始 `4_emref/io.json` 与对应坐标；
2. 若某 case 的 emref 输出数 >=8，且输入、io.json、坐标与哈希全部闭包，则按第 7 节从该阶段确定性取 Top-8；
3. 若原 emref 坐标不存在或无法闭包，必须在冻结 V1.2 development package 下重跑该 case，不得用旧 final pose 替代；
4. 47/47 case 都必须产生恰好 8 个 canonical emref pose 后，才能启动阈值拟合；
5. 所有 reuse/rerun 状态必须记录在 calibration audit 中。

校准角色为：

- 11 个 known positive：success anchors 和 family-level sensitivity；
- 36 个 mutant/perturbation control：base reference、conservative、alanine/center 和 framework 几何敏感性；标签只来自重算，不预设负类；
- 21 个 Pilot64 matched control：独立 development-distribution cohort，不并入 mutant 核心校准表；
- 32 个 Teacher500 stratified：独立 development-distribution cohort，检查几何/生成器分布，不当作实验阳性或负性。
...
## 10. 52-run emref recovery/failure-regression 门

8-run smoke PASS 后，使用冻结 RC 处理 V1.1 的全部 52 个失败 run。执行顺序是：**先闭包回收已有 emref，只对无法闭包的 run 重跑 HADDOCK**。

通过条件：

```text
52/52 emref source closure or preregistered rerun PASS
...
execution_mode recorded for 52/52 runs
```

这个 cohort 是 V1.2 对已知最困难运行的 regression test。它的结果可用于 development，但不可成为 formal 重复性评估。

对于 `REUSE_HASH_CLOSED_EMREF`，“runtime completion”指原 emref stage 和新 V1.2 postprocess 都闭包，不要求为了改变 protocol ID 而无意义地重算同一批 docking 坐标。对于 `RERUN_HADDOCK_REQUIRED`，则必须记录触发重跑的客观闭包失败原因。

如果其中任一 run 在可回收或重跑后的 emref 仍少于 8 个合法 pose，必须判定整个 failure-regression gate FAIL；不得从其他 stage 补齐。
...
8-run smoke PASS
52-run emref recovery/failure-regression PASS
...
calibration-only success/mutant/matched-control cohorts excluded from ordinary training
```
...
| 11 known positive + 36 mutant/perturbation controls | 核心 calibration/sensitivity，不进入普通训练；mutant 不预设负类 |
| 21 Pilot64 matched controls | 独立 development-distribution cohort，不与 mutant 合并为负例，不进入普通训练 |
| 32 Pilot64 Teacher500 stratified rows | V1.2 fixed Top-8 全合同 PASS 后可作 non-formal development rows |
...
| D5 regression | 52-run emref recovery/rerun audit | 52/52 PASS，无回填/豁免 |
...
下表中 `experiments/...` 路径相对于 `/mnt/d/work/抗体/data`；`/mnt/d/work/抗体/docking/...` 为校准面板的绝对路径。
...
| `/mnt/d/work/抗体/docking/calibration/patent_success_validation/BATCH_OUTPUT_INTEGRITY_REPORT.md` | `c5820c102fa2bae32b54403c3d29df2a9ea2442f08d3c894cbb3dc1e49ce1fed` | 11 success cases、109 current final pose rows、0 failures |
| `/mnt/d/work/抗体/docking/calibration/mutant_validation_panel/MUTANT_PANEL_COMPLETION_VALIDATION.md` | `3fe1e0510caf7f310ce89d61c71dba5fa7d0a92ac85bd7915fd278ba494c5765` | 36 perturbation cases、357 current final pose rows、7 exact + 29 near positives、0 failures |
| `/mnt/d/work/抗体/docking/calibration/mutant_validation_panel/mutant_panel.csv` | `81f42361be2e31dd8a083eb5cf28b35e1d09292635801a9a021fbe29b1d19248` | base/conservative/alanine-center/framework/stability-delta roles |
...
The old 109+357 final poses are scorer-sensitivity inputs, not threshold-freeze labels.
Threshold freeze requires deterministic emref Top-8 rebuilds for all 11+36 calibration cases.
Mutant intent never supplies a negative label; recomputed geometry does.
All continuous metrics and tier thresholds must be recalibrated before freeze.
The frozen 8-run smoke must pass in full.
All 52 prior failure runs must pass hash-closed emref recovery or preregistered rerun.
