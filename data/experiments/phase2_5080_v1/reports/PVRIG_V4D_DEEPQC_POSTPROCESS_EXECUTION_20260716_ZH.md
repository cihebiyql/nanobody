...
- 2026-07-16 14:59 快照：`390 SUCCESS / 12 RUNNING / 1620 PENDING / 0 FAILED`；controller PID `265751`。
...
- 当前 packager SHA256：`66a2b7b502abe370eadd66f93eec09676888b07d1bef54609fe74809dd8081d8`
...
### 2.6 Top50/Top10 deterministic release 与 clean replay

脚本：`src/prepare_pvrig_submission_release.py`

当前已提前固定最终计算发布合同：

- 只接受正好 50 条、rank 1..50 的 open FullQC290 shortlist；
- 重新计算 Top50 parent、parent+patch+mode 和 CDR3 caps，不信任未绑定的 audit 自报值；
- geometry shortlist audit 必须以 SHA256 绑定实际 `shortlist50.tsv`；
- 只接受真实 Top20 `360` pose manifest，且 audit 必须绑定 manifest SHA256；
- candidate、conformation、seed、model、job ID/hash 和 canonical bundle path 必须逐行闭合，拒绝跨候选 pose 替换及重复 target；
- 20 条 pose verdict 必须完整；Top10 只允许 `ACCEPT_COMPUTATIONAL_PRIORITY` 或 `ACCEPT_DIVERSITY_HEDGE`；
- Top10 必须正好 10 条、来自 pose-reviewed Top20，并满足至少 5 个 CDR3 clusters、至少 4 个 parent families、同 cluster/parent 最多 2 条和三类 target patch 全覆盖；
- 发布目录包含 Top50/Top10 FASTA/CSV/TSV、lineage、evidence、Top10 dossier、冻结的 Top20 360 poses、SHA256SUMS 和确定性 tar.gz；
- `clean_replay.sh` 先校验当前目录，再在干净临时目录重建并要求 SHA256SUMS 与 tar.gz 字节一致。

当前 builder SHA256：`15db6a043434b99278ff4eb1bd919262881b587df8b2230be084be46905d6453`。该工具不会在上游 pose 或人工计算姿势 verdict 不完整时生成 READY release。
...
### 3.5 Submission release watcher

- 脚本：`src/monitor_pvrig_submission_release.sh`
- tmux session：`pvrig-submission-release`
- controller PID：`1925357`
- 当前状态：`WAITING_UPSTREAM`
- 当前原因：等待 open258 Top50 和 hash-bound Top20 pose bundle。

上游完成后，该 watcher 自动生成 20 条 pose verdict 和 Top10 selection 的填写模板；模板通过 `review_context_id` 绑定 shortlist、geometry audit、pose manifest、pose audit 和 Top20 ID/rank。计算姿势复核输入完整后，自动运行受限时的 release builder、clean replay 和最终哈希 receipt；旧 context、伪 upstream receipt、伪 existing release 和无界执行均会 fail-closed。
...
- 原 V4-D/Deep-QC/postprocess 合同：43 tests；
- submission release 与 clean replay 合同：14 tests；
- submission watcher 集成合同：4 tests；
- 总计：`61 tests = 60 PASS + 1 optional remote skipped`。

新增发布回归覆盖：sealed-before-pose、Top50 audit/hash 欺骗、跨候选 pose 替换、non-Top10 缺 pose、duplicate model/target、claim boundary drift、known-positive、Top10 verdict/diversity、当前目录篡改和 clean replay 字节一致性。

此外：

- Python `py_compile`：PASS；
- 两个 local watcher `bash -n`：PASS；
- `git diff --check`：PASS；
- release builder 与 submission watcher 两轮独立 code review 最终结论均为：`APPROVE — no remaining HIGH/MEDIUM`；
...
11. 生成 pose review/Top10 选择模板；
12. 计算姿势 verdict 完整后构建 Top50/Top10 release 并执行 clean replay。
