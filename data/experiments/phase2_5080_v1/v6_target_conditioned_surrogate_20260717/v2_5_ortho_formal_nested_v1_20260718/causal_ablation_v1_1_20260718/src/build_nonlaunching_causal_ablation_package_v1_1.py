#!/usr/bin/env python3
"""Build the immutable, nonlaunching V2.5 causal-ablation hardened V1.1 plan package."""
...
def build_job_graph() -> dict[str, Any]:
    ...
    for outer in OUTER_FOLDS:
        for perturbation in INFERENCE_PERTURBATIONS:
            ...
            for seed in SEEDS:
                ...
                job = {
                    ...
                    "runner_interface": "causal_inference_adapter_v1_1_REQUIRED_FUTURE_VERSION",
                }
                if perturbation == "HOTSPOT_INTERFACE_MASK_SWAP":
                    job["required_null_control"] = "MATCHED_CARDINALITY_PREVALENCE_MASK_POSITION_NULL"
                    job["mask_null_replicates"] = MASK_NULL_REPLICATES
                    job["mask_null_master_seed"] = MASK_NULL_MASTER_SEED
                    job["mask_null_rescore_uses_same_frozen_contact_map"] = True
                    job["mask_null_requires_additional_model_forward"] = False
                jobs.append(job)
            ...
            jobs.append({
                ...
                "execution_authorized": False,
                "runner_interface": "causal_ensemble_adapter_v1_1_REQUIRED_FUTURE_VERSION",
            })
            ...
            jobs.append({
                ...
                "execution_authorized": False,
                "runner_interface": "causal_meta_evaluator_v1_1_REQUIRED_FUTURE_VERSION",
            })
        ...
        for inner in INNER_FOLDS:
            ...
            jobs.append({
                ...
                "seed": 43,
                "donor_seed": 1931,
                "pre_optimizer_payload_power_audit_required": True,
                "audit_scope": "CURRENT_INNER_TRAIN_ONLY",
                "audit_failure_policy": "FAIL_CLOSED_BEFORE_MODEL_OR_OPTIMIZER_INITIALIZATION",
                "perturbation": "CONTACT_LABEL_WITHIN_PARENT_DONOR_SHUFFLE",
                "dependencies": [],
                "external_dependencies": [_external_clean_selection(outer)],
                "execution_authorized": False,
                "runner_interface": "contact_donor_shuffle_training_adapter_v1_1_REQUIRED_FUTURE_VERSION",
            })
        ...
        for seed in SEEDS:
            ...
            jobs.append({
                ...
                "donor_seed": 1931,
                "pre_optimizer_payload_power_audit_required": True,
                "audit_scope": "CURRENT_OUTER_TRAIN_ONLY",
                "audit_failure_policy": "FAIL_CLOSED_BEFORE_MODEL_OR_OPTIMIZER_INITIALIZATION",
                "perturbation": "CONTACT_LABEL_WITHIN_PARENT_DONOR_SHUFFLE",
                "dependencies": [],
                "external_dependencies": [_external_clean_selection(outer)],
    ...
    jobs.append({
        ...
        "external_dependencies": [],
        "execution_authorized": False,
        "runner_interface": "causal_final_collector_v1_1_REQUIRED_FUTURE_VERSION",
    })
    ...
    return {
        "schema_version": "pvrig_v2_5_causal_ablation_nonlaunching_job_graph_v1_1",
        ...
        "v1_1_hardening_embedded_without_extra_jobs": {
            "mask_null_bank_jobs": 15,
            "mask_null_replicates_per_job": MASK_NULL_REPLICATES,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    contract_path = source / "CAUSAL_ABLATION_CONTRACT_V1_1.json"
    ...
    for name in (
        "causal_perturbations_v1_1.py",
        "validate_causal_ablation_package_v1_1.py",
        "watch_formal_terminal_then_mark_ablation_ready_v1_1.py",
    ):
    ...
    manifest = {
        "schema_version": "pvrig_v2_5_causal_ablation_nonlaunching_package_v1_1",
        "status": "PASS_IMMUTABLE_NONLAUNCHING_PLAN_BUILT",
        "launch_authorized": False,
        "training_or_prediction_executed": False,
        "contract_sha256": files["CAUSAL_ABLATION_CONTRACT_V1_1.json"],
        ...
        "gpu_job_count": 85,
        "cpu_job_count": 46,
        "mask_null_bank_job_count": 15,
        "mask_null_replicates_per_job": MASK_NULL_REPLICATES,
        "contact_shuffle_pre_optimizer_audit_count": 40,
