...
def seed_before_model_construction(seed: int) -> None:
    """Make checkpoint-missing HF modules byte-stable across processes."""

    require(seed == SEED, "production seed must be 43")
    random.seed(seed)
    np.random.seed(seed)
...
def prepare_production_inputs(args: argparse.Namespace) -> tuple[FoldConfig, FoldInputs]:
    # ESM2 checkpoints do not contain the optional pooler weights.  Transformers
    # constructs that unused module randomly, so seeding only inside
    # run_fold_core is too late for the separately bound backbone runtime hash.
