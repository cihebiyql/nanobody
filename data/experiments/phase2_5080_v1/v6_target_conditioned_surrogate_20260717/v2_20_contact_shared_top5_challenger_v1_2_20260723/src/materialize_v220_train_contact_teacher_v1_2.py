...
SCHEMA = "pvrig_v2_20_phase0_train_only_contact_teacher_v1_2"
STATUS = "PASS_TRAIN_ONLY_738_CONTACT_TEACHER_MATERIALIZED_V1_2"
CLAIM = (
...
def validated_v29_pose_count(row: Mapping[str, Any]) -> int:
    selected=int(row["selected_model_count"]); fixed=int(row["fixed_top8_count"])
    require(selected>=MINIMUM_POSES and fixed==min(selected,TOP_K_MAX),"v29_selected_pose_count")
    return fixed
...
def process_v29_job(task:Mapping[str,Any],guard:AccessGuard,pose_root:Path,target_map:Mapping[str,Mapping[int,tuple[int,str]]])->dict[str,Any]:
    ...
    models=[x for x in row["top8_model_ids"].split(",") if x]; expected_count=validated_v29_pose_count(row); require(MINIMUM_POSES<=expected_count<=TOP_K_MAX and len(models)==expected_count and len(set(models))==expected_count,"v29_top8_models")
...
def materialize(*,scalar_teacher:Path,split_manifest:Path,phase0_contract:Path,implementation_freeze:Path,v4d_pair:Path,v4d_marginal:Path,v4d_receipt:Path,v4h_pair:Path,v4h_marginal:Path,v4h_state:Path,v4h_receipt:Path,v29_release:Path,v29_pose_root:Path,target_cache:Path,target_manifest:Path,target_receipt:Path,output_dir:Path,workers:int=8)->dict[str,Any]:
    ...
    for r in v29_job_rows:
        ...
        pose_count=validated_v29_pose_count(r)
