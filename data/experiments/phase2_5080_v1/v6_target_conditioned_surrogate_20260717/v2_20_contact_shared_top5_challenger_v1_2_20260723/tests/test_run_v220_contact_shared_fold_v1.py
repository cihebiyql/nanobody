...
class V220FoldRunnerTests(unittest.TestCase):
    def test_production_model_construction_is_seeded_before_backbone_load(self) -> None:
        source = inspect.getsource(runner.prepare_production_inputs)
        self.assertLess(
            source.index("seed_before_model_construction(args.seed)"),
            source.index("base.load_backbone(args)"),
        )
        runner.seed_before_model_construction(43)
        first = torch.rand(8)
        runner.seed_before_model_construction(43)
        second = torch.rand(8)
        self.assertTrue(torch.equal(first, second))
        with self.assertRaises(runner.V220FoldError):
            runner.seed_before_model_construction(44)
