...
ROOT=HERE.parents[1]
MODULE_PATH=ROOT/'src/materialize_v220_train_contact_teacher_v1_1.py'
...
class Phase0FourteenTests(unittest.TestCase):
    ...
    def test_03_v29_top8_coordinate_hash_and_contact_closure(self)->None:
        ...
        row={'job_id':job,'conformation':'8x6b','seed':'917','top8_model_ids':','.join(models),'fixed_top8_count':'4'}
        ...
        self.assertEqual(guard.audit()['train_pose_files_opened'],4);self.assertAlmostEqual(sum(float(x['pose_weight']) for x in result['inventory']),1.0,places=10);self.assertEqual(len(MOD.raw_pose_weights(8)),8)
    ...
    def test_06_frozen_primary_join_count_and_source_contract(self)->None:
        ...
        contract=json.loads((ROOT/'PHASE0_TEACHER_MATERIALIZATION_CONTRACT_V1_1.json').read_text())
