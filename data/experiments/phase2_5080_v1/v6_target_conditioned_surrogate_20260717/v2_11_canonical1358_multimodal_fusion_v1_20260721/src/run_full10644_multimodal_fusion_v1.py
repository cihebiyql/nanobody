...
def run(args: argparse.Namespace) -> dict[str,Any]:
    ...
    require(receipt.get("status")=="PASS_CANONICAL10644_OPEN_MULTIMODAL_MATERIALIZED","materialization_status")
    counts=receipt.get("counts",{});features=receipt.get("features",{});output=receipt.get("output",{})
    receipt_rows=counts.get("rows",receipt.get("rows"));split_counts=counts.get("splits",{})
    require(int(receipt_rows)==len(rows),"materialization_rows")
    require(output.get("sha256",receipt.get("output_table_sha256"))==args.expected_multimodal_sha256,"materialization_table_hash")
    structure_names=list(features.get("m2_feature_names",receipt.get("structure_feature_names",[])));c2_names=[f"C2__{n}" for n in features.get("coarse_model_feature_names",receipt.get("coarse_model_feature_names",[]))]
    ...
    require(len(train_rows)==int(split_counts.get("train",receipt.get("train_rows"))) and len(dev_rows)==int(split_counts.get("development",receipt.get("development_rows"))),"split_counts")
