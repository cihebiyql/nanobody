...
MAT=load('v211mat2',ROOT/'src/materialize_canonical_multimodal_v1.py');RUN=load('v211runfull',ROOT/'src/run_full10644_multimodal_fusion_v1.py')
class Tests(unittest.TestCase):
 def test_end_to_end_and_full_s0_base(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   table=prepared/'canonical_multimodal_open.tsv';receipt=prepared/'MATERIALIZATION_RECEIPT.json';payload=json.loads(receipt.read_text());payload['status']='PASS_CANONICAL10644_OPEN_MULTIMODAL_MATERIALIZED';payload['counts']={'rows':payload.pop('rows'),'splits':{'train':payload.pop('train_rows'),'development':payload.pop('development_rows')}};payload['features']={'m2_feature_names':payload.pop('structure_feature_names'),'coarse_model_feature_names':payload.pop('coarse_model_feature_names')};payload['output']={'sha256':payload.pop('output_table_sha256')};receipt.write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n');out=root/'run';argv=['--multimodal-tsv',str(table),'--expected-multimodal-sha256',sha(table),'--materialization-receipt',str(receipt),'--expected-materialization-receipt-sha256',sha(receipt),'--esm2-650m-cache',str(f['cache']),'--output-dir',str(out),'--folds','4','--c2-alphas','0.1,1','--gbdt-min-samples-leaf','8',*preds]
