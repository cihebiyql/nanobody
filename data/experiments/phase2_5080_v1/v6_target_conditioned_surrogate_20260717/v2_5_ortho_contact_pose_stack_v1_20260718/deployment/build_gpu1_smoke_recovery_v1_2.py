#!/usr/bin/env python3
"""Build the isolated V1.2 recovery source/overlay/autostart packages.

V1.2 retains the V1.1 dynamic-import recovery and changes only the empty
full-pair mini-batch behavior: no eligible pair weight yields a differentiable
zero contact component.  Every batch with eligible pair mass keeps the same
balanced BCE algorithm and weights.  Model, data, split, hyperparameters,
...
ROOT = HERE.parents[1]
DATE = "20260718"
SOURCE_REMOTE_ROOT = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_smoke_package_v1_2_{DATE}"
OVERLAY_REMOTE_ROOT = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_gpu1_sequential_overlay_v1_2_{DATE}"
RUNTIME_REMOTE_ROOT = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_gpu1_sequential_smoke_runtime_v1_2_{DATE}"
AUTH_REMOTE_PATH = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_gpu1_sequential_operator_authorization_v1_2_{DATE}.json"
AUTOSTART_REMOTE_ROOT = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_gpu1_recovery_autostart_package_v1_2_{DATE}"
WATCH_REMOTE_ROOT = f"/data1/qlyu/projects/pvrig_v2_5_ortho_heads_gpu1_recovery_watch_v1_2_{DATE}"
...
AUTH_TOKEN = "I_ACCEPT_V2_5_GPU1_SEQUENTIAL_ONE_EPOCH_REAL_SMOKE_V1_2_RECOVERY"
CLAIM_BOUNDARY = (
...
"""Fail-closed V1.2 sequential recovery launcher."""
...
def run():
    ...
    require(auth.get("status") == "EXPLICITLY_AUTHORIZED_FOR_ONE_GPU1_SEQUENTIAL_REAL_SMOKE_V1_2_RECOVERY", "authorization_status")
    ...
    write_final(
        "PASS_GPU1_SEQUENTIAL_REAL_SMOKE_V1_2_RECOVERY",
...
def main():
    try:
        ...
        write_final(
            "FAIL_GPU1_SEQUENTIAL_REAL_SMOKE_V1_2_RECOVERY",
...
def autostart_source(
    *,
    ...
    template = r'''#!/usr/bin/env python3
"""Checksum-closed V1.2 recovery autostart; waits for child terminal."""
import hashlib, json, os, subprocess, traceback
from pathlib import Path
...
def run():
    ...
    atomic_json(WATCH_ROOT / "STATUS.json", {
        "status": "VERIFYING_PILOT_AND_V1_2_PACKAGE_FILE_CLOSURE",
        "physical_gpu": 1,
        "v4_f_test32_access_count": 0,
    })
    ...
    authorization = {
        "status": "EXPLICITLY_AUTHORIZED_FOR_ONE_GPU1_SEQUENTIAL_REAL_SMOKE_V1_2_RECOVERY",
    ...
    with stdout_path.open("w") as stdout, stderr_path.open("w") as stderr:
        ...
        atomic_json(WATCH_ROOT / "LAUNCH_RECEIPT.json", {
            "status": "PASS_V1_2_RECOVERY_CHILD_LAUNCHED_WAITING_TERMINAL",
            "pid": process.pid,
            "source_files_verified": source_files,
            "overlay_files_verified": overlay_files,
            "source_sha256s_sha256": EXPECTED_SOURCE_SHA256S_SHA,
    ...
    require(child.get("status") == "PASS_GPU1_SEQUENTIAL_REAL_SMOKE_V1_2_RECOVERY", "child_terminal_status")
    require(child.get("v4_f_test32_access_count") == 0, "child_sealed_access")
    write_watch(
        "PASS_GPU1_RECOVERY_AUTOSTART_V1_2_CHILD_TERMINAL_VERIFIED",
...
def main():
    try:
        ...
        write_watch(
            "FAIL_GPU1_RECOVERY_AUTOSTART_V1_2",
            error_type=type(exc).__name__,
            error=str(exc),
            traceback=traceback.format_exc(),
        )
...
def build_recovery(source_evidence_root: Path, output_parent: Path) -> dict[str, Any]:
    source_out = output_parent / "node1_smoke_source_package_v1_2"
    overlay_out = output_parent / "gpu1_sequential_overlay_v1_2"
    autostart_out = output_parent / "gpu1_recovery_autostart_package_v1_2"
    for path in (source_out, overlay_out, autostart_out):
        require(not path.exists(), f"output_exists:{path}")

    source_builder = load_module(HERE.with_name("build_node1_smoke_package_v1.py"), "v25_source_builder_v1_2")
    source_builder.SCHEMA_VERSION = "pvrig_v2_5_ortho_node1_smoke_package_v1_2"
    ...
    rewrite_json(
        ...
        {
            "schema_version": "pvrig_v2_5_ortho_node1_smoke_nonlaunching_plan_v1_2",
            "recovery_change_scope": "retain_v1_1_dynamic_import_fix_plus_differentiable_zero_for_empty_full_pair_batch",
        },
    )
    manifest = json.loads((source_out / "PACKAGE_MANIFEST.json").read_text())
    manifest.update(
        {
            "schema_version": "pvrig_v2_5_ortho_node1_smoke_package_v1_2",
            "recovery_change_scope": "retain_v1_1_dynamic_import_fix_plus_differentiable_zero_for_empty_full_pair_batch",
            "supersedes_failed_remote_v1_1": True,
        }
    )
    ...
    require(
        ...
        == "af93c39054a1a73568a68d498406fb3eddbffe1d688c93e16f59319148e285b0",
        "trainer_hash_changed",
    )

    overlay_builder = load_module(
        HERE.with_name("build_gpu1_sequential_authorization_overlay_v1.py"),
        "v25_overlay_builder_v1_2",
    )
    overlay_builder.SCHEMA_VERSION = "pvrig_v2_5_gpu1_sequential_explicit_authorization_overlay_v1_2"
    ...
    autostart = autostart_out / "src" / "launch_gpu1_smoke_recovery_after_pilot_v1_2.py"
    ...
    autostart_manifest = {
        "schema_version": "pvrig_v2_5_gpu1_recovery_autostart_package_v1_2",
    ...
    autostart_hashes = audit_file_closure(
        autostart_out,
        {"PACKAGE_MANIFEST.json", "src/launch_gpu1_smoke_recovery_after_pilot_v1_2.py"},
    )
    return {
        "schema_version": "pvrig_v2_5_gpu1_smoke_recovery_build_v1_2",
        "status": "PASS_V1_2_RECOVERY_PACKAGES_BUILT_AUDITED_NOT_DEPLOYED_NOT_LAUNCHED",
