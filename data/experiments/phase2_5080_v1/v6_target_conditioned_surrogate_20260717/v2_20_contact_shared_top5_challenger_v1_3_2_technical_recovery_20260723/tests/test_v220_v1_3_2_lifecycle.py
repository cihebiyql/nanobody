...
PREFLIGHT = load("v131_preflight_test", "src/build_v220_v1_3_2_preflight_receipt.py")
FINALIZER = load("v131_finalizer_test", "src/finalize_v220_v1_3_2_training_authorization.py")
HELPER = ROOT / "launchers/run_shared_fold_materialization_once_v1_3_1.sh"
TEMPLATE = ROOT / "launchers/run_phase1_core_fold_pair_node1_v1_3_2.template.sh"
PREREG = ROOT / "PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3_2.json"
LEGACY_ADAPTER = ROOT / "launchers/run_legacy_102_tests_python311_v1_3_2.sh"
...
class FiveFoldPreflightReceiptTests(unittest.TestCase):
    def test_five_fold_synthetic_separate_process_evidence_closes_no_training_receipt(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_json(freeze, {
                "status": "FROZEN_V1_3_2_IMPLEMENTATION_PENDING_INDEPENDENT_REVIEW_AND_NODE1_PREFLIGHT",
                "training_started": False,
                "implementation_hashes": {"PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3_2.json": sha(prereg)},
            })
            ...
            args = argparse.Namespace(
                ...
                preregistration=prereg, legacy_test_log=legacy, v1_3_2_test_log=new,
                expected_new_tests=17, output_receipt=runtime / "NODE1_V1_3_2_PREFLIGHT_RECEIPT.json",
            )
...
class FinalizationLifecycleTests(unittest.TestCase):
    def fixture(self, root: Path):
        ...
        shutil.copytree(ROOT, package, symlinks=True)
        ...
        freeze = package / "IMPLEMENTATION_FREEZE_PHASE1_TECHNICAL_RECOVERY_V1_3_2.json"
    ...
    def test_success_writes_only_launcher_and_false_started_authorization(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(sorted(p.name for p in args.output_dir.iterdir()), ["FINAL_TRAINING_AUTHORIZATION_V1_3_2.json", "run_phase1_core_fold_pair_node1_v1_3_2.sh"])
...
class SourceClosureTests(unittest.TestCase):
    def test_package_allowlist_rejects_extra_file(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            freeze = root / "IMPLEMENTATION_FREEZE_PHASE1_TECHNICAL_RECOVERY_V1_3_2.json"
    ...
    def test_preflight_launcher_has_no_arm_training_runner_and_binds_helper_argv(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_2.sh").read_text()
...
class Python311LegacyAdapterTests(unittest.TestCase):
    ...
    def test_combined_launcher_uses_adapter_with_explicit_legacy_root(self):
        source = (ROOT / "launchers/run_combined_legacy_and_v1_3_2_tests.sh").read_text()
        self.assertIn('run_legacy_102_tests_python311_v1_3_2.sh" "$LEGACY_ROOT"', source)
    ...
    def test_preflight_binds_adapter_and_unchanged_v131_core(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_2.sh").read_text()
        self.assertIn("run_legacy_102_tests_python311_v1_3_2.sh", source)
    ...
    def test_new_test_launcher_uses_relative_paths_not_root_absolute_argv(self):
        source = (ROOT / "launchers/run_tests_v1_3_2.sh").read_text()
