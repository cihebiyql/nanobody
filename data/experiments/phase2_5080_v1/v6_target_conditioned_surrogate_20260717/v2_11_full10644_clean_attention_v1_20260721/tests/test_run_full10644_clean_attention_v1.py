...
SOURCE = ROOT / "src/run_full10644_clean_attention_v1.py"
EVALUATOR_SOURCE = ROOT / "src/evaluate_multiseed_clean_attention_v1.py"
V6_ROOT = ROOT.parent
...
EVALUATOR = import_module("v211_full10644_clean_attention_evaluator", EVALUATOR_SOURCE)
...
class Fixture:
    ...
    def write_all(self) -> None:
        ...
        self.contract.write_text(json.dumps({
            ...
            "status": "FROZEN_PRE_LAUNCH",
            "lane": MOD.LANE,
            "contact_supervision_enabled": False,
            "expected_counts": {"total": 8, "train": 6, "development": 2},
...
class Full10644CleanAttentionTests(unittest.TestCase):
    def test_tiny_e2e_is_pure_b_and_exact_min(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["status"], "PASS_FULL10644_CLEAN_ATTENTION_FIXED_EPOCH_TRAINING")
            self.assertEqual(result["split"]["train_rows"], 6)
            self.assertEqual(result["split"]["development_rows"], 2)
    ...
    def test_multiseed_evaluator_aggregates_without_seed_selection(self):
        with tempfile.TemporaryDirectory() as temporary:
            fixture = Fixture(Path(temporary))
            seeds = (43, 917, 1931, 3253)
            contract = json.loads(fixture.contract.read_text())
            contract["four_gpu_tasks"] = [
                {"fold_id": "D1", "gpu": gpu, "seed": seed}
                for gpu, seed in enumerate(seeds)
            ]
            fixture.contract.write_text(json.dumps(contract))
            run_root = fixture.root / "four_seed_run"
            fixture.output = run_root / "D1_seed43"
            MOD.train(fixture.args())
            for seed in seeds[1:]:
                destination = run_root / f"D1_seed{seed}"
                shutil.copytree(fixture.output, destination)
                result_path = destination / MOD.RESULT_NAME
                result = json.loads(result_path.read_text())
                result["seed"] = seed
                result_path.write_text(json.dumps(result))
            output = run_root / "EARLY_ENRICHMENT"
            result = EVALUATOR.aggregate(fixture.contract, run_root, output)
            self.assertEqual(result["status"], "PASS_FULL10644_CLEAN_ATTENTION_MULTISEED_EARLY_ENRICHMENT")
            self.assertEqual(result["selection"], "NONE_REPORT_ALL_FROZEN_SEEDS_AND_MEAN_R8_R9_ENSEMBLE")
            self.assertEqual(result["counts"]["seed_runs"], 4)
            self.assertEqual(result["seed_stability"]["candidate_R_dual_min_prediction_std"]["max"], 0.0)
            self.assertTrue((output / EVALUATOR.ENSEMBLE_NAME).is_file())
            self.assertTrue((output / EVALUATOR.SHA256SUMS_NAME).is_file())
