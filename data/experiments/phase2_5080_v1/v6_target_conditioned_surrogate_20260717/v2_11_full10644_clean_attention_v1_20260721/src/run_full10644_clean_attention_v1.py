#!/usr/bin/env python3
"""Train the full10644 B_CLEAN_TARGET_ATTENTION challenger.
...
SCHEMA_VERSION = "pvrig_v2_11_full10644_clean_attention_runner_v1"
CONTRACT_SCHEMA = "pvrig_v2_11_full10644_clean_attention_contract_v1"
...
CHECKPOINT_NAME = "clean_attention_head_final.pt"
...
CLAIM_BOUNDARY = (
    "Open-development sequence plus label-free VHH monomer graph plus fixed public "
    "8X6B/9E6Y target-graph approximation of independent dual-receptor computational "
    "Docking geometry; not binding, affinity, experimental blocking, Docking Gold, "
    "frozen-test, or submission evidence."
)
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    require(contract.get("status") == "FROZEN_PRE_LAUNCH", "contract_status_invalid")
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.tiny_e2e:
        ...
        allowed_seeds = {int(task["seed"]) for task in contract["four_gpu_tasks"]}
        require(args.seed in allowed_seeds, f"seed_not_frozen:{args.seed}")
    ...
    split = load_split(split_path, rows, int(expected["train"]), int(expected["development"]))
    ...
    _atomic_json(args.output_dir / "RUNNING.json", {
        ...
        "status": "RUNNING_FIXED_EPOCH_CLEAN_ATTENTION",
        "seed": args.seed,
    ...
    receipt = {
        ...
        "split": {
            ...
            "development_rows": len(split.development_indices),
