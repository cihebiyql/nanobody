...
V1.1 is a technical post-processing recovery for IEEE-754 float32 target
serialization.  It does not change predictions, seeds, model weights, targets,
ranking logic, or metrics.
"""
...
SCHEMA_VERSION = "pvrig_v2_11_full10644_clean_attention_multiseed_evaluation_v1_1"
RESULT_NAME = "EARLY_ENRICHMENT.json"
ENSEMBLE_NAME = "ensemble_development_predictions.tsv"
SHA256SUMS_NAME = "SHA256SUMS"
FLOAT32_TRUTH_ATOL = 4e-8
...
def load_seed_prediction(
    ...
    max_receptor_truth_abs_diff = 0.0
    max_target_exact_min_abs_diff = 0.0
    max_prediction_exact_min_abs_diff = 0.0
    with prediction_path.open(newline="", encoding="utf-8") as handle:
        ...
        for raw in reader:
            ...
            receptor_truth_abs_diff = max(abs(target[index] - row.targets[index]) for index in range(2))
            target_exact_min_abs_diff = abs(float(raw["target_R_dual_min"]) - min(target))
            prediction_exact_min_abs_diff = abs(float(raw["prediction_R_dual_min"]) - min(prediction))
            max_receptor_truth_abs_diff = max(max_receptor_truth_abs_diff, receptor_truth_abs_diff)
            max_target_exact_min_abs_diff = max(max_target_exact_min_abs_diff, target_exact_min_abs_diff)
            max_prediction_exact_min_abs_diff = max(max_prediction_exact_min_abs_diff, prediction_exact_min_abs_diff)
            require(receptor_truth_abs_diff <= FLOAT32_TRUTH_ATOL, f"seed_truth_mismatch:{seed}:{candidate}")
            require(target_exact_min_abs_diff <= FLOAT32_TRUTH_ATOL, f"seed_truth_exact_min:{seed}:{candidate}")
            require(prediction_exact_min_abs_diff <= FLOAT32_TRUTH_ATOL, f"seed_prediction_exact_min:{seed}:{candidate}")
    ...
    return (
        np.asarray([loaded[candidate] for candidate in ordered_ids], dtype=np.float64),
        {
            RUNNER.RESULT_NAME: RUNNER.sha256_file(result_path),
            RUNNER.PREDICTION_NAME: RUNNER.sha256_file(prediction_path),
        },
        {
            "float32_truth_atol": FLOAT32_TRUTH_ATOL,
            "max_receptor_truth_abs_diff": max_receptor_truth_abs_diff,
            "max_target_exact_min_abs_diff": max_target_exact_min_abs_diff,
            "max_prediction_exact_min_abs_diff": max_prediction_exact_min_abs_diff,
        },
    )
...
def aggregate(contract_path: Path, run_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    seed_numeric_closure: dict[str, dict[str, float]] = {}
    for task in tasks:
        seed = int(task["seed"])
        require(task.get("fold_id") == "D1", f"fold_contract_invalid:{seed}")
        prediction, hashes, numeric_closure = load_seed_prediction(run_root / f"D1_seed{seed}", seed, truth)
        seed_predictions[str(seed)] = prediction
        input_hashes[str(seed)] = hashes
        seed_numeric_closure[str(seed)] = numeric_closure
    ...
    try:
        ...
        result = {
            ...
            "numeric_closure": {
                "policy": "IEEE754_FLOAT32_SERIALIZATION_ONLY_NO_TARGET_OR_PREDICTION_REWRITE",
                "seeds": seed_numeric_closure,
            },
            "inputs": {
