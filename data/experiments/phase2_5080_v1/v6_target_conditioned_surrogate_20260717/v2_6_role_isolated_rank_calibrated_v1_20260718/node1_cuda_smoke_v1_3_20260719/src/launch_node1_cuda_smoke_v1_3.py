...
AUTH_SHA='c7402b7857ceda27c91bd2caa9b663f0819477b2482a293938c734c442b3727b'
DRIVER_FREEZE_SHA='3654a7844fc209f280b2d15067aac1aeb914e1630082b7886a67934f61e3e716'
...
DRIVER_SHA='7f1ca77c76e4ba00082aedc9ff1afebf06ee693da00db595b651ef44dacfcb7d'
...
def main():
 ...
 files={root/'AUTHORIZATION_V1_3.json':AUTH_SHA,root/'DRIVER_FREEZE_V1_3.json':DRIVER_FREEZE_SHA,root/'vendor/integration/IMPLEMENTATION_FREEZE_V1_3.json':INTEGRATION_FREEZE_SHA,root/'vendor/integration/real1507_role_isolated_trainer_v1_3.py':TRAINER_SHA,root/'src/run_node1_cuda_smoke_v1_3.py':DRIVER_SHA,root/'vendor/trust_anchors/TRUST_ANCHOR_SET_RECEIPT.json':TRUST_SHA}
 for p,h in files.items():req(p.is_file() and not p.is_symlink(),f'missing:{p}');req(sha(p)==h,f'hash:{p.name}')
 auth=read(root/'AUTHORIZATION_V1_3.json');req(auth['execution_authorized'] is True and auth['status']=='EXPLICITLY_AUTHORIZED_NODE1_CUDA_SMOKE','authorization');req(auth['v4_f_test32_access_count']==0,'authorization_v4f')
 ...
 cmd=[str(PYTHON),str(root/'src/run_node1_cuda_smoke_v1_3.py'),'--package-root',str(root),'--runtime-root',str(x.runtime_root),'--result',str(x.runtime_root/'SMOKE_RESULT.json')]
