#!/usr/bin/env python3
"""Nonlaunching V2.6 real1507 trainer integration V1.1.
...
SCHEMA_VERSION = "pvrig_v2_6_real1507_role_isolated_trainer_v1_1"
CLAIM_BOUNDARY = (
...
@dataclass(frozen=True)
class CalibrationFitReceiptV11:
    calibration: Any
    outer_fold: int
    ensemble_id: str
    seeds: tuple[int, ...]
    ...
    def to_payload(self) -> dict[str, Any]:
        ...
        return {
            "schema_version": "pvrig_v2_6_inner_oof_calibration_receipt_v1_1",
...
def fit_fold_local_calibration_from_inner_oof(
    ...
    *,
    ...
    return CalibrationFitReceiptV11(
        calibration=result,
        outer_fold=outer_fold,
...
class V26TrainerConfig:
    ...
    rank_trust_anchor_path: str = ""
    expected_rank_trust_anchor_sha256: str = ""
    ...
    def validate(self) -> None:
        ...
        require(bool(self.rank_trust_anchor_path), "external_rank_trust_anchor_path_missing")
        reject_sealed_text(self.rank_trust_anchor_path, "rank_trust_anchor_path")
        require(valid_sha256(self.expected_rank_trust_anchor_sha256), "external_rank_trust_anchor_sha256_invalid")
...
def _optimizer_state_sha256(optimizers: Sequence[Any]) -> str:
    digest = hashlib.sha256()
    for optimizer_index, optimizer in enumerate(optimizers):
        digest.update(f"optimizer:{optimizer_index}\0".encode("ascii"))
        for parameter_index, (parameter, state) in enumerate(
            sorted(optimizer.state.items(), key=lambda item: id(item[0]))
        ):
            digest.update(f"parameter:{parameter_index}:{tuple(parameter.shape)}\0".encode("ascii"))
...
def load_external_rank_trust_anchor(
    *,
    ...
    """Validate split/label hashes supplied by an immutable external file.

    The trainer deliberately provides no materializer for this file.  This
    prevents the runtime from computing labels and then accepting its own
    hashes as a trust decision.
    """
    path = Path(config.rank_trust_anchor_path)
    require(path.is_file() and not path.is_symlink(), "external_rank_trust_anchor_not_regular")
    observed_file_sha = sha256_file(path)
    require(observed_file_sha == config.expected_rank_trust_anchor_sha256, "external_rank_trust_anchor_file_sha256_mismatch")
    try:
    ...
    require(valid_sha256(payload.get("source_teacher_sha256")), "external_rank_trust_anchor_teacher_sha_invalid")
    require(valid_sha256(payload.get("source_inner_manifest_sha256")), "external_rank_trust_anchor_manifest_sha_invalid")
    ...
    *,
    ...
    for epoch in range(config.fixed_epochs):
        ...
        for step_in_epoch, raw_window in enumerate(_iter_windows(source, config.gradient_accumulation)):
            ...
            optimizer_sha_before = _optimizer_state_sha256(optimizers)
            ...
            optimizer_sha_after = _optimizer_state_sha256(optimizers)
            event = {
                "epoch": epoch,
                "step_in_epoch": step_in_epoch,
...
def integration_contract() -> dict[str, Any]:
    return {
        ...
        "rank_cache_trust": "EXTERNAL_FROZEN_SPLIT_AND_LABEL_HASH_ANCHOR_REQUIRED",
