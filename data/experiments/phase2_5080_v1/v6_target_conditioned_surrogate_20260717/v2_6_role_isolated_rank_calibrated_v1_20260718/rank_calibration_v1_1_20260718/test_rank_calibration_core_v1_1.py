...
spec = importlib.util.spec_from_file_location("rank_calibration_core_v1_1", ROOT / "rank_calibration_core_v1_1.py")
        ...
        for sibling_index in range(siblings):
            ...
            output.append(
                core.CandidateLabel(
                    ...
                    true_r8=value + 0.02,
                    true_r9=value,
                    teacher_source=core.RANK_ELIGIBLE_TEACHER_SOURCE,
                    development_reliability_tier=core.RANK_ELIGIBLE_RELIABILITY_TIER,
                    docking_evidence_tier=core.RANK_ELIGIBLE_DOCKING_TIER,
                    teacher_reliability=core.RANK_ELIGIBLE_TEACHER_RELIABILITY,
                    ranking_release=core.RANK_ELIGIBLE_RELEASE,
                )
            )
    return output
...
class BindingAndDualTests(unittest.TestCase):
    ...
    def test_softmin_auxiliary_and_exact_min_rank_gradient(self):
        ...
        rank_receptors = torch.tensor([[0.21, 0.25], [0.22, 0.22]], requires_grad=True)
        exact_batch = core.build_exact_min_dual_prediction_batch(("A", "B"), rank_receptors)
        self.assertTrue(torch.equal(exact_batch.values, torch.tensor([0.21, 0.22])))
        exact_batch.values.sum().backward()
        self.assertTrue(torch.all(torch.isfinite(rank_receptors.grad)))
        self.assertGreater(float(rank_receptors.grad[0, 0]), 0.0)
        self.assertEqual(float(rank_receptors.grad[0, 1]), 0.0)
        with self.assertRaisesRegex(core.RankCalibrationError, "softmin_rank_builder_removed"):
            core.build_softmin_dual_prediction_batch(("A", "B"), rank_receptors)
...
class ParentPairEpochCacheTests(unittest.TestCase):
    ...
    def test_v4h_is_scalar_only_and_provenance_mutations_fail_closed(self):
        data = labels()
        for index, (tier, evidence, reliability) in enumerate(sorted(core.V4H_ALLOWED_STRATA)):
            data.append(
                core.CandidateLabel(
                    candidate_id=f"V4H_{index}",
                    parent_cluster_id="V4H_PARENT",
                    true_r8=0.30 + 0.04 * index,
                    true_r9=0.28 + 0.04 * index,
                    teacher_source=core.V4H_SOURCE,
                    development_reliability_tier=tier,
                    docking_evidence_tier=evidence,
                    teacher_reliability=reliability,
                    ranking_release=core.V4H_RELEASE,
                )
            )
        cache = self.build(labels=data)
        self.assertEqual(cache.rank_eligible_candidate_count, 40)
        self.assertEqual(cache.rank_ineligible_candidate_count, 3)
        self.assertEqual(
            dict(cache.rank_ineligible_reason_counts),
            {"EXCLUDED_ADAPTIVE_V4H_SOURCE_SPECIFIC_NOISE_UNVALIDATED": 3},
        )
        self.assertNotIn("V4H_PARENT", dict(cache.eligible_pairs_per_parent))
        malformed_v4h = list(data)
        malformed_v4h[-1] = dataclasses.replace(malformed_v4h[-1], teacher_reliability="MULTI_SEED")
        with self.assertRaisesRegex(core.RankCalibrationError, "v4h_source_tier_provenance_invalid"):
            self.build(labels=malformed_v4h)
        malformed_v4d = labels()
        malformed_v4d[0] = dataclasses.replace(malformed_v4d[0], development_reliability_tier="B")
        with self.assertRaisesRegex(core.RankCalibrationError, "v4d_source_tier_provenance_invalid"):
            self.build(labels=malformed_v4d)
    ...
    def test_persisted_cache_has_audit_and_zero_sealed_access(self):
        ...
        self.assertEqual(payload["rank_eligibility_policy_id"], core.RANK_ELIGIBILITY_POLICY_ID)
...
class PairLogitTests(unittest.TestCase):
    ...
    def predictions(self, correct: bool):
        ...
        batch = core.build_exact_min_dual_prediction_batch([row.candidate_id for row in rows], values)
        return batch, values
    ...
    def test_missing_endpoint_and_short_batch_fail_closed(self):
        ...
        predictions = core.ExactMinDualPredictionBatch(
        ...
        with self.assertRaisesRegex(core.RankCalibrationError, "typed_exact_min"):
            core.noise_aware_pairlogit({}, records)

    def test_softmin_can_reverse_deployment_order_but_v1_1_uses_exact_min(self):
        receptors = torch.tensor([[0.21, 0.25], [0.22, 0.22]], dtype=torch.float32)
        exact = core.build_exact_min_dual_prediction_batch(("A", "B"), receptors)
        smooth = core.softmin_dual(receptors)
        self.assertLess(float(exact.values[0]), float(exact.values[1]))
        self.assertGreater(float(smooth[0]), float(smooth[1]))
        self.assertEqual(exact.provenance, "FP32_EXACT_MIN_FROM_DIRECT_R8_R9")
...
class ContractTests(unittest.TestCase):
    def test_contract_declares_no_independent_dual_or_outer_fit(self):
        ...
        self.assertTrue(contract["forbidden"]["softmin_in_rank_loss"])
        self.assertEqual(contract["dual"]["ranking"], "FP32_exact_min_only")
        self.assertEqual(contract["rank_eligibility"]["policy_id"], core.RANK_ELIGIBILITY_POLICY_ID)
