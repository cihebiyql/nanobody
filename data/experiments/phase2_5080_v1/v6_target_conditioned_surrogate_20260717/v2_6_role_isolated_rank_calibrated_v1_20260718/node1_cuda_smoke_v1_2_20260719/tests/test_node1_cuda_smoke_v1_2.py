...
HERE=pathlib.Path(__file__).resolve(); ROOT=HERE.parents[1]; DRIVER=ROOT/'src/run_node1_cuda_smoke_v1_2.py'
...
class TestDriver(unittest.TestCase):
 def test_contract_and_no_sealed_inputs(self):
  ...
  self.assertEqual(m.INTEGRATION_TRAINER_SHA,sha(ROOT/'vendor/integration/real1507_role_isolated_trainer_v1_2.py'))
 ...
 def test_driver_has_real_lane_and_step_evidence(self):
  ...
  self.assertIn("'B_CLEAN_TARGET_ATTENTION'",text)
