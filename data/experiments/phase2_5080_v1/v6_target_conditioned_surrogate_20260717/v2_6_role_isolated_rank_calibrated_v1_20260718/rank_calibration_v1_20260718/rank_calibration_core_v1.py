#!/usr/bin/env python3
"""Frozen V2.6 rank/calibration primitives.
...
SCHEMA_VERSION = "pvrig_v2_6_rank_calibration_core_v1"
CLAIM_BOUNDARY = (
...
class ParentPairEpochCache:
    ...
    def content_payload(self) -> dict[str, Any]:
        return {
            "schema_version": "pvrig_v2_6_parent_pair_epoch_cache_content_v1",
    ...
    def to_payload(self) -> dict[str, Any]:
        ...
        payload.update(
            {
                "schema_version": "pvrig_v2_6_parent_pair_epoch_cache_v1",
                "status": "PASS_FROZEN_DELTA_NOISE_SPLIT_SAFE_EXACT_STEP_CACHE",
...
def load_parent_pair_epoch_cache(
    ...
    *,
    ...
    require(payload.get("schema_version") == "pvrig_v2_6_parent_pair_epoch_cache_v1", "pair_cache_schema_invalid")
    require(
        payload.get("status") == "PASS_FROZEN_DELTA_NOISE_SPLIT_SAFE_EXACT_STEP_CACHE",
...
@dataclass(frozen=True)
class SoftminDualPredictionBatch:
    """Typed provenance barrier preventing exact-min use in rank loss."""

    candidate_ids: tuple[str, ...]
    values: Tensor
    tau: float
    provenance: str = "FP32_NORMALIZED_SOFTMIN_FROM_DIRECT_R8_R9"

    def validate(self) -> None:
        require(bool(self.candidate_ids), "softmin_prediction_ids_empty")
        require(len(set(self.candidate_ids)) == len(self.candidate_ids), "softmin_prediction_ids_duplicate")
        require(self.values.shape == (len(self.candidate_ids),), "softmin_prediction_shape_invalid")
        require(self.values.dtype == torch.float32, "softmin_prediction_not_fp32")
        require_finite_tensor(self.values, "softmin_prediction_nonfinite")
        require(self.tau == SOFTMIN_TAU, "softmin_prediction_tau_not_frozen")
        require(
            self.provenance == "FP32_NORMALIZED_SOFTMIN_FROM_DIRECT_R8_R9",
            "softmin_prediction_provenance_invalid",
        )
...
def build_softmin_dual_prediction_batch(
    candidate_ids: Sequence[str], receptor_predictions: Tensor
) -> SoftminDualPredictionBatch:
    require(len(candidate_ids) == len(receptor_predictions), "softmin_prediction_candidate_count_mismatch")
    result = SoftminDualPredictionBatch(
        candidate_ids=tuple(candidate_ids),
        values=softmin_dual(receptor_predictions, SOFTMIN_TAU),
        tau=SOFTMIN_TAU,
    )
    result.validate()
    return result
...
def noise_aware_pairlogit(
    predicted_soft_dual: SoftminDualPredictionBatch,
    ...
    *,
    ...
    ``predicted_soft_dual`` is a typed product of direct R8/R9 softmin.
    Exact-min mappings are structurally rejected by the API.
    """
    require(len(records) == RANK_PAIRS_PER_STEP, "pair_batch_not_exactly_eight")
    require(math.isfinite(float(tau_rank)) and float(tau_rank) > 0.0, "rank_tau_invalid")
    require(isinstance(predicted_soft_dual, SoftminDualPredictionBatch), "pairlogit_requires_typed_softmin_batch")
    prediction_index = predicted_soft_dual.index()
...
def implementation_contract() -> dict[str, Any]:
    return {
        ...
        "dual": {
            "training": "FP32_normalized_softmin_tau_0.02",
            "ranking": "FP32_normalized_softmin_tau_0.02_only",
        ...
        "pairlogit": {
            ...
            "prediction_input": "typed_FP32_normalized_softmin_batch_from_direct_R8_R9",
        },
        "calibration": {
            "fit_role": CALIBRATION_FIT_ROLE,
