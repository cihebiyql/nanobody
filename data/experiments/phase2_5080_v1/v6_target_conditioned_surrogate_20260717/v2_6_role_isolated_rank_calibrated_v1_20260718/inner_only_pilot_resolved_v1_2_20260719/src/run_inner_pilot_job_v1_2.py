#!/usr/bin/env python3
"""Run one frozen outer0/inner0 V2.6 role-isolated 8-epoch pilot job."""
...
SCHEMA = "pvrig_v2_6_inner_only_pilot_job_v1_2"
CLAIM = (
...
def modules(package_root: Path):
    dependencies = (
        ...
        (package_root / "vendor/integration/IMPLEMENTATION_FREEZE_V1_3.json", INTEGRATION_FREEZE_SHA),
        (package_root / "vendor/integration/real1507_role_isolated_trainer_v1_3.py", INTEGRATION_TRAINER_SHA),
        (package_root / "vendor/optimizer/role_isolated_optimization_v1.py", OPT_SHA),
        (package_root / "vendor/rank/rank_calibration_core_v1_1.py", RANK_SHA),
        (package_root / "vendor/trust_anchors/TRUST_ANCHOR_SET_RECEIPT.json", TRUST_SHA),
        (package_root / "smoke_evidence/SMOKE_RESULT_V1_3_1.json", SMOKE_SHA),
    )
    ...
    sys.path[:0] = [
        str(V25_SRC),
        str(package_root / "vendor/integration"),
        str(package_root / "vendor/optimizer"),
        str(package_root / "vendor/rank"),
    ]
    ...
    policy = integ.RankPolicyAdapter.load(package_root / "vendor/rank/rank_calibration_core_v1_1.py", RANK_SHA)
...
def main() -> int:
    ...
    parser.add_argument("--lambda-rank", type=float, choices=(0.0, 0.1), required=True)
    parser.add_argument("--physical-gpu", type=int, choices=(1, 4, 5, 6), required=True)
    ...
    allowed = {
        "F0_SHARED_GATED_NO_RANK": (integ.LANE_F, 0.0, "E_DECOUPLED_CONTACT_SHARED"),
        "F1_SHARED_GATED_V4D_EXACT_MIN_RANK": (integ.LANE_F, 0.1, "E_DECOUPLED_CONTACT_SHARED"),
        "B_SCALAR_ATTENTION_ONLY": (integ.LANE_B, 0.0, "E_DECOUPLED_CONTACT_DETACHED"),
        "E_STRICT_DETACHED_DYNAMICS_CONTROL": (integ.LANE_E, 0.0, "E_DECOUPLED_CONTACT_DETACHED"),
    }
    require(args.variant in allowed, "variant")
    lane, rank_lambda, source_variant = allowed[args.variant]
    ...
    config = integ.V26TrainerConfig(
        ...
        rank_trust_anchor_set_receipt_path=str(args.package_root / "vendor/trust_anchors/TRUST_ANCHOR_SET_RECEIPT.json"),
        rank_trust_anchor_dir=str(args.package_root / "vendor/trust_anchors"),
    ...
    receipt = integ.train_v25_real1507_context_nonlaunching(
        ...
        optimizer_api=opt,
        rank_policy=policy,
        delta_noise_binding_path=args.package_root / "vendor/V2_6_DELTA_NOISE_BINDING.json",
    ...
    expected_steps = 8 * math.ceil(math.ceil(1085 / 8) / 2)
    ...
    result = {
        ...
        "status": "PASS_INNER_ONLY_EIGHT_EPOCH_PILOT_JOB",
