#!/usr/bin/env python3
"""Collect five whole-parent fold predictions into one exact-closure OOF table."""
...
RUNNER_PATH = HERE / "run_top5_clean_attention_fold_v1.py"
SPEC = importlib.util.spec_from_file_location("v213_top5_clean_attention_oof_runner_for_collection", RUNNER_PATH)
...
SCHEMA = "pvrig_v2_13_top5_clean_attention_seed_oof_collection_v1"
...
def collect(
    ...
    require(seed in {917, 1931}, "seed_invalid")
    output_name = f"TOP5_{variant}_SEED{seed}_TRAIN9849_OOF_PREDICTIONS.tsv"
    require(not output_dir.exists(), "output_exists")
    rows = RUNNER.load_rows(teacher_path, 9849)
    ...
    for fold_id in range(5):
        contract_path = contracts_dir / f"seed_{seed}_fold_{fold_id}_contract.json"
        contract = RUNNER.load_contract(contract_path)
        require(int(contract["task"]["fold_id"]) == fold_id and int(contract["task"]["seed"]) == seed, f"fold_contract:{fold_id}")
        require((contract.get("phase_b_provenance") or {}).get("selected_variant") == variant, f"fold_variant_provenance:{fold_id}")
        split_path = RUNNER._verify_bound_file(contract["split_manifest"], f"fold_split_{fold_id}")
        expected = contract["expected_counts"]
        split = RUNNER.load_split(split_path, rows, int(expected["train"]), int(expected["score"]))
        ...
        require(result.get("status") == "PASS_V2_13_TOP5_CLEAN_ATTENTION_FOLD_TRAINING", f"fold_status:{fold_id}")
        require(int(result.get("fold_id", -1)) == fold_id and int(result.get("seed", -1)) == seed, f"fold_identity:{fold_id}")
        ...
        require((result.get("outputs") or {}).get(RUNNER.PREDICTION_NAME) == RUNNER.sha256_file(prediction_path), f"fold_prediction_hash:{fold_id}")
        ...
        for raw in prediction_rows:
            ...
            require(int(raw["fold_id"]) == fold_id and int(raw["seed"]) == seed, f"prediction_fold_seed:{candidate}")
        ...
        input_hashes[str(fold_id)] = {
            "contract": RUNNER.sha256_file(contract_path), "result": RUNNER.sha256_file(result_path),
            "predictions": RUNNER.sha256_file(prediction_path),
        }
    ...
    metrics = RUNNER.comprehensive_metrics(candidate_ids, parents, targets, predictions)
    ...
    try:
        ...
        with output_path.open("w", newline="", encoding="utf-8") as handle:
            ...
            for row in rows:
                ...
                writer.writerow({
                    ...
                    "parent_framework_cluster": row.parent, "fold_id": assignment[row.candidate_id]["fold_id"], "seed": str(seed), "variant": variant,
        ...
        receipt = {
            ...
            "status": "PASS_V2_13_TOP5_TRAIN9849_WHOLE_PARENT_OOF",
            "variant": variant,
            "claim_boundary": RUNNER.CLAIM_BOUNDARY,
            "counts": {"rows": 9849, "parents": 54, "folds": 5, "seed": seed},
            ...
            "inputs": {
                "teacher_sha256": RUNNER.sha256_file(teacher_path),
                "assignment_sha256": RUNNER.sha256_file(assignment_path),
                "folds": input_hashes,
            },
            "input_access": {"open_development_rows": 0, "frozen_test_rows": 0},
            "outputs": {output_name: RUNNER.sha256_file(output_path), METRICS_NAME: RUNNER.sha256_file(metrics_path)},
        }
        receipt_path = staging / RECEIPT_NAME
        receipt_path.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n")
        hashes = {name: RUNNER.sha256_file(staging / name) for name in (output_name, METRICS_NAME, RECEIPT_NAME)}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--variant", choices=("L1", "L2", "L3"), required=True)
    parser.add_argument("--seed", type=int, choices=(917, 1931), required=True)
    args = parser.parse_args(argv)
    result = collect(args.teacher, args.assignment, args.contracts_dir, args.run_root, args.output_dir, args.variant, args.seed)
