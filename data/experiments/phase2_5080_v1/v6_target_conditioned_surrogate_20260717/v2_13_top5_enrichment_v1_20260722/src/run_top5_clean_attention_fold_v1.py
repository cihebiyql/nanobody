#!/usr/bin/env python3
"""Train one leakage-safe Top5-directed whole-parent clean-attention fold.
...
SCHEMA_VERSION = "pvrig_v2_13_top5_clean_attention_fold_runner_v1"
CONTRACT_SCHEMA = "pvrig_v2_12_clean_attention_inner_oof_fold_contract_v1"
TOP5_CONTRACT_SCHEMA = "pvrig_v2_13_top5_enrichment_contract_v1"
...
class CleanCollator:
    def __init__(
        ...
        tokenizer: Any,
        graphs: GraphCacheStore,
        weights: Mapping[int, float],
        truth_percentiles: Mapping[int, float],
    ) -> None:
        self.rows, self.tokenizer, self.graphs = rows, tokenizer, graphs
        self.weights, self.truth_percentiles = dict(weights), dict(truth_percentiles)
    ...
    def __call__(self, indices: Sequence[int]) -> dict[str, Tensor]:
        ...
        batch: dict[str, Tensor] = {
            ...
            # Loss-only metadata.  The frozen positive neural-forward allowlist
            # never forwards this tensor into the model.
            "truth_percentile": torch.tensor([self.truth_percentiles.get(index, 0.0) for index in indices], dtype=torch.float32),
        }
...
def iter_top_balanced_batches(
    ...
    *,
    ...
    top_per_batch: int,
    top_threshold: float,
) -> Iterable[tuple[list[int], dict[str, Tensor]]]:
    """Yield deterministic full batches containing both top and non-top rows.

    The partition is computed only from the current fold's fit rows.  Rows may
    be repeated within an epoch to keep every batch rank-informative; IDs and
    parent labels remain outside the neural forward.
    """
    require(0 < top_per_batch < batch_size, "balanced_top_per_batch_invalid")
    rng = random.Random(shuffle_seed)
    top = [index for index in indices if truth_percentiles[index] >= top_threshold]
    other = [index for index in indices if truth_percentiles[index] < top_threshold]
    require(top and other, "balanced_partition_empty")
    rng.shuffle(top)
    rng.shuffle(other)
    other_per_batch = batch_size - top_per_batch
    batches = max(math.ceil(len(top) / top_per_batch), math.ceil(len(other) / other_per_batch))
    for batch_index in range(batches):
        selected = [top[(batch_index * top_per_batch + offset) % len(top)] for offset in range(top_per_batch)]
        selected += [other[(batch_index * other_per_batch + offset) % len(other)] for offset in range(other_per_batch)]
        rng.shuffle(selected)
        yield selected, collator(selected)


def training_truth_percentiles(rows: Sequence[CandidateRow], indices: Sequence[int]) -> dict[int, float]:
    values = np.asarray([min(rows[index].targets) for index in indices], dtype=np.float64)
    ranks = _rankdata(values)
    denominator = max(1, len(indices) - 1)
    return {index: float((rank - 1.0) / denominator) for index, rank in zip(indices, ranks)}


def top_weight(percentile: float, strength: float, center: float, scale: float) -> float:
    require(strength >= 0.0 and scale > 0.0, "top_weight_config_invalid")
    return 1.0 + strength / (1.0 + math.exp(-(percentile - center) / scale))
...
def pairwise_rank_loss(
    ...
    *,
    ...
    targets = batch["targets"].to(predictions.device, dtype=torch.float32)
    percentiles = batch["truth_percentile"].to(predictions.device, dtype=torch.float32)
    predicted_dual = (
        -float(softmin_tau) * torch.logsumexp(-predictions / float(softmin_tau), dim=1)
        + float(softmin_tau) * math.log(2.0)
    )
    truth_dual = torch.minimum(targets[:, 0], targets[:, 1])
    truth_difference = truth_dual[:, None] - truth_dual[None, :]
    prediction_difference = predicted_dual[:, None] - predicted_dual[None, :]
    mask = torch.triu(torch.ones_like(truth_difference, dtype=torch.bool), diagonal=1)
    mask &= torch.abs(truth_difference) >= float(margin)
    if not bool(mask.any()):
        return predicted_dual.sum() * 0.0
    signed_margin = torch.sign(truth_difference[mask]) * prediction_difference[mask]
    loss = F.softplus(-signed_margin / float(temperature))
    pair_percentile = torch.maximum(percentiles[:, None], percentiles[None, :])[mask]
    weights = 1.0 + float(top_strength) * pair_percentile
    result = (loss * weights).sum() / weights.sum().clamp_min(1e-12)
    require(bool(torch.isfinite(result)), "pairwise_rank_loss_nonfinite")
    return result
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    top5_contract = json.loads(args.top5_contract.read_text(encoding="utf-8"))
    require(top5_contract.get("schema_version") == TOP5_CONTRACT_SCHEMA, "top5_contract_schema_invalid")
    require(top5_contract.get("status") == "FROZEN_BEFORE_PHASE_A_LAUNCH", "top5_contract_status_invalid")
    variant_config = (top5_contract.get("phase_a_variants") or {}).get(args.variant)
    require(isinstance(variant_config, Mapping), f"top5_variant_invalid:{args.variant}")
    ...
    truth_percentiles = training_truth_percentiles(rows, split.train_indices)
    weights = {
        index: rows[index].sample_weight * top_weight(
            truth_percentiles[index],
            float(variant_config["top_weight_strength"]),
            float(variant_config["top_weight_center"]),
            float(variant_config["top_weight_scale"]),
        )
        for index in split.train_indices
    }
    collator = CleanCollator(rows, tokenizer, graph_store, weights, truth_percentiles)
    ...
    for epoch in range(args.epochs):
        ...
        batches = 0
        top_per_batch = int(variant_config.get("balanced_top_per_batch", 0))
        if top_per_batch:
            batches_for_epoch = iter_top_balanced_batches(
                split.train_indices,
                collator,
                args.batch_size,
                ...
                top_threshold=float(variant_config["balanced_top_threshold"]),
            )
        else:
            batches_for_epoch = iter_batches(
                split.train_indices,
                collator,
                args.batch_size,
                shuffle_seed=args.seed + epoch,
            )
        for _batch_indices, raw in batches_for_epoch:
            batch = move(raw, device)
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=args.precision == "bf16"):
                ...
                rank_loss = pairwise_rank_loss(
                    ...
                    batch,
                    softmin_tau=args.softmin_tau,
                    margin=float(variant_config["pair_margin"]),
                    temperature=float(variant_config["pair_temperature"]),
                    top_strength=float(variant_config["top_weight_strength"]),
                )
                total = total + float(variant_config["pair_rank_weight"]) * rank_loss
                parts = dict(parts)
                parts["pair_rank"] = rank_loss
                parts["total_with_pair_rank"] = total
    ...
    receipt = {
        "schema_version": SCHEMA_VERSION,
        "status": "PASS_V2_13_TOP5_CLEAN_ATTENTION_FOLD_TRAINING",
        "lane": LANE,
        ...
        "training": {
            ...
            "top5_loss": dict(variant_config),
            "optimizer_parameter_roles": optimizer_audit,
            "weighting": "manifest_sample_weight_times_fit_fold_top_weight_normalized_within_each_minibatch",
            "balanced_top_batches": bool(int(variant_config.get("balanced_top_per_batch", 0))),
        },
        "metrics": metrics,
        ...
        "input_bindings": {
            ...
            "top5_contract_sha256": sha256_file(args.top5_contract),
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--top5-contract", type=Path, required=True)
    value.add_argument("--variant", choices=("L1", "L2", "L3"), required=True)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    require(args.top5_contract.is_file() and not args.top5_contract.is_symlink(), "top5_contract_invalid")
