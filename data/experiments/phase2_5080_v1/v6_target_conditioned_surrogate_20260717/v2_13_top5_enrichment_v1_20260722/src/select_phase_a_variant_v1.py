#!/usr/bin/env python3
"""Apply the result-blind V2.13 Phase-A to Phase-B promotion contract."""
...
SCHEMA = "pvrig_v2_13_phase_a_selection_v1"
CONTRACT_SCHEMA = "pvrig_v2_13_phase_b_promotion_contract_v1"
VARIANTS = ("L1", "L2", "L3")
...
def select(contract_path: Path, baseline_path: Path, phase_a_root: Path, output_path: Path) -> dict[str, Any]:
    ...
    require(contract.get("status") == "FROZEN_RESULT_BLIND_DURING_PHASE_A_TRAINING", "contract_status")
    ...
    eligible = []
    complexity = {"L1": 1, "L2": 2, "L3": 3}
    for variant in VARIANTS:
        path = phase_a_root / variant / "OOF_AGGREGATE" / f"TOP5_{variant}_TRAIN9849_OOF_PREDICTIONS.tsv"
    ...
    result = {
        ...
        "status": "PASS_PHASE_A_VARIANT_PROMOTED" if selected else contract["failure"]["no_eligible_variant"],
