...
COLLECTOR = load_module("v213_top5_collector_test", PKG / "src/collect_top5_clean_attention_oof_v1.py")
    ...
    def test_collects_exact_variant_five_fold_closure(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for fold in range(5):
                ...
                for assigned in (row for row in assignments if int(row["fold_id"]) == fold):
                    ...
                    prediction_rows.append({
                        ...
                        "fold_id": str(fold), "seed": "43", "variant": "L2",
                ...
                result = {
                    ...
                    "fold_id": fold, "seed": 43, "variant": "L2",
            ...
            receipt = COLLECTOR.collect(
                ...
                contracts, runs, output, "L2",
            )
            self.assertEqual(receipt["status"], "PASS_V2_13_TOP5_TRAIN9849_WHOLE_PARENT_OOF")
            self.assertEqual(receipt["variant"], "L2")
            output_path = output / "TOP5_L2_TRAIN9849_OOF_PREDICTIONS.tsv"
            with output_path.open(newline="", encoding="utf-8") as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual(len(rows), 9849)
            self.assertEqual({row["variant"] for row in rows}, {"L2"})
