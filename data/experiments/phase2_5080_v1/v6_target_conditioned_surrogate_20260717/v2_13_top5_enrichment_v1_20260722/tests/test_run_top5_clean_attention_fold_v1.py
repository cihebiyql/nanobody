...
SOURCE = ROOT / "src/run_top5_clean_attention_fold_v1.py"
LOCAL_V6_ROOT = ROOT.parent
if (LOCAL_V6_ROOT / "v2_5_ortho_contact_pose_stack_v1_20260718").is_dir():
    MODEL_PATH = LOCAL_V6_ROOT / "v2_5_ortho_contact_pose_stack_v1_20260718/model/residue_model_v2_5_ortho.py"
    TRAINER_PATH = LOCAL_V6_ROOT / "v2_5_ortho_contact_pose_stack_v1_20260718/trainer/train_v2_5_ortho_heads.py"
    GRAPH_BUILDER_PATH = LOCAL_V6_ROOT / "residue_v2/src/build_residue_graph_cache_v2.py"
else:
    MODEL_PATH = Path("/data1/qlyu/projects/pvrig_v2_5_ortho_formal_nested_package_v1_3_20260718/node1_bundle/model/residue_model_v2_5_ortho.py")
    TRAINER_PATH = Path("/data1/qlyu/projects/pvrig_v2_5_ortho_formal_nested_package_v1_3_20260718/node1_bundle/trainer/train_v2_5_ortho_heads.py")
    GRAPH_BUILDER_PATH = Path("/data1/qlyu/projects/pvrig_v6_residue_v2_deployment_bundle_v1_20260718/residue_v2/src/build_residue_graph_cache_v2.py")
...
MOD = import_module("v213_top5_clean_attention", SOURCE)
...
class Fixture:
    def __init__(self, root: Path):
        ...
        self.top5_contract = root / "top5_contract.json"
    ...
    def write_all(self) -> None:
        ...
        self.top5_contract.write_text(json.dumps({
            "schema_version": MOD.TOP5_CONTRACT_SCHEMA,
            "status": "FROZEN_BEFORE_PHASE_A_LAUNCH",
            "phase_a_variants": {
                "L1": {
                    "top_weight_strength": 3.0, "top_weight_center": 0.85, "top_weight_scale": 0.05,
                    "balanced_top_per_batch": 0, "pair_rank_weight": 0.0,
                    "pair_margin": 0.005, "pair_temperature": 0.02,
                },
                "L2": {
                    "top_weight_strength": 3.0, "top_weight_center": 0.85, "top_weight_scale": 0.05,
                    "balanced_top_per_batch": 2, "balanced_top_threshold": 0.90,
                    "pair_rank_weight": 0.25, "pair_margin": 0.005, "pair_temperature": 0.02,
                },
                "L3": {
                    "top_weight_strength": 3.0, "top_weight_center": 0.85, "top_weight_scale": 0.05,
                    "balanced_top_per_batch": 2, "balanced_top_threshold": 0.90,
                    "pair_rank_weight": 0.50, "pair_margin": 0.005, "pair_temperature": 0.02,
                },
            },
        }), encoding="utf-8")
    ...
    def args(self) -> argparse.Namespace:
        return argparse.Namespace(
            ...
            top5_contract=self.top5_contract,
            variant="L2",
...
class Full10644CleanAttentionTests(unittest.TestCase):
    def test_tiny_e2e_is_pure_b_and_exact_min(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["status"], "PASS_V2_13_TOP5_CLEAN_ATTENTION_FOLD_TRAINING")
            self.assertEqual(result["variant"], "L2")
    ...
    def test_collator_batch_has_no_ids_m2_c2_or_contact(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            percentiles = MOD.training_truth_percentiles(rows, split.train_indices)
            weights = {index: rows[index].sample_weight for index in split.train_indices}
            batch = MOD.CleanCollator(rows, MOD.TinyTokenizer(), store, weights, percentiles)(split.train_indices[:2])
    ...
    def test_balanced_batches_contain_top_and_non_top(self):
        indices = tuple(range(20))
        percentiles = {index: index / 19 for index in indices}
        class Dummy:
            def __call__(self, selected):
                return {"selected": torch.tensor(selected)}
        batches = list(MOD.iter_top_balanced_batches(
            indices, Dummy(), 8, shuffle_seed=43, truth_percentiles=percentiles,
            top_per_batch=2, top_threshold=0.90,
        ))
        self.assertTrue(batches)
        for selected, _batch in batches:
            self.assertEqual(sum(percentiles[index] >= 0.90 for index in selected), 2)
            self.assertEqual(sum(percentiles[index] < 0.90 for index in selected), 6)

    def test_pairwise_rank_loss_prefers_correct_order(self):
        batch = {
            "targets": torch.tensor([[0.7, 0.72], [0.4, 0.45]], dtype=torch.float32),
            "truth_percentile": torch.tensor([1.0, 0.0], dtype=torch.float32),
        }
        correct = {"receptor_predictions": torch.tensor([[0.68, 0.70], [0.42, 0.44]], dtype=torch.float32)}
        reversed_ = {"receptor_predictions": torch.tensor([[0.42, 0.44], [0.68, 0.70]], dtype=torch.float32)}
        kwargs = dict(softmin_tau=0.02, margin=0.005, temperature=0.02, top_strength=3.0)
        self.assertLess(
            float(MOD.pairwise_rank_loss(correct, batch, **kwargs)),
            float(MOD.pairwise_rank_loss(reversed_, batch, **kwargs)),
        )
