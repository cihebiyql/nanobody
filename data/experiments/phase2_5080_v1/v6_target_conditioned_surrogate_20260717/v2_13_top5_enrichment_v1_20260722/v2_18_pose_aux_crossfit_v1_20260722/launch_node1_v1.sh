...
OUT=$ROOT/training/strict_oof_v1
STATUS=$ROOT/status
...
check 002107bb9ff56f8ef9be93b3811cbc5bc5e68716a00f4a8acc5f994f9e0af51f "$ROOT/src/run_v218_pose_aux_crossfit_oof_v1.py"

printf '{"status":"RUNNING_V2_18_POSE_AUX_STRICT_OOF","timestamp":"%s"}\n' "$(date --iso-8601=seconds)" > "$STATUS/LIVE_STATUS.json"
OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 OPENBLAS_NUM_THREADS=32 NUMEXPR_NUM_THREADS=32 \
"$PY" "$ROOT/src/run_v218_pose_aux_crossfit_oof_v1.py" \
  --raw "$RAW" \
  --strict-oof "$STRICT" \
  --l1-oof "$L1" \
  --pose-aux "$AUX" \
  --output-dir "$OUT" > "$ROOT/logs/strict_oof.log" 2>&1

cp "$OUT/RUN_RECEIPT.json" "$STATUS/TERMINAL.json"
