...
SOURCE = ROOT / "src" / "run_v218_pose_aux_crossfit_oof_v1_1.py"
...
class CrossfitTests(unittest.TestCase):
    def test_hgb_uses_supported_loss(self) -> None:
        x = np.arange(80, dtype=float).reshape(20, 4)
        y = np.column_stack((np.linspace(0.4, 0.8, 20), np.linspace(0.5, 0.7, 20)))
        models = MOD.fit_hgb(x, y, np.ones(20))
        self.assertEqual([model.loss for model in models], ["squared_error", "squared_error"])
