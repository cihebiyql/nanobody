#!/usr/bin/env python3
"""Materialize the hash-bound V2.4 adaptive-multiseed V2 ready manifest and implementation freeze.
...
PENDING_STATUS = "PREFREEZE_V2_ADAPTIVE_MULTI_SEED_CALIBRATION_PENDING_DO_NOT_START"
READY_STATUS = "PREFREEZE_V2_ADAPTIVE_MULTI_SEED_READY_DO_NOT_START"
...
def validate_prefreeze_manifest(path: pathlib.Path) -> dict[str, Any]:
    ...
    require(manifest.get("manifest_generation") == "V2_ADAPTIVE_MULTI_SEED", "prefreeze_manifest_generation")
    ...
    require(contract.get("binding_status") == "PENDING_V2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION", "calibration_binding_status")
...
def materialize(
    *,
    ...
    require(ready_manifest_path.name == "V2_4_NODE1_READY_MANIFEST_V2.json", "ready_manifest_filename")
    require(freeze_path.name == "IMPLEMENTATION_FREEZE_V2_4_ADAPTIVE_V2.json", "implementation_freeze_filename")
    ...
    require(
        prefreeze_manifest_path == bundle_root / "V2_4_NODE1_PREFREEZE_MANIFEST_V2.json",
        "prefreeze_manifest_noncanonical_path",
    )
    require(
        ready_manifest_path == bundle_root / "V2_4_NODE1_READY_MANIFEST_V2.json",
        "ready_manifest_noncanonical_path",
    )
    require(
        freeze_path == bundle_root / "IMPLEMENTATION_FREEZE_V2_4_ADAPTIVE_V2.json",
    ...
    ready["calibration_contract"].update({
        "binding_status": "FROZEN_V2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION",
    ...
    freeze = {
        "schema_version": "pvrig_v6_residue_v2_4_implementation_freeze_v2_adaptive_multiseed",
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_V2_4_ADAPTIVE_V2_POSTCALIBRATION_FREEZE:{error}", file=os.sys.stderr)
        raise SystemExit(1)
