#!/usr/bin/env python3
"""Wait for and collect V2.2.2 open-only four-lane outer OOF evidence (collector V1.1).
...
RESULT_SPLIT_PROVENANCE_ONLY_FIELDS = {"schema_version", "source_outer_manifest_sha256", "training_tsv_sha256"}
...
def validate_result(
    *, lane: str, fold: int, runtime_root: pathlib.Path, manifest: Mapping[str, Any],
    ...
    # The frozen trainer serializes the executable split projection into RESULT.json;
    # provenance-only fields remain authoritative in the content-addressed split file.
    # Require the exact documented projection key set and exact values, rather than
    # weakening any train/score-parent or open-only gate.
    result_split = result.get("split")
    require(isinstance(result_split, dict), f"result_split_not_object:{lane}:{fold}")
    expected_result_split_keys = set(split) - RESULT_SPLIT_PROVENANCE_ONLY_FIELDS
    require(set(result_split) == expected_result_split_keys, f"result_split_projection_keys:{lane}:{fold}")
    require(
        all(result_split[key] == split[key] for key in expected_result_split_keys),
        f"result_split_projection_values:{lane}:{fold}",
    )
    require(split.get("schema_version") == "pvrig_v2_4_open_base_split_manifest_v1", f"split_schema:{fold}")
    require(split.get("training_tsv_sha256") == manifest["artifacts"]["training_tsv"]["sha256"], f"split_training_sha:{fold}")
    require(isinstance(split.get("source_outer_manifest_sha256"), str) and len(split["source_outer_manifest_sha256"]) == 64, f"split_source_manifest_sha:{fold}")
...
def collect(
    *, manifest_path: pathlib.Path, freeze_path: pathlib.Path, runtime_root: pathlib.Path,
    ...
    collection_root = output_root / "collection_v1_1"
    ...
    try:
        ...
        metrics_payload = {
            "schema_version": "pvrig_v2_4_v2_2_2_four_lane_open_outer_oof_metrics_v1_1",
        ...
        validation_payload = {
            "schema_version": "pvrig_v2_4_v2_2_2_four_lane_outer_validation_v1_1",
        ...
        raise
    terminal = {
        "schema_version": "pvrig_v2_4_v2_2_2_post_outer_terminal_receipt_v1_1",
...
def watch(
    *, manifest_path: pathlib.Path, freeze_path: pathlib.Path, runtime_root: pathlib.Path,
    ...
    if terminal_path.is_file() and not terminal_path.is_symlink():
        ...
        require(terminal.get("schema_version") == "pvrig_v2_4_v2_2_2_post_outer_terminal_receipt_v1_1", "existing_terminal_schema")
    ...
    while not receipt_path.is_file():
        ...
        atomic_json(status_path, {
            "schema_version": "pvrig_v2_4_v2_2_2_post_outer_watcher_status_v1_1",
