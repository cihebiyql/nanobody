#!/usr/bin/env python3
"""Run the V2.2.2 status-constant-corrected multibatch calibration.

V2.2 fixes only the exact observation/manifest claim-boundary mismatch found
after the numerically valid V2.1 calibration.  It preserves the V2.1 batches,
grid, gates, model, and observed selected weights in a new immutable attempt.
"""
...
import node1_v2_4_outer_development_launcher_v2_2_2 as deployment
...
CALIBRATION_SCHEMA = (
    ...
    "v2_2_2_bundle_receipt_contract_corrected"
)
CALIBRATION_STATUS = (
    "PASS_OPEN_ONLY_PRESTEP_CONTACT_GRADIENT_CALIBRATION_"
    "V2_4_ADAPTIVE_V2_2_2_BUNDLE_RECEIPT_CONTRACT_CORRECTED"
)
SUPERSESSION_VERSION = "V2.2_CLAIM_BOUNDARY_ALIGNMENT_ONLY"
EXPECTED_SELECTED_WEIGHTS = {
...
def validate_observation(payload: Mapping[str, Any], manifest: Mapping[str, Any], lane: str) -> dict[str, float]:
    ...
    require(payload.get("technical_supersession_version") == SUPERSESSION_VERSION,
            "calibration_observation_supersession_version")
    require(payload.get("claim_boundary") == manifest["claim_boundary"],
            "calibration_observation_claim_boundary")
    ...
    selected = {"marginal": marginal, "pair": pair}
    require(selected == EXPECTED_SELECTED_WEIGHTS[lane], "calibration_v2_1_weight_equivalence")
    return selected
...
def aggregate_receipt(
    ...
    return {
        ...
        "v2_1_selected_weight_equivalence_required": EXPECTED_SELECTED_WEIGHTS,
...
def dry_run(manifest_path: pathlib.Path) -> dict[str, Any]:
    ...
    require(manifest["status"] == PENDING_STATUS, "calibration_requires_pending_manifest")
    ...
    return {
        "schema_version": "pvrig_v6_residue_v2_4_calibration_dry_run_v2_2_2_bundle_receipt_contract_corrected",
        "status": "PASS_V2_2_2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION_DRY_RUN_NO_MUTATION",
        "manifest_sha256": deployment.sha256_file(manifest_path),
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_V2_4_ADAPTIVE_V2_2_2_CALIBRATION:{error}", file=os.sys.stderr)
        raise SystemExit(1)
