#!/usr/bin/env python3
"""Fail-closed V2.4 V2.2.2 post-calibration tiny-smoke and outer launcher.
...
FREEZE_NAME = "IMPLEMENTATION_FREEZE_V2_4_ADAPTIVE_V2_2_2.json"
...
CALIBRATION_SCHEMA = "pvrig_v6_residue_v2_4_open_only_prestep_calibration_receipt_v2_2_2_bundle_receipt_contract_corrected"
CALIBRATION_STATUS = "PASS_OPEN_ONLY_PRESTEP_CONTACT_GRADIENT_CALIBRATION_V2_4_ADAPTIVE_V2_2_2_BUNDLE_RECEIPT_CONTRACT_CORRECTED"
...
def load_manifest(path: pathlib.Path, *, allow_pending_calibration: bool = False) -> dict[str, Any]:
    ...
    require(payload.get("schema_version") == MANIFEST_SCHEMA, "manifest_schema")
    require(payload.get("manifest_generation") == "V2_2_2_BUNDLE_RECEIPT_CONTRACT_CORRECTED", "manifest_generation")
    status = payload.get("status")
    ready = status == "PREFREEZE_V2_2_2_ADAPTIVE_MULTI_SEED_READY_DO_NOT_START"
    pending = status == "PREFREEZE_V2_2_2_ADAPTIVE_MULTI_SEED_CALIBRATION_PENDING_DO_NOT_START"
    ...
    require(payload.get("claim_boundary") == CLAIM_BOUNDARY, "manifest_claim_boundary")
    require(payload.get("trainer_result_claim_boundary") == TRAINER_RESULT_CLAIM_BOUNDARY,
            "manifest_trainer_result_claim_boundary")
    supersession = payload.get("technical_supersession") or {}
    require(supersession.get("version") == SUPERSESSION_VERSION, "manifest_supersession_version")
    require(supersession.get("bundle_revision") == BUNDLE_REVISION, "manifest_bundle_revision")
    require(supersession.get("v2_1_selected_contact_weights") == EXPECTED_CONTACT_WEIGHTS,
            "manifest_v2_1_weight_contract")
    ...
    adaptive = payload.get("adaptive_supervision") or {}
    ...
    require(adaptive.get("input_contract_artifact_label") == "adaptive_input_contract", "adaptive_contract_label")
    require(adaptive.get("source_receipt_artifact_label") == "v4h_adaptive_source_receipt", "adaptive_source_receipt_label")
    required_adaptive = {
    ...
    require(required_adaptive <= set(artifacts), "adaptive_artifact_closure")
    ...
    require("calibration_trainer" in artifacts, "calibration_trainer_artifact_missing")
    ...
    require(
        {"adaptive_marginal_tsv_gz", "adaptive_pair_tsv_gz"} <= placeholders,
        "adaptive_trainer_input_placeholders",
    )
    ...
    band = calibration.get("target_median_gradient_fraction_band")
    require(isinstance(band, list) and band == [0.05, 0.15], "calibration_median_band")
    require(calibration.get("maximum_per_batch_gradient_fraction") == 0.30, "calibration_maximum_fraction")
    batch_selection = calibration.get("batch_selection")
    require(isinstance(batch_selection, dict), "calibration_batch_selection")
    require(len(batch_selection.get("batch_records") or []) == 8, "calibration_batch_count")
    require(isinstance(batch_selection.get("contract_sha256"), str) and len(batch_selection["contract_sha256"]) == 64, "calibration_batch_contract_sha")
    ...
    if ready:
        ...
        require(calibration.get("binding_status") == "PENDING_V2_2_2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION", "pending_calibration_status")
...
def validate_calibration_receipt(manifest: Mapping[str, Any], *, use_source: bool) -> dict[str, Any]:
    ...
    require(receipt.get("schema_version") == CALIBRATION_SCHEMA, "calibration_schema")
    require(receipt.get("status") == CALIBRATION_STATUS, "calibration_status")
    require(receipt.get("trainer_sha256") == manifest["artifacts"]["trainer"]["sha256"], "calibration_trainer_base_sha")
    require(
        receipt.get("calibration_trainer_sha256")
        == manifest["artifacts"]["calibration_trainer"]["sha256"],
        "calibration_trainer_wrapper_sha",
    )
    require(
        receipt.get("calibration_runner_sha256")
        == manifest["artifacts"]["calibration_runner"]["sha256"],
        "calibration_runner_sha",
    )
    ...
    require(receipt.get("claim_boundary") == CLAIM_BOUNDARY, "calibration_claim_boundary")
    require(receipt.get("technical_supersession_version") == SUPERSESSION_VERSION,
            "calibration_supersession_version")
    require(receipt.get("bundle_revision") == BUNDLE_REVISION, "calibration_bundle_revision")
    require(receipt.get("trainer_result_claim_boundary") == TRAINER_RESULT_CLAIM_BOUNDARY,
            "calibration_trainer_result_claim_boundary")
    require(receipt.get("v2_1_selected_weight_equivalence_required") == EXPECTED_CONTACT_WEIGHTS,
            "calibration_v2_1_weight_contract")
    require(receipt.get("fixed_grid") == contract["fixed_grid"], "calibration_grid_mismatch")
    require(receipt.get("target_median_gradient_fraction_band") == contract["target_median_gradient_fraction_band"], "calibration_median_band_mismatch")
    require(receipt.get("maximum_per_batch_gradient_fraction") == contract["maximum_per_batch_gradient_fraction"], "calibration_maximum_fraction_mismatch")
    require(receipt.get("calibration_batch_count") == 8, "calibration_batch_count_mismatch")
    require(receipt.get("calibration_batch_contract_sha256") == contract["batch_selection"]["contract_sha256"], "calibration_batch_contract_mismatch")
    ...
    require(receipt.get("adaptive_input_contract_sha256") == adaptive["input_contract_sha256"], "calibration_adaptive_contract_sha")
    require(receipt.get("adaptive_source_receipt_sha256") == adaptive["source_receipt_sha256"], "calibration_adaptive_source_sha")
    require(receipt.get("adaptive_v4d_source_receipt_sha256") == adaptive["v4d_source_receipt_sha256"], "calibration_adaptive_v4d_source_sha")
    require(receipt.get("adaptive_teacher_generation") == ADAPTIVE_TEACHER_GENERATION, "calibration_adaptive_generation")
    return receipt
...
def substitute_command(
    ...
    if smoke:
        ...
        lane_argv = manifest["trainer"]["lane_outer_extra_argv"]
        if lane_argv is not None:
            command.extend(lane_argv[lane])
    else:
...
def dry_run_plan(manifest_path: pathlib.Path, *, runtime_override: pathlib.Path | None = None) -> dict[str, Any]:
    ...
    calibration_ready = manifest["status"] == "PREFREEZE_V2_2_2_ADAPTIVE_MULTI_SEED_READY_DO_NOT_START"
    ...
    for lane in LANE_GPU:
        ...
        if calibration_ready:
            ...
            production[lane]["blocked_by"] = "PENDING_V2_2_2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION"
    return {
        "schema_version": "pvrig_v6_residue_v2_4_node1_dry_run_v2_2_2_bundle_receipt_contract_corrected",
        "status": (
            "PASS_V2_2_2_ADAPTIVE_PREFREEZE_DRY_RUN_NO_RUNTIME_MUTATION"
            if calibration_ready else "PASS_V2_2_2_ADAPTIVE_PREFREEZE_DRY_RUN_BLOCKED_PENDING_CALIBRATION"
        ),
...
def validate_freeze(manifest_path: pathlib.Path, freeze_path: pathlib.Path) -> dict[str, Any]:
    ...
    require(
        freeze.get("trainer_result_claim_boundary") == TRAINER_RESULT_CLAIM_BOUNDARY,
        "freeze_trainer_result_claim_boundary",
    )
    require(freeze.get("bundle_revision") == BUNDLE_REVISION, "freeze_bundle_revision")
...
def _run_one(
    ...
    require(result.get("claim_boundary") == manifest["trainer_result_claim_boundary"],
            f"trainer_result_claim_boundary:{lane}:{fold}")
...
def execute_smoke(manifest_path: pathlib.Path, freeze_path: pathlib.Path) -> dict[str, Any]:
    ...
    receipt = {
        "schema_version": "pvrig_v6_residue_v2_4_node1_tiny_smoke_receipt_v2_2_2_bundle_receipt_contract_corrected",
...
def validate_smoke_receipt(manifest_path: pathlib.Path, freeze_path: pathlib.Path, runtime: pathlib.Path) -> dict[str, Any]:
    ...
    require(receipt.get("status") == "PASS_V2_4_ADAPTIVE_V2_2_2_TINY_SMOKE_ALL_FOUR_LANES_STOP_BEFORE_OUTER", "smoke_receipt_status")
    ...
    require(receipt.get("claim_boundary") == CLAIM_BOUNDARY, "smoke_claim_boundary")
    require(
        receipt.get("trainer_result_claim_boundary") == TRAINER_RESULT_CLAIM_BOUNDARY,
        "smoke_trainer_result_claim_boundary",
    )
    require(receipt.get("bundle_revision") == BUNDLE_REVISION, "smoke_bundle_revision")
...
def execute_outer(manifest_path: pathlib.Path, freeze_path: pathlib.Path) -> dict[str, Any]:
    ...
    receipt = {
        "schema_version": "pvrig_v6_residue_v2_4_node1_outer_development_receipt_v2_2_2_bundle_receipt_contract_corrected",
        "status": "PASS_V2_4_ADAPTIVE_V2_2_2_FOUR_LANE_OUTER_DEVELOPMENT_AFTER_INDEPENDENT_GATES",
        "manifest_sha256": sha256_file(manifest_path),
        "implementation_freeze_sha256": sha256_file(freeze_path),
        "formal_artifact_sha256": observed,
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_V2_4_ADAPTIVE_V2_2_2_DEPLOYMENT:{error}", file=os.sys.stderr)
        raise SystemExit(1)
