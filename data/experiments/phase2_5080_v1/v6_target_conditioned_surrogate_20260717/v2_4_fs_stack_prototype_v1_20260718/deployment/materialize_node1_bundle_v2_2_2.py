#!/usr/bin/env python3
"""Stage the V2.2.2 bundle-receipt-contract technical-supersession Node1 bundle."""
...
import node1_v2_4_outer_development_launcher_v2_2_2 as deployment  # noqa: E402


PENDING_STATUS = "PREFREEZE_V2_2_2_ADAPTIVE_MULTI_SEED_CALIBRATION_PENDING_DO_NOT_START"
MANIFEST_NAME = "V2_4_NODE1_PREFREEZE_MANIFEST_V2_2_2.json"


def validate_bundle_receipt(receipt: dict[str, Any], manifest: dict[str, Any]) -> None:
    deployment.require(receipt.get("claim_boundary") == manifest["claim_boundary"], "bundle_receipt_claim_boundary")
    deployment.require(
        receipt.get("trainer_result_claim_boundary") == manifest["trainer_result_claim_boundary"],
        "bundle_receipt_trainer_result_claim_boundary",
    )
    deployment.require(receipt.get("bundle_revision") == deployment.BUNDLE_REVISION, "bundle_receipt_bundle_revision")
...
def materialize(manifest_path: pathlib.Path, readme_path: pathlib.Path, output_root: pathlib.Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "prefreeze_builder", "calibration_runner", "calibration_runner_test",
        "deployment_launcher", "deployment_launcher_test", "postcalibration_materializer",
        "postcalibration_materializer_test", "v2_migration_test",
        "bundle_materializer", "v2_2_supersession_audit",
        "v2_2_2_bundle_receipt_supersession_audit",
    ...
    receipt = {
        "schema_version": "pvrig_v6_residue_v2_4_node1_staged_bundle_v2_2_2_bundle_receipt_contract_corrected",
        "status": "PASS_NODE1_BUNDLE_V2_2_2_STAGED_CALIBRATION_PENDING_DO_NOT_EXECUTE_OUTER",
        ...
        "next_permitted_action": (
            "Deploy to the exact V2.2.2 bundle_root, validate all Node1 artifacts, then run the "
            "open-only pre-optimizer-step calibration; do not start smoke or outer training."
        ),
    }
    validate_bundle_receipt(receipt, manifest)
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_V2_4_BUNDLE_V2_2_2_MATERIALIZATION:{error}", file=os.sys.stderr)
        raise SystemExit(1)
