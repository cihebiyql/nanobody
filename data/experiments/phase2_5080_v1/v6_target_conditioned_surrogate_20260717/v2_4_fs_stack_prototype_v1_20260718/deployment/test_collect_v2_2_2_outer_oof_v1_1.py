...
from . import collect_v2_2_2_outer_oof_v1_1 as mod
...
class Fixture:
    ...
    def _write_one_result(self, lane: str, fold: int) -> None:
        ...
        split = json.loads(self.split_paths[fold].read_text())
        result_split = {key: value for key, value in split.items() if key not in mod.RESULT_SPLIT_PROVENANCE_ONLY_FIELDS}
        artifacts = {
        ...
        result = {
            ...
            "split": result_split, "claim_boundary": mod.TRAINER_RESULT_CLAIM_BOUNDARY,
...
class CollectV222OuterOOFTests(unittest.TestCase):
    ...
    def test_collects_all_lanes_and_stops_before_nested_stack(self) -> None:
        ...
        metrics = json.loads((self.fixture.output / "collection_v1_1" / "OOF_METRICS.json").read_text())
        ...
        for lane in mod.LANES:
            self.assertTrue((self.fixture.output / "collection_v1_1" / "oof" / f"{lane}.outer_oof.tsv").is_file())
    ...
    def test_accepts_exact_trainer_split_projection_without_provenance_only_fields(self) -> None:
        f = self.fixture
        result = json.loads((f.runtime / "outer_development" / "A_VHH_ONLY" / "fold_0" / "RESULT.json").read_text())
        frozen = json.loads(f.split_paths[0].read_text())
        self.assertEqual(set(frozen) - set(result["split"]), mod.RESULT_SPLIT_PROVENANCE_ONLY_FIELDS)
        self.assertEqual(set(result["split"]) & mod.RESULT_SPLIT_PROVENANCE_ONLY_FIELDS, set())
        terminal = self.collect()
        self.assertEqual(terminal["result_count"], 20)

    def test_rejects_trainer_split_projection_missing_parent_gate(self) -> None:
        f = self.fixture; lane, fold = "A_VHH_ONLY", 0
        result_path = f.runtime / "outer_development" / lane / f"fold_{fold}" / "RESULT.json"
        result = json.loads(result_path.read_text())
        result["split"].pop("score_parents")
        write_json(result_path, result)
        outer_path = f.runtime / "status" / "OUTER_DEVELOPMENT_RECEIPT.json"
        outer = json.loads(outer_path.read_text())
        outer["outer_development"][lane][fold]["result_sha256"] = mod.sha256_file(result_path)
        write_json(outer_path, outer)
        with self.assertRaisesRegex(mod.CollectionError, "result_split_projection_keys"):
            self.collect()

    def test_rejects_trainer_split_projection_parent_mismatch(self) -> None:
        f = self.fixture; lane, fold = "A_VHH_ONLY", 0
        result_path = f.runtime / "outer_development" / lane / f"fold_{fold}" / "RESULT.json"
        result = json.loads(result_path.read_text())
        result["split"]["score_parents"] = ["P99"]
        write_json(result_path, result)
        outer_path = f.runtime / "status" / "OUTER_DEVELOPMENT_RECEIPT.json"
        outer = json.loads(outer_path.read_text())
        outer["outer_development"][lane][fold]["result_sha256"] = mod.sha256_file(result_path)
        write_json(outer_path, outer)
        with self.assertRaisesRegex(mod.CollectionError, "result_split_projection_values"):
            self.collect()
    ...
    def test_existing_terminal_resume_is_hash_validated(self) -> None:
        f = self.fixture
        self.collect()
        metrics_path = f.output / "collection_v1_1" / "OOF_METRICS.json"
