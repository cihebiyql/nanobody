#!/usr/bin/env python3
"""Build the V2.4 adaptive-multiseed V2.1 prefreeze manifest.

V2.1 supersedes the pre-observation V2 command-construction failure and uses
new immutable bundle/calibration/production roots.  It never accepts the
...
BUNDLE = "/data1/qlyu/projects/pvrig_v6_residue_v2_4_deployment_bundle_v2_1_20260718"
RUNTIME = "/data1/qlyu/projects/pvrig_v6_residue_v2_4_four_lane_oof_v2_1_20260718"
CALIBRATION_RUNTIME = "/data1/qlyu/projects/pvrig_v6_residue_v2_4_calibration_v2_1_20260718"
...
MANIFEST_STATUS = "PREFREEZE_V2_ADAPTIVE_MULTI_SEED_CALIBRATION_PENDING_DO_NOT_START"
...
EXPECTED_V2_IMPLEMENTATION_SHA256 = {
    "calibration_trainer": "a8136b28ded7f173e5327f1e0d30d8e1ec780f03bf411367058804b6afee61be",
    "calibration_trainer_test": "3ddc57cd971a2fb77bbb4d5283bb012aa9bf1401625c8afebb5268984af010d4",
    "deployment_launcher": "fefd32ed04a28d1516010face537e6e00c265e06689d8f9ed215461889a4d2a8",
    "calibration_runner": "d78a86c567f969b57455e5fea830c50ddbf2ca853a6a70ce56814a8b2872fa64",
    "postcalibration_materializer": "83f354730241ed5554dc6e1b971dbd65850118407ec84bfa4f4622dc172368a0",
    "postcalibration_materializer_test": "6538971b4acdd0515b3e02d1fe53570c1b65d926d6eca4787554171f0f00f40d",
    "v2_migration_test": "35a55afa72c37471df622a71107d845ed44dd026a1b73bc50d8c2fb26b03653c",
}
...
def compose_payload(
    ...
    return {
        ...
        "manifest_generation": "V2_ADAPTIVE_MULTI_SEED",
        ...
        "calibration_contract": {
            "binding_status": "PENDING_V2_ADAPTIVE_OPEN_ONLY_PRESTEP_CALIBRATION",
        ...
        "pending": ["CALIBRATION_RECEIPT.json", "frozen_lane_contact_weights", "IMPLEMENTATION_FREEZE_V2_4_ADAPTIVE_V2.json"],
...
def run(
    *, repo: pathlib.Path, output: pathlib.Path, input_contract: pathlib.Path,
    ...
    specs = {
        ...
        "calibration_trainer": (f"{p}/trainer/observe_v2_4_multibatch_calibration_v2.py", f"{BUNDLE}/src/observe_v2_4_multibatch_calibration_v2.py"),
        "calibration_trainer_test": (f"{p}/trainer/test_observe_v2_4_multibatch_calibration_v2.py", f"{BUNDLE}/tests/test_observe_v2_4_multibatch_calibration_v2.py"),
        ...
        "calibration_runner": (f"{p}/deployment/run_open_only_prestep_calibration_v2_1.py", f"{BUNDLE}/src/run_open_only_prestep_calibration_v2_1.py"),
        "deployment_launcher": (f"{p}/deployment/node1_v2_4_outer_development_launcher_v2.py", f"{BUNDLE}/src/node1_v2_4_outer_development_launcher_v2.py"),
        "postcalibration_materializer": (f"{p}/deployment/materialize_postcalibration_freeze_v2.py", f"{BUNDLE}/src/materialize_postcalibration_freeze_v2.py"),
        "postcalibration_materializer_test": (f"{p}/deployment/test_materialize_postcalibration_freeze_v2.py", f"{BUNDLE}/tests/test_materialize_postcalibration_freeze_v2.py"),
        "v2_migration_test": (f"{p}/deployment/test_v2_adaptive_deployment_migration.py", f"{BUNDLE}/tests/test_v2_adaptive_deployment_migration.py"),
    ...
    require(output.resolve() == pathlib.Path(BUNDLE) / "V2_4_NODE1_PREFREEZE_MANIFEST_V2.json" or str(output).startswith(str(repo)), "output_path_not_local_or_canonical_node1")
    output.parent.mkdir(parents=True, exist_ok=True)
    require(not output.exists(), f"output_exists:{output}")
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return {"status": "PASS_V2_ADAPTIVE_PREFREEZE_MANIFEST_MATERIALIZED_CALIBRATION_PENDING", "output": str(output), "sha256": sha256_file(output), "artifact_count": len(artifacts)}
...
def input_preflight(paths: Mapping[str, pathlib.Path]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "pvrig_v2_4_adaptive_prefreeze_input_preflight_v2",
        "status": "BLOCKED_INPUT_ADAPTIVE_ARTIFACTS_NOT_MATERIALIZED" if missing else "READY_FOR_V2_MANIFEST_MATERIALIZATION",
        "missing": missing,
        "production_authorized": False,
    }
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_V2_ADAPTIVE_PREFREEZE_MANIFEST:{error}", file=os.sys.stderr)
        raise SystemExit(1)
