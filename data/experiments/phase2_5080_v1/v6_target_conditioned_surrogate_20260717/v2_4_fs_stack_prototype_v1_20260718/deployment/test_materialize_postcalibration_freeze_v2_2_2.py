...
from . import materialize_postcalibration_freeze_v2_2_2 as mod
...
class PostCalibrationFreezeTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.prefreeze = self.bundle / "V2_4_NODE1_PREFREEZE_MANIFEST_V2_2_2.json"
        self.receipt = self.bundle / "CALIBRATION_RECEIPT.json"
        self.ready = self.bundle / "V2_4_NODE1_READY_MANIFEST_V2_2_2.json"
        self.freeze = self.bundle / "IMPLEMENTATION_FREEZE_V2_4_ADAPTIVE_V2_2_2.json"

        files = {}
        for label in (
            "trainer", "calibration_trainer", "prefreeze_builder", "calibration_runner",
            "calibration_runner_test", "deployment_launcher_test",
            "outer_split_0",
            "vhh_graph_cache_npz", "bundle_materializer", "v2_2_supersession_audit",
            "v2_2_2_bundle_receipt_supersession_audit", "postcalibration_materializer_test",
            "v2_migration_test",
        ):
    ...
    def test_materializes_ready_manifest_and_freeze_with_test_only_alias(self) -> None:
        ...
        self.assertEqual(freeze["schema_version"], mod.FREEZE_SCHEMA)
        self.assertEqual(freeze["claim_boundary"], mod.CLAIM_BOUNDARY)
        self.assertEqual(freeze["trainer_result_claim_boundary"], mod.TRAINER_RESULT_CLAIM_BOUNDARY)
        self.assertEqual(freeze["bundle_revision"], mod.BUNDLE_REVISION)
        ...
        for field, error in (
            ...
            ("trainer_result_claim_boundary", "freeze_trainer_result_claim_boundary"),
            ("bundle_revision", "freeze_bundle_revision"),
        ):
            with self.subTest(field=field):
                tampered = copy.deepcopy(freeze)
                tampered[field] = "tampered"
                write_json(self.freeze, tampered)
                with self.assertRaisesRegex(launcher.DeploymentError, error):
                    launcher.validate_freeze(self.ready, self.freeze)
