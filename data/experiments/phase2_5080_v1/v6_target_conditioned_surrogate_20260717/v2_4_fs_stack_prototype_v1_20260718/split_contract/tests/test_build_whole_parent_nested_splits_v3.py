...
MODULE_PATH = ROOT / "src" / "build_whole_parent_nested_splits_v3.py"
SPEC = importlib.util.spec_from_file_location("build_whole_parent_nested_splits_v3", MODULE_PATH)
...
class NestedSplitContractTests(unittest.TestCase):
    ...
    def test_inner_parent_counts_differ_by_at_most_one(self):
        _, _, summary = build()
        for detail in summary["outer_splits"].values():
            counts = list(detail["inner_score_parent_counts"].values())
            self.assertLessEqual(max(counts) - min(counts), 1)
            self.assertLessEqual(detail["inner_parent_count_difference"], 1)
