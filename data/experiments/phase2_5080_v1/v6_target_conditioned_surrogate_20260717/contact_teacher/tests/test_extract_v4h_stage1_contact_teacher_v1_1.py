...
MODULE_PATH = Path(__file__).parents[1] / "src/extract_v4h_stage1_contact_teacher_v1_1.py"
...
class Stage1ContactTeacherTests(unittest.TestCase):
    ...
    def test_selected_models_remap_inaccessible_scorer_paths(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            selected = []
            scores = []
            for index in range(1, 5):
                relative = f"runs/JOB/haddock_run/6_seletopclusts/cluster_{index}_model_1.pdb.gz"
                path = root / relative
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(b"placeholder")
                selected.append(relative)
                scores.append({
                    "pose": f"/root/pvrig_local_offload/runtime/{path.name}",
                    "haddock_io": {"score": -100.0 + index},
                })
            ranked = MOD.ranked_poses(root, {"selected_models": selected, "pose_scores": scores}, "JOB")
            self.assertEqual(len(ranked), 4)
            self.assertTrue(all(str(item[2]).startswith(str((root / "runs").resolve())) for item in ranked))
    ...
    def test_outputs_are_byte_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as first_temporary, tempfile.TemporaryDirectory() as second_temporary:
            ...
            root1, package1, contract1 = self.build_fixture(first)
            root2, package2, contract2 = self.build_fixture(second)
            MOD.extract(root1, package1, contract1, first / "output", workers=1)
            MOD.extract(root2, package2, contract2, second / "output", workers=1)
            for name in (MOD.PAIR_OUTPUT, MOD.RECEPTOR_OUTPUT, MOD.CANDIDATE_OUTPUT):
                self.assertEqual(digest(first / "output" / name), digest(second / "output" / name), name)

    def test_frozen_contract_matches_local_terminal_package(self) -> None:
        contact_root = Path(__file__).parents[1]
        repository_root = contact_root.parents[3]
        package = (
            repository_root
            / "experiments/phase2_5080_v1/prepared/pvrig_v4_h_stage1_terminal_v1_20260717"
        )
        contract = json.loads((contact_root / "V4H_STAGE1_CONTACT_TEACHER_CONTRACT_V1.json").read_text())
        ...
        for name in (
            ...
            self.assertEqual(digest(package / name), expected[name], name)
