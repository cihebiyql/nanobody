...
RUN_SPEC = importlib.util.spec_from_file_location(
    "v13_arm_runner", ROOT / "src" / "run_v220_contact_shared_fold_v1_3.py"
)
...
class SharedCalibrationArtifactTests(unittest.TestCase):
    ...
    def test_execution_sources_preserve_single_materializer_and_zero_arm_calibrator(self):
        materializer = (ROOT / "src/materialize_v220_shared_fold_calibration_v1_3.py").read_text()
        arm = (ROOT / "src/run_v220_contact_shared_fold_v1_3.py").read_text()
        ...
        prereg = json.loads((ROOT / "PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3.json").read_text())
        self.assertFalse(prereg["authorization"]["training_authorized"])
        self.assertTrue(prereg["non_interference"]["all_ten_arms_must_rerun"])
        self.assertFalse(prereg["non_interference"]["v1_2_checkpoint_history_prediction_or_calibration_input"])
