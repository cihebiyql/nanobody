# Node1 Residue V2 open-only contact-gradient calibration V1.2

## V1 失败保留与 V1.2 修复

V1.1 完成 import 闭包后真实运行因四进程 CPU 线程总量接近 64 核而被安全 SIGTERM；其 partial runtime 保留。V1.2 仅新增每进程 8 线程硬环境上限。此前 V1 因 bundle 遗漏 `build_residue_graph_cache_v2.py` 和 `domain_balance_v2.py` 而 fail-closed；V1 文件、matrix、runtime 和失败证据保持原样。V1.2 使用全新 bundle/runtime，并对 trainer 的完整 repo-local import 闭包逐文件绑定 SHA256。

## CPU 上限

每个 GPU 进程固定 `OMP_NUM_THREADS=8`、`MKL_NUM_THREADS=8`、`OPENBLAS_NUM_THREADS=8`、`NUMEXPR_NUM_THREADS=8`；四 lane 总上限约 32 CPU 核。augmentation 也使用同一单进程上限。
...
- bundle：`/data1/qlyu/projects/pvrig_v6_residue_v2_contact_gradient_calibration_bundle_v1_2_20260718`
- runtime：`/data1/qlyu/projects/pvrig_v6_residue_v2_contact_gradient_calibration_v1_2_20260718`
...
```text
residue_v2/calibration/node1_contact_gradient_calibration_v1_2.py
residue_v2/calibration/CONTACT_GRADIENT_CALIBRATION_MATRIX_V1_2.json
...
```bash
B=/data1/qlyu/projects/pvrig_v6_residue_v2_contact_gradient_calibration_bundle_v1_2_20260718
PY=/data1/qlyu/software/envs/pvrig-v6-tc/bin/python
L=$B/residue_v2/calibration/node1_contact_gradient_calibration_v1_2.py
M=$B/residue_v2/calibration/CONTACT_GRADIENT_CALIBRATION_MATRIX_V1_2.json
