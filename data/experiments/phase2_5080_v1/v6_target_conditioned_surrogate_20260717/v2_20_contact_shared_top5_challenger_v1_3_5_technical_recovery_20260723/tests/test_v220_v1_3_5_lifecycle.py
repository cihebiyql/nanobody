...
PREFLIGHT = load("v131_preflight_test", "src/build_v220_v1_3_5_preflight_receipt.py")
FINALIZER = load("v131_finalizer_test", "src/finalize_v220_v1_3_5_training_authorization.py")
HELPER = ROOT / "launchers/run_shared_fold_materialization_once_v1_3_1.sh"
TEMPLATE = ROOT / "launchers/run_phase1_core_fold_pair_node1_v1_3_5.template.sh"
PREREG = ROOT / "PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3_5.json"
LEGACY_ADAPTER = ROOT / "launchers/run_legacy_102_tests_python311_v1_3_4.sh"
...
def make_preflight_fixture(root: Path, expected_new_tests: int = 17) -> dict[str, Path]:
    ...
    write_json(freeze, {
        "status": "FROZEN_V1_3_5_IMPLEMENTATION_PENDING_INDEPENDENT_REVIEW_AND_NODE1_PREFLIGHT",
        "training_started": False,
        "implementation_hashes": {
            "PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3_5.json": sha(prereg),
        },
    })
    legacy = root / "legacy.log"
    legacy.write_text(
        "Ran 102 tests in 1.0s\n\nOK\n"
        "PASS_LEGACY_102_PYTHON311_COMPATIBLE python=Python 3.11.14\n"
    )
    new = root / "new.log"
    new.write_text(f"Ran {expected_new_tests} tests in 0.1s\n\nOK\n")
    for fold in range(5):
        shared = runtime / "shared_calibration" / f"fold_{fold}"
        ...
        artifact_path = shared / "CONTACT_WEIGHT_CALIBRATION.json"
        write_json(artifact_path, artifact)
        digest = sha(artifact_path)
        write_json(shared / "MATERIALIZATION_TERMINAL.json", {
            "status": PREFLIGHT.MATERIALIZATION_STATUS,
            "fold_id": fold,
            "seed": 43,
        ...
        write_json(runtime / "load_only" / f"fold_{fold}.json", {
            "status": PREFLIGHT.LOAD_ONLY_STATUS,
            "fold_id": fold,
            "seed": 43,
            "loaded_arms": {
                arm: {"shared_artifact_sha256": digest} for arm in ("C0", "C1")
            },
...
class FiveFoldPreflightReceiptTests(unittest.TestCase):
    def test_five_fold_synthetic_separate_process_evidence_closes_no_training_receipt(self):
        with tempfile.TemporaryDirectory() as directory:
            paths = make_preflight_fixture(Path(directory))
            args = argparse.Namespace(
                runtime_root=paths["runtime"],
                training_sentinel=paths["runtime"] / "training_output_forbidden",
                implementation_freeze=paths["freeze"],
                expected_implementation_freeze_sha256=sha(paths["freeze"]),
                preregistration=paths["prereg"],
                legacy_test_log=paths["legacy"],
                v1_3_5_test_log=paths["new"],
                expected_new_tests=17,
                output_receipt=paths["runtime"] / "NODE1_V1_3_5_PREFLIGHT_RECEIPT.json",
            )
    ...
    def test_receipt_builder_real_cli_parses_v1_3_5_log_and_builds(self):
        with tempfile.TemporaryDirectory() as directory:
            paths = make_preflight_fixture(Path(directory), expected_new_tests=18)
            output = paths["runtime"] / "NODE1_V1_3_5_PREFLIGHT_RECEIPT.json"
            result = subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "src/build_v220_v1_3_5_preflight_receipt.py"),
                    "--runtime-root", str(paths["runtime"]),
                    "--training-sentinel", str(paths["runtime"] / "training_output_forbidden"),
                    "--implementation-freeze", str(paths["freeze"]),
                    "--expected-implementation-freeze-sha256", sha(paths["freeze"]),
                    "--preregistration", str(paths["prereg"]),
                    "--legacy-test-log", str(paths["legacy"]),
                    "--v1-3-5-test-log", str(paths["new"]),
            ...
            self.assertEqual(payload["tests"]["v1_3_5"]["tests_run"], 18)
            self.assertEqual(payload["tests"]["combined_tests_run"], 120)
            self.assertFalse(payload["training_started"])
            self.assertFalse((paths["runtime"] / "training_output_forbidden").exists())
...
class FinalizationLifecycleTests(unittest.TestCase):
    ...
    def test_success_writes_only_launcher_and_false_started_authorization(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(sorted(p.name for p in args.output_dir.iterdir()), ["FINAL_TRAINING_AUTHORIZATION_V1_3_5.json", "run_phase1_core_fold_pair_node1_v1_3_5.sh"])
            launcher = args.output_dir / "run_phase1_core_fold_pair_node1_v1_3_5.sh"
...
class SourceClosureTests(unittest.TestCase):
    def test_fully_frozen_package_exact_test_launcher_passes(self):
        if os.environ.get("V220_V135_NESTED_FROZEN_TEST") == "1":
            return
        with tempfile.TemporaryDirectory() as directory:
            ...
            sidecar.write_text(f"{freeze_sha}  {freeze.name}\n")
            result = subprocess.run(
                ["bash", str(package / "launchers/run_tests_v1_3_5.sh")],
                env={
                    **os.environ,
                    "PYTHON_BIN": sys.executable,
                    "V220_V135_NESTED_FROZEN_TEST": "1",
                },
                text=True,
                capture_output=True,
            )
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn("Ran 46 tests", result.stdout)
            self.assertRegex(result.stdout, r"(?m)^OK$")

    def test_package_allowlist_rejects_extra_file(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            freeze = root / "IMPLEMENTATION_FREEZE_PHASE1_TECHNICAL_RECOVERY_V1_3_5.json"
    ...
    def test_preflight_launcher_has_no_arm_training_runner_and_binds_helper_argv(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_5.sh").read_text()
...
class Python311LegacyAdapterTests(unittest.TestCase):
    ...
    def test_adapter_uses_exact_relative_argv_and_never_discover(self):
        ...
        self.assertIn('"$PYTHON_BIN" "$EXACT_FILE_RUNNER" "$LEGACY_ROOT" "${REL_TEST_FILES[@]}" --verbosity 2', source)
    ...
    def test_adapter_does_not_execute_unbound_nested_test(self):
        ...
        runner = (ROOT / "src/run_unittest_file_paths_v1_3_4.py").read_text()
        self.assertIn('relative.parent == Path("tests")', runner)
        self.assertIn('not path.is_symlink()', runner)
    ...
    def test_combined_launcher_uses_adapter_with_explicit_legacy_root(self):
        source = (ROOT / "launchers/run_combined_legacy_and_v1_3_5_tests.sh").read_text()
        self.assertIn('run_legacy_102_tests_python311_v1_3_4.sh" "$LEGACY_ROOT"', source)
    ...
    def test_preflight_binds_adapter_and_unchanged_v131_core(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_5.sh").read_text()
        self.assertIn("run_legacy_102_tests_python311_v1_3_4.sh", source)
    ...
    def test_preflight_bound_legacy_launcher_sha_matches_actual_frozen_file(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_5.sh").read_text()
        match = re.search(
            r'^EXPECTED_LEGACY_TEST_LAUNCHER_SHA="([0-9a-f]{64})"$',
            source,
            re.MULTILINE,
        )
        ...
        self.assertEqual(match.group(1), sha(LEGACY_ADAPTER))
    ...
    def test_new_test_launcher_uses_relative_paths_not_root_absolute_argv(self):
        source = (ROOT / "launchers/run_tests_v1_3_5.sh").read_text()
        self.assertIn('cd "$ROOT"', source)
        self.assertNotIn("-m unittest", source)
        self.assertIn('"$PYTHON_BIN" "$EXACT_FILE_RUNNER" "$ROOT" "${REL_TEST_FILES[@]}" --verbosity 2', source)
