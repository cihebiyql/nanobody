#!/usr/bin/env python3
"""Build canonical sparse V4D + adaptive V4H pair teachers for D_FULL_PAIR.
...
SCHEMA_VERSION = "pvrig_v6_adaptive_dual_source_pair_contact_targets_v3"
RECEIPT_SCHEMA = "pvrig_v2_4_adaptive_multiseed_dual_source_pair_receipt_v1"
STATUS = "PASS_V2_4_ADAPTIVE_MULTI_SEED_DUAL_SOURCE_PAIR_MATERIALIZED"
TEACHER_GENERATION = "V4D_MULTI_SEED_PLUS_V4H_ADAPTIVE_MULTI_SEED_V2"
OUTPUT_NAME = "v6_adaptive_dual_source_pair_contact_targets_v3.tsv.gz"
GROUP_AUDIT_NAME = "v6_adaptive_dual_source_pair_group_audit_v3.tsv"
...
V4H_REQUIRED = {
    ...
    "pvrig_uniprot_position", "pvrig_aa", "contact_target_mean",
    "contact_target_variance", "contact_uncertainty_weight",
    "supporting_seed_count", "seed_contact_values",
}
V4H_CANDIDATE_REQUIRED = {
    "schema_version", "teacher_state", "candidate_id", "sequence_sha256",
    "parent_framework_cluster", "paired_seed_ids", "paired_seed_count",
}
OUTPUT_FIELDS = (
...
def validate_v4h_receipt(
    ...
    payload = json.loads(path.read_text())
    require(payload.get("schema_version") == "pvrig_v6_v4h_adaptive_multiseed_contact_teacher_v2_receipt", "v4h_receipt_schema")
    require(payload.get("status") == "PASS_V4H_ADAPTIVE_MULTI_SEED_CONTACT_EXTRACTION", "v4h_receipt_status")
    require(int(payload.get("candidate_rows", -1)) == expected_candidates + 39, "v4h_receipt_total_candidate_count")
    require(int(payload.get("valid_candidate_rows", -1)) == expected_candidates, "v4h_receipt_candidate_count")
    require(int(payload.get("technical_incomplete_candidate_rows", -1)) == 39, "v4h_receipt_incomplete_count")
    ...
    require((payload.get("output_hashes") or {}).get(candidate_path.name) == sha256_file(candidate_path), "v4h_receipt_candidate_hash")
    return payload
    ...
    for source, rows in ((V4D, v4d_rows), (V4H, v4h_rows)):
        for row in rows:
            ...
            if source == V4D:
                ...
                require(row["schema_version"] == "pvrig_v6_v4h_adaptive_multiseed_contact_teacher_v2", "v4h_row_schema")
                target = finite_unit(row["contact_target_mean"], "v4h_target")
                variance = float(row["contact_target_variance"])
                uncertainty = finite_unit(row["contact_uncertainty_weight"], "v4h_uncertainty")
                observed = int(row["observed_seed_count"])
                expected = observed
                require(observed in {1, 2, 3}, f"v4h_observed_seed_count:{key}")
                ...
                require(abs(float(numeric.mean()) - target) <= 5e-9, f"v4h_seed_mean:{key}")
                require(abs(float(numeric.var(ddof=0)) - variance) <= 5e-9, f"v4h_seed_variance:{key}")
                require(abs(uncertainty - 1.0 / (1.0 + 4.0 * variance)) <= 5e-9, f"v4h_uncertainty_formula:{key}")
                supporting = int(row["supporting_seed_count"])
                require(supporting == int(np.sum(numeric > 0.0)), f"v4h_supporting_seed_count:{key}")
                require(supporting > 0 and target > 0.0, f"v4h_nonzero_sparse_row:{key}")
                aggregation = "paired_seed_intersection_mean_over_observed_successful_seeds"
...
def group_audit_rows(
    ...
    for candidate in sorted(training):
        ...
        for receptor in RECEPTORS:
            ...
            expected: int | str = 3 if truth["teacher_source"] == V4D else (observed_max if seeds else "")
            yield {
...
def build_targets(
    *,
    ...
    v4h_receipt_payload = validate_v4h_receipt(
        v4h_receipt, v4h_pair_tsv_gz, v4h_candidate_tsv_gz,
    ...
    v4h_candidate_fields, v4h_candidate_rows = read_tsv(v4h_candidate_tsv_gz)
    ...
    require(V4H_CANDIDATE_REQUIRED <= set(v4h_candidate_fields), f"v4h_candidate_fields_missing:{sorted(V4H_CANDIDATE_REQUIRED-set(v4h_candidate_fields))}")
    ...
    v4h_candidate_seed_counts: dict[str, int] = {}
    for row in v4h_candidate_rows:
        candidate = row["candidate_id"].strip()
        ...
        if row["teacher_state"] == "TECHNICAL_INCOMPLETE_NA":
            require(row["paired_seed_count"] == "", f"v4h_na_numeric_not_empty:{candidate}")
            ...
            require(candidate in training, f"v4h_candidate_not_training:{candidate}")
            require(row["sequence_sha256"] == training[candidate]["sequence_sha256"], f"v4h_candidate_sequence_hash:{candidate}")
            require(row["parent_framework_cluster"] == training[candidate]["parent_framework_cluster"], f"v4h_candidate_parent:{candidate}")
            valid_v4h_candidates.add(candidate)
            v4h_candidate_seed_counts[candidate] = count
    training_v4h = {candidate for candidate, row in training.items() if row["teacher_source"] == V4H}
    require(valid_v4h_candidates == training_v4h, "v4h_valid_training_candidate_closure")
    ...
    try:
        ...
        require(
            set(group_counts) == {(candidate, receptor) for candidate in training for receptor in RECEPTORS},
            "nonzero_pair_candidate_receptor_closure",
        )
        for candidate in sorted(valid_v4h_candidates):
            for receptor in RECEPTORS:
                require(
                    set(group_seed_ranges[(candidate, receptor)]) == {v4h_candidate_seed_counts[candidate]},
                    f"v4h_group_seed_count_closure:{candidate}:{receptor}",
                )
        source_pair_rows = {
        ...
        input_paths = {
            ...
            "v4h_candidate_tsv_gz": v4h_candidate_tsv_gz,
        ...
        receipt = {
            ...
            "v4h_adaptive_source_receipt_sha256": sha256_file(v4h_receipt),
            "v4d_source_receipt_sha256": sha256_file(v4d_receipt),
            "v4h_adaptive_raw_pair_teacher_sha256": sha256_file(v4h_pair_tsv_gz),
            "v4h_adaptive_raw_marginal_teacher_sha256": v4h_receipt_payload["output_hashes"]["v4h_adaptive_vhh_residue_marginal_teacher.tsv.gz"],
            "candidate_rows": len(training),
            "v4d_candidate_rows": int(expected_source_counts[V4D]),
            ...
            "counts": {
                **training_audit,
                ...
                "v4h_valid_candidates": len(valid_v4h_candidates),
                "v4h_technical_na_candidates": len(na_v4h_candidates),
            },
            "source_semantics": {
                V4D: "multi-seed mean/variance over observed successful seeds; missing technical seed never zero-imputed",
                V4H: "adaptive paired-seed-intersection mean/variance; A/B/C preserve 3/2/1 paired seeds; sparse absence exact zero",
            },
            "sealed_boundary": {
                ...
                "v4h_technical_incomplete_pose_files_opened": 0,
                "v4h_technical_na_rows_imputed": 0,
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--v4h-candidate-tsv-gz", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    receipt = build_targets(
        ...
        v4h_candidate_tsv_gz=args.v4h_candidate_tsv_gz,
