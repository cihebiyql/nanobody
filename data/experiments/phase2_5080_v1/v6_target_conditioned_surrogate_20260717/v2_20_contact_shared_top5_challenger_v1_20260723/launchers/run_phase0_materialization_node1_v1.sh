...
FREEZE=$ROOT/IMPLEMENTATION_FREEZE_PHASE0_V1.json
EXPECTED_FREEZE_SHA=cc17898d6bb40b5081e8d131498c06cd1aa6e76abda2bc26998755bce1e527db
OUTPUT=$ROOT/release/train_contact_teacher_v1
LOG=$ROOT/PHASE0_MATERIALIZATION_V1.log
STATUS=$ROOT/PHASE0_MATERIALIZATION_TERMINAL_V1.json
...
ARGS=(
  ...
  --phase0-contract "$ROOT/PHASE0_TEACHER_MATERIALIZATION_CONTRACT_V1.json"
...
set +e
"$PYTHON" "$ROOT/src/materialize_v220_train_contact_teacher_v1.py" "${ARGS[@]}" > "$LOG" 2>&1
RC=$?
set -e
if [ "$RC" -eq 0 ]; then
  VERIFY=$("$PYTHON" "$ROOT/src/materialize_v220_train_contact_teacher_v1.py" "${ARGS[@]}" --verify-only)
...
payload={'schema_version':'pvrig_v2_20_phase0_materialization_terminal_v1','status':status,'return_code':int(rc),'oof_training_authorized':False,'log_path':log,'log_sha256':sha(log),'release_path':release,'release_receipt_sha256':sha(Path(release)/'MATERIALIZATION_RECEIPT.json'),'sha256sums_sha256':sha(Path(release)/'SHA256SUMS'),'verification':json.loads(verify)}
