...
ROOT=HERE.parents[1]
MODULE_PATH=ROOT/'src/materialize_v220_train_contact_teacher_v1_2.py'
...
class Phase0FourteenTests(unittest.TestCase):
    ...
    def test_03_v29_top8_coordinate_hash_and_contact_closure(self)->None:
        ...
        row={'job_id':job,'conformation':'8x6b','seed':'917','top8_model_ids':','.join(models),'fixed_top8_count':'4','selected_model_count':'4'}
        result=MOD.process_v29_job({'candidate':candidate,'job':row},guard,self.root,{'8x6b':{41:(1,'A')}})
        self.assertEqual(len(result['inventory']),4);self.assertEqual(len({x['pose_sha256'] for x in result['inventory']}),1)
        self.assertAlmostEqual(result['pair'][(1,41)],1.0);self.assertAlmostEqual(result['marginal'][1],1.0)
        self.assertEqual(guard.audit()['train_pose_files_opened'],4);self.assertAlmostEqual(sum(float(x['pose_weight']) for x in result['inventory']),1.0,places=10);self.assertEqual(len(MOD.raw_pose_weights(8)),8);self.assertEqual(MOD.validated_v29_pose_count({'selected_model_count':'10','fixed_top8_count':'8'}),8)
    ...
    def test_06_frozen_primary_join_count_and_source_contract(self)->None:
        ...
        contract=json.loads((ROOT/'PHASE0_TEACHER_MATERIALIZATION_CONTRACT_V1_2.json').read_text())
