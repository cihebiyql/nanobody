#!/usr/bin/env python3
"""Build the allocation-exact V2.9 independent dual-conformation job manifest."""
...
def build_jobs() -> list[dict[str, str]]:
    entities = {row["entity_id"]: row for row in load_candidates()}
    allocation = read_tsv(root() / "inputs/docking_allocation25000.tsv")
    if len(allocation) != int(protocol()["docking"]["expected_candidate_jobs"]):
        raise RuntimeError(f"allocation count mismatch: {len(allocation)}")
    if len({row["job_id"] for row in allocation}) != len(allocation):
        raise RuntimeError("allocation job IDs are not unique")
    allowed_seeds = {int(seed) for seed in protocol()["docking"]["seeds"]}
    jobs: list[dict[str, str]] = []
    for priority, row in enumerate(allocation, start=1):
        entity_id = row["candidate_id"]
        conformation = row["receptor"].lower()
        seed = int(row["seed"])
        if entity_id not in entities:
            raise RuntimeError(f"allocation candidate missing from executable monomers: {entity_id}")
        if conformation not in CONFORMATIONS or seed not in allowed_seeds:
            raise RuntimeError(f"invalid allocation receptor/seed: {conformation}/{seed}")
        jobs.append(make_job("candidate", entities[entity_id], conformation, seed, priority))
