...
TOP = ROOT.parent
PROFILE_SOURCE = ROOT / "src/run_profiled_v211_b4_inference_v3.py"
VALIDATOR_SOURCE = ROOT / "src/validate_top150k_b_only_recovery_v3.py"
...
PROFILE = import_module("top150k_b3_profile", PROFILE_SOURCE)
VALIDATOR = import_module("top150k_b3_validator", VALIDATOR_SOURCE)
BASE_INFER = import_module("top150k_b3_base_infer", BASE_INFER_SOURCE)
...
class ExactProfileTests(unittest.TestCase):
    ...
    def test_10_production_config_and_state_signature_constants_are_reproducible(self):
        model = import_module("top150k_b3_model_contract", MODEL_SOURCE)
    ...
    def test_12_v211_schema_only_change_preserves_tiny_predictions(self):
        fixture_module = import_module("top150k_b3_old_fixture", BASE_TEST_SOURCE)
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
