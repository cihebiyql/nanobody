#!/usr/bin/env python3
"""Validate and atomically publish the Top150K B-only V3 recovery terminal."""
...
SCHEMA_VERSION = "pvrig_top150k_b_only_recovery_validation_v3"
PREFLIGHT_STATUS = "PASS_EXISTING_GRAPH_L1_AND_V2_FAILURE_CLOSED"
TERMINAL_STATUS = "PASS_GRAPH_L1_B_FULL150K_COMPLETE_VIA_B_ONLY_RECOVERY_V3"
...
def validate_prediction_table(
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        require(set(COMPACT_FIELDS) <= set(fields) and required_ensemble <= set(fields), "prediction_header_incomplete")
...
def validate_graph_receipt(path: Path, expected_rows: int) -> Mapping[str, Any]:
    receipt = load_json(path)
...
def build_preflight(
    *, manifest: Path, graph_receipt: Path, l1_output: Path, l1_receipt: Path,
    ...
    validate_graph_receipt(graph_receipt, expected_rows)
    ...
    return {
        ...
        "rows": expected_rows,
        "bindings": {
            "manifest": sha256_file(manifest), "graph_receipt": sha256_file(graph_receipt),
...
def build_terminal(
    *, manifest: Path, graph_receipt: Path, l1_output: Path, l1_receipt: Path,
    ...
    manifest_rows = load_manifest(manifest, expected_rows)
    graph = validate_graph_receipt(graph_receipt, expected_rows)
    ...
    return {
        ...
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "rows": expected_rows,
        "graph": {"receipt_sha256": sha256_file(graph_receipt), "entities": graph["counts"]["entities"]},
