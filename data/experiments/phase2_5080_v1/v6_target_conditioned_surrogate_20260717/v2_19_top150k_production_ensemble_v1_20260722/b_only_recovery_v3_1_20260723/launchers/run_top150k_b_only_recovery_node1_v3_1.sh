...
PKG="$ROOT/code/b_only_recovery_v3_1_20260723"
...
STATUS="$ROOT/status"
LOGS="$ROOT/logs"
RECOVERY="$ROOT/recovery_b_only_v3_1_20260723"
...
INFER_PROFILED="$PKG/src/run_profiled_v211_b4_inference_v3_1.py"
VALIDATOR="$PKG/src/validate_top150k_b_only_recovery_v3_1.py"
...
"$PY" - "$PKG/IMPLEMENTATION_FREEZE_V3_1.json" "$PKG" "$INFER_BASE" "$BASE" <<'PY'
...
[[ ! -e "$RECOVERY/EXISTING_GRAPH_L1_BINDINGS_V3_1.json" ]]
[[ ! -e "$RECOVERY/B4_PROFILE_VALIDATION_V3_1.json" ]]
[[ ! -e "$RECOVERY/GRAPH_L1_B_TERMINAL_RECOVERY_V3_1.json" ]]
...
"$PY" "$VALIDATOR" preflight \
  ...
  --preflight-receipt "$RECOVERY/EXISTING_GRAPH_L1_BINDINGS_V3_1.json" \
  > "$LOGS/b_only_recovery_v3_1_preflight.log" 2>&1
...
CUDA_VISIBLE_DEVICES=7 OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 \
  ...
  --profile-receipt "$RECOVERY/B4_PROFILE_VALIDATION_V3_1.json" \
  ...
  > "$LOGS/b_4seed_full150k_recovery_v3_1.log" 2>&1
...
"$PY" "$VALIDATOR" publish \
  ...
  --failed-b-log "$FAILED_B_LOG" --expected-rows 150000 \
  --preflight-receipt "$RECOVERY/EXISTING_GRAPH_L1_BINDINGS_V3_1.json" \
  --versioned-terminal "$RECOVERY/GRAPH_L1_B_TERMINAL_RECOVERY_V3_1.json" \
  --canonical-terminal "$STATUS/GRAPH_L1_B_TERMINAL.json" \
  > "$LOGS/b_only_recovery_v3_1_publish.log" 2>&1
