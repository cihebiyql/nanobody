...
NORMALIZED=/data1/qlyu/projects/pvrig_1m_vhh_m2_ca_cache_v1_20260723
...
[[ ! -d /proc/743054 ]]
[[ ! -e "$STATUS/M2_S0M2_TERMINAL.json" && ! -e "$M2_OUT" && ! -e "$PRED_OUT" && ! -e "$NORMALIZED" ]]
...
"$PY" - "$EQUIVALENCE" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x['status']=='PASS_M2_CA_NORMALIZATION_FULL10644_EQUIVALENCE' and x['rows']==10644 and x['features']==126 and x['max_abs_feature_difference']<=1e-8
PY
"$PY" "$PKG/normalize_m2_ca_monomers_v1.py" --m2-manifest "$MANIFEST" \
  --sequence-manifest "$STAGING/top150k_graph_structure_manifest_v1.tsv" --output-dir "$NORMALIZED" \
  --expected-rows 150000 --workers 32 > "$LOGS/m2_ca_normalize_full150k_v1.log" 2>&1
NORM_MANIFEST="$NORMALIZED/normalized_m2_structure_manifest_v1.tsv"
NORM_SHA="$(sha256sum "$NORM_MANIFEST" | awk '{print $1}')"
"$PY" "$M2_SCRIPT" --input-manifest "$NORM_MANIFEST" --expected-manifest-sha256 "$NORM_SHA" \
  --output-dir "$M2_OUT" --expected-rows 150000 --workers 32 > "$LOGS/m2_126d_full150k_recovery_v2_2.log" 2>&1
...
payload={'status':'PASS_M2_AND_S0M2_FULL150K_COMPLETE','m2':json.loads(m2.read_text()),'predictions':json.loads(pred.read_text()),'recovery_validation':json.loads(validation.read_text()),'normalization':json.loads(Path('/data1/qlyu/projects/pvrig_1m_vhh_m2_ca_cache_v1_20260723/NORMALIZATION_RECEIPT.json').read_text()),'equivalence':json.loads(Path('/data1/qlyu/projects/pvrig_top150k_recovery_validation_v1_20260723/M2_CA_NORMALIZATION_EQUIVALENCE_V1.json').read_text())}
