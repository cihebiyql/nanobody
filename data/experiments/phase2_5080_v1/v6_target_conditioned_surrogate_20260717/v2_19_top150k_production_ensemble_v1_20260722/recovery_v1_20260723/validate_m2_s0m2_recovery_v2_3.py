...
def validate_m2(path: Path, expected_rows: int) -> tuple[dict[str, tuple[str, str]], list[str]]:
    mapping: dict[str, tuple[str, str]] = {}
    with path.open(newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        require(reader.fieldnames is not None, "m2_header_missing")
        features = [field for field in reader.fieldnames if field not in M2_METADATA]
...
def main() -> None:
    ...
    mapping, features = validate_m2(args.m2_tsv, args.expected_rows)
    ...
    payload = {
        "schema_version": "pvrig_top150k_m2_s0m2_recovery_validation_v2_3",
