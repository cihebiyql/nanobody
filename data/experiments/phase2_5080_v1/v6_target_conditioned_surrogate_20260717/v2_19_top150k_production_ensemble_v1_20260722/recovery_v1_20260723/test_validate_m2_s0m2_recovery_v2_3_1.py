...
SPEC = importlib.util.spec_from_file_location("validator", HERE / "validate_m2_s0m2_recovery_v2_3_1.py")
...
class ValidatorTests(unittest.TestCase):
    ...
    def test_claim_boundary_is_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            mapping, observed = MOD.validate_m2(path, 1, expected)
            self.assertEqual(expected, observed)
            self.assertEqual(mapping["c1"], ("s1", "p1"))

    def test_any_nontraining_header_column_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "m2.tsv"
            self.write_m2(path, ["ALL__unexpected_feature"])
            with self.assertRaisesRegex(MOD.ValidationError, "m2_header_not_exact_training_schema"):
                MOD.validate_m2(path, 1, [f"ALL__feature_{index:03d}" for index in range(126)])
