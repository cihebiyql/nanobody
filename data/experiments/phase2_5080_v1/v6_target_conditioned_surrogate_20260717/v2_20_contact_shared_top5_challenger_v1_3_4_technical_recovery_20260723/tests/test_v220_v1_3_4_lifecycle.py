...
PREFLIGHT = load("v131_preflight_test", "src/build_v220_v1_3_4_preflight_receipt.py")
FINALIZER = load("v131_finalizer_test", "src/finalize_v220_v1_3_4_training_authorization.py")
HELPER = ROOT / "launchers/run_shared_fold_materialization_once_v1_3_1.sh"
TEMPLATE = ROOT / "launchers/run_phase1_core_fold_pair_node1_v1_3_4.template.sh"
...
FREEZE_NAME = "IMPLEMENTATION_FREEZE_PHASE1_TECHNICAL_RECOVERY_V1_3_4.json"
...
class FiveFoldPreflightReceiptTests(unittest.TestCase):
    def test_five_fold_synthetic_separate_process_evidence_closes_no_training_receipt(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_json(freeze, {
                "status": "FROZEN_V1_3_4_IMPLEMENTATION_PENDING_INDEPENDENT_REVIEW_AND_NODE1_PREFLIGHT",
                "training_started": False,
                "implementation_hashes": {"PREREGISTRATION_PHASE1_TECHNICAL_RECOVERY_V1_3_4.json": sha(prereg)},
            })
            ...
            args = argparse.Namespace(
                ...
                preregistration=prereg, legacy_test_log=legacy, v1_3_4_test_log=new,
                expected_new_tests=17, output_receipt=runtime / "NODE1_V1_3_4_PREFLIGHT_RECEIPT.json",
            )
...
class FinalizationLifecycleTests(unittest.TestCase):
    ...
    def test_success_writes_only_launcher_and_false_started_authorization(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(sorted(p.name for p in args.output_dir.iterdir()), ["FINAL_TRAINING_AUTHORIZATION_V1_3_4.json", "run_phase1_core_fold_pair_node1_v1_3_4.sh"])
            launcher = args.output_dir / "run_phase1_core_fold_pair_node1_v1_3_4.sh"
...
class SourceClosureTests(unittest.TestCase):
    def test_fully_frozen_package_exact_test_launcher_passes(self):
        if os.environ.get("V220_V134_NESTED_FROZEN_TEST") == "1":
            return
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ["bash", str(package / "launchers/run_tests_v1_3_4.sh")],
                env={
                    **os.environ,
                    "PYTHON_BIN": sys.executable,
                    "V220_V134_NESTED_FROZEN_TEST": "1",
                },
                text=True,
                capture_output=True,
            )
    ...
    def test_package_allowlist_rejects_extra_file(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            freeze = root / "IMPLEMENTATION_FREEZE_PHASE1_TECHNICAL_RECOVERY_V1_3_4.json"
    ...
    def test_preflight_launcher_has_no_arm_training_runner_and_binds_helper_argv(self):
        source = (ROOT / "launchers/run_phase1_preflight_node1_v1_3_4.sh").read_text()
...
class Python311LegacyAdapterTests(unittest.TestCase):
    ...
    def test_combined_launcher_uses_adapter_with_explicit_legacy_root(self):
        source = (ROOT / "launchers/run_combined_legacy_and_v1_3_4_tests.sh").read_text()
    ...
    def test_new_test_launcher_uses_relative_paths_not_root_absolute_argv(self):
        source = (ROOT / "launchers/run_tests_v1_3_4.sh").read_text()
