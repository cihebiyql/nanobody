...
RUNNER_PATH = HERE / "run_clean_attention_inner_oof_fold_v1.py"
SPEC = importlib.util.spec_from_file_location("v212_clean_attention_oof_runner_for_collection", RUNNER_PATH)
...
SCHEMA = "pvrig_v2_12_clean_attention_inner_oof_collection_v1"
OUTPUT_NAME = "CLEAN_ATTENTION_TRAIN9849_OOF_PREDICTIONS.tsv"
...
def collect(teacher_path: Path, assignment_path: Path, contracts_dir: Path, run_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for fold_id in range(5):
        ...
        require(result.get("status") == "PASS_V2_12_CLEAN_ATTENTION_INNER_OOF_FOLD_TRAINING", f"fold_status:{fold_id}")
        ...
        required = {
            "candidate_id", "sequence_sha256", "parent_framework_cluster", "fold_id", "seed",
    ...
    try:
        ...
        fields = (
            ...
            "B_CLEAN_TARGET_ATTENTION__R8", "B_CLEAN_TARGET_ATTENTION__R9", "B_CLEAN_TARGET_ATTENTION__Rdual_exact_min",
        )
        with output_path.open("w", newline="", encoding="utf-8") as handle:
            ...
            for row in rows:
                ...
                writer.writerow({
                    ...
                    "parent_framework_cluster": row.parent, "fold_id": assignment[row.candidate_id]["fold_id"], "seed": "43",
                    "truth_R8": f"{row.targets[0]:.12g}", "truth_R9": f"{row.targets[1]:.12g}",
                    "truth_Rdual_exact_min": f"{min(row.targets):.12g}",
                    "B_CLEAN_TARGET_ATTENTION__R8": f"{pred[0]:.12g}",
                    "B_CLEAN_TARGET_ATTENTION__R9": f"{pred[1]:.12g}",
                    "B_CLEAN_TARGET_ATTENTION__Rdual_exact_min": f"{min(pred):.12g}",
                })
        metrics_path = staging / METRICS_NAME
        metrics_path.write_text(json.dumps(metrics, indent=2, sort_keys=True) + "\n")
        receipt = {
            "schema_version": SCHEMA,
            "status": "PASS_CLEAN_ATTENTION_TRAIN9849_WHOLE_PARENT_OOF",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = collect(args.teacher, args.assignment, args.contracts_dir, args.run_root, args.output_dir)
