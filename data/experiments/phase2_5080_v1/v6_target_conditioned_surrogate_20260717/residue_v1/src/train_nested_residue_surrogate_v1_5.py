#!/usr/bin/env python3
"""V1.5 production trainer with nested whole-parent selection and resumability."""
...
SCHEMA_VERSION = "pvrig_v6_nested_residue_surrogate_v1_5"
COLLECTOR_SCHEMA_VERSION = "pvrig_v6_residue_v1_5_oof_collector"
POSITIVE_PROMOTION_STATUS = "PROMOTE_RESIDUE_V1_5_OVER_M2"
NEGATIVE_PROMOTION_STATUS = "DO_NOT_PROMOTE_RESIDUE_V1_5"
FORMAL_EVIDENCE_LABELS = {
    "collector_schema": COLLECTOR_SCHEMA_VERSION,
    "negative_status": NEGATIVE_PROMOTION_STATUS,
    "positive_status": POSITIVE_PROMOTION_STATUS,
    "trainer_schema": SCHEMA_VERSION,
}
INNER_FOLD_COUNT = 5
...
COLLECTOR_MATRIX = {
    ...
    "governance_amendment_sha256": GOVERNANCE_SHA256,
    "promotion_gate": EXACT_PROMOTION_GATE,
    "schema_version": "pvrig_v6_residue_v1_4_collector_matrix",
}
COLLECTOR_MATRIX_SHA256 = hashlib.sha256(
    json.dumps(COLLECTOR_MATRIX, sort_keys=True, separators=(",", ":")).encode()
).hexdigest()
...
def result_affecting_binding(
    args: argparse.Namespace,
    feature_names: Sequence[str],
    external_hashes: Mapping[str, str],
    outer_fold: int,
) -> dict[str, Any]:
    """Canonical exhaustive config bound before any resume or training step."""
    payload = {
        "schema_version": f"{SCHEMA_VERSION}_result_affecting_binding",
        "outer_fold": int(outer_fold),
        "external_hashes": dict(sorted((str(key), str(value)) for key, value in external_hashes.items())),
        "structure": {
            "dimension": int(args.structure_dim),
            "prefixes": list(args.structure_prefix),
            "feature_names": list(feature_names),
            "feature_names_sha256": hashlib.sha256("\n".join(feature_names).encode()).hexdigest(),
        },
        "m2": {"ridge_alpha": float(args.ridge_alpha)},
        "backbone": {
            "kind": str(args.backbone_kind),
            "mode": str(args.backbone_mode),
            "model_path": str(args.model_path) if args.model_path is not None else None,
            "model_identity_file": str(args.model_identity_file) if args.model_identity_file is not None else None,
            "model_identity_sha256": str(args.expected_model_sha256) if args.expected_model_sha256 is not None else None,
            "trust_remote_code": bool(args.trust_remote_code),
            "tiny_hidden_size": int(args.tiny_hidden_size),
        ...
        "head": {
            ...
            "end_to_end_contact_pooling": bool(args.end_to_end_contact_pooling),
        },
        "loss": {
            "dual_weight": float(args.dual_weight),
            "receptor_weight": float(args.receptor_weight),
            "contact_weight": float(args.contact_weight),
            "ranking_weight": float(args.ranking_weight),
            "residual_weight": float(args.residual_weight),
            "huber_delta": float(args.huber_delta),
            "ranking_minimum_delta": float(args.ranking_minimum_delta),
            "ranking_temperature": float(args.ranking_temperature),
        },
        "optimization": {
            "max_epochs": int(args.max_epochs),
            "batch_size": int(args.batch_size),
            "per_parent_batch": int(args.per_parent_batch),
        ...
        "runtime": {
            "device": str(getattr(args, "device", "unspecified")),
            "safe_stop_free_gb": float(args.safe_stop_free_gb),
            "checkpoint_min_free_gb": float(args.checkpoint_min_free_gb),
            "smoke_mode": bool(getattr(args, "smoke_mode", False)),
        },
        "protocol": {
            "inner_assignment": "sha256(PVRIG_V6_INNER|outer=<outer>|<parent>)_mod_5",
            "inner_fold_count": INNER_FOLD_COUNT,
            "epoch_selection": ["spearman", "parent_centered_spearman", "top20_recall", "negative_mae"],
            "final_epoch": "rounded_median_one_based_inner_selected_epochs",
        },
    }
    return payload


def binding_hash(payload: Mapping[str, Any]) -> str:
    return hashlib.sha256(json.dumps(dict(payload), sort_keys=True, separators=(",", ":")).encode()).hexdigest()


def assert_resume_binding(expected_hash: str, observed_hash: str) -> None:
    require(bool(expected_hash) and bool(observed_hash), "resume_binding_hash_missing")
    require(expected_hash == observed_hash, "resume_binding_hash_mismatch")
...
def validate_governance_amendment(path: Path) -> dict[str, Any]:
    ...
    require(int(frozen.get("bootstrap_repetitions", 0)) == BOOTSTRAP_REPETITIONS, "governance_bootstrap_repetitions_invalid")
    return payload


def validate_collector_matrix_binding(payload: Mapping[str, Any]) -> None:
    require(payload.get("collector_matrix") == COLLECTOR_MATRIX, "freeze_collector_matrix_mismatch")
    require(
        payload.get("collector_matrix_sha256") == COLLECTOR_MATRIX_SHA256,
        "freeze_collector_matrix_sha256_mismatch",
    )


def validate_formal_evidence_labels(payload: Mapping[str, Any]) -> None:
    require(
        payload.get("formal_evidence_labels") == FORMAL_EVIDENCE_LABELS,
        "freeze_formal_evidence_labels_mismatch",
    )


def validate_implementation_freeze(freeze_path: Path, root: Path, governance_path: Path) -> dict[str, str]:
    v1.require_regular(freeze_path, "implementation_freeze")
    payload = json.loads(freeze_path.read_text(encoding="utf-8"))
    require(payload.get("schema_version") == "pvrig_v6_residue_v1_5_implementation_freeze", "implementation_freeze_schema_invalid")
    ...
    validate_collector_matrix_binding(payload)
    validate_formal_evidence_labels(payload)
    require((root / GOVERNANCE_RELATIVE_PATH).resolve() == governance_path.resolve(), "freeze_governance_resolved_path_mismatch")
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    existing_terminal: Path | None = None
    ...
    if not args.smoke_mode:
        ...
        require(args.governance_amendment is not None, "governance_amendment_required")
        implementation_hashes = validate_implementation_freeze(
    ...
    split_manifest = "\n".join(sorted(
        f"{row.candidate_id}\t{row.sequence_sha256}\t{row.parent}\t{row.outer_fold}"
        for row in rows
    )) + "\n"
    external_hashes = {
        ...
        "contact_independent_validation_sha256": sha256_file(args.contact_validation) if args.contact_validation is not None else "smoke_not_applicable",
        "implementation_freeze_sha256": sha256_file(args.implementation_freeze) if args.implementation_freeze is not None else "smoke_not_applicable",
        "governance_amendment_sha256": sha256_file(args.governance_amendment) if args.governance_amendment is not None else "smoke_not_applicable",
        "split_manifest_sha256": hashlib.sha256(split_manifest.encode()).hexdigest(),
    }
    run_binding = result_affecting_binding(args, feature_names, external_hashes, args.outer_fold)
    run_binding.update({
        "frozen_external_sha256": dict(FROZEN_EXTERNAL_SHA256),
        "implementation_hashes": implementation_hashes,
        "observed_inner_folds": observed_inner,
        "row_count": len(rows),
        "parent_count": len({row.parent for row in rows}),
    })
    run_binding_hash = binding_hash(run_binding)
    preflight_path = args.output_dir / "PREFLIGHT.json"
    if preflight_path.is_file():
        assert_resume_binding(json.loads(preflight_path.read_text())["binding_hash"], run_binding_hash)
    else:
        atomic_json(preflight_path, {
            "status": "PASS_PREFLIGHT_TRAINING_NOT_COMPLETE",
            "binding_hash": run_binding_hash,
            "binding": run_binding,
    ...
    if existing_terminal is not None:
        terminal_payload = json.loads(existing_terminal.read_text(encoding="utf-8"))
        assert_resume_binding(terminal_payload.get("binding_hash", ""), run_binding_hash)
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--governance-amendment", type=Path)
