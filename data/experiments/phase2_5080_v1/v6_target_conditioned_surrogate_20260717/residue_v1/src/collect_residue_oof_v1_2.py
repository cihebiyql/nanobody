#!/usr/bin/env python3
"""Independently collect V1.2 outer-fold predictions and bootstrap by parent."""
...
SCHEMA_VERSION = "pvrig_v6_residue_v1_2_oof_collector"
CLAIM_BOUNDARY = (
...
def promotion_decision(rows: Sequence[Mapping[str, Any]], bootstrap: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "PROMOTE_RESIDUE_V1_2_OVER_M2" if all(gates.values()) else "DO_NOT_PROMOTE_RESIDUE_V1_2",
        "point_deltas": point,
        "gates": gates,
        "diagnostics": {
...
def validate_implementation_freeze(path: Path, root: Path) -> dict[str, str]:
    ...
    require(payload.get("schema_version") == "pvrig_v6_residue_v1_2_implementation_freeze", "collector_implementation_freeze_schema_invalid")
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    oof_path = args.output_dir / "residue_v1_2_nested_oof_predictions.tsv"
