#!/usr/bin/env python3
"""Independently collect V1.1 outer-fold predictions and bootstrap by parent."""
...
SCHEMA_VERSION = "pvrig_v6_residue_v1_1_oof_collector"
CLAIM_BOUNDARY = (
...
def parent_bootstrap(rows: Sequence[Mapping[str, Any]], *, replicates: int, seed: int) -> dict[str, Any]:
    ...
    samples: dict[str, list[float]] = {name: [] for name in ("spearman", "parent_centered_spearman", "top20_recall", "mae_improvement")}
    ...
    return {"replicates": replicates, "seed": seed, "parent_count": len(parents), "delta_intervals": summary}
...
def promotion_decision(rows: Sequence[Mapping[str, Any]], bootstrap: Mapping[str, Any]) -> dict[str, Any]:
    ...
    gates = {
        "point_spearman_positive": point["spearman"] > 0,
        "point_parent_centered_positive": point["parent_centered_spearman"] > 0,
        "point_top20_nonnegative": point["top20_recall"] >= 0,
        "point_mae_improvement_positive": point["mae_improvement"] > 0,
        "bootstrap_spearman_lower_positive": intervals["spearman"]["lower_95"] > 0,
        "bootstrap_parent_centered_lower_positive": intervals["parent_centered_spearman"]["lower_95"] > 0,
        "bootstrap_top20_lower_nonnegative": intervals["top20_recall"]["lower_95"] >= 0,
        "bootstrap_mae_improvement_lower_positive": intervals["mae_improvement"]["lower_95"] > 0,
    }
    return {
        "status": "PROMOTE_RESIDUE_V1_1_OVER_M2" if all(gates.values()) else "DO_NOT_PROMOTE_RESIDUE_V1_1",
        "point_deltas": point,
        "gates": gates,
    }
...
def validate_implementation_freeze(path: Path, root: Path) -> dict[str, str]:
    require(path.is_file() and not path.is_symlink(), "collector_implementation_freeze_missing")
    payload = json.loads(path.read_text(encoding="utf-8"))
    require(payload.get("schema_version") == "pvrig_v6_residue_v1_1_implementation_freeze", "collector_implementation_freeze_schema_invalid")
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    implementation_hashes_bound = validate_implementation_freeze(args.implementation_freeze, Path(__file__).parents[1])
    ...
    for run_dir in args.outer_run_dir:
        ...
        require(result.get("status") == "PASS_OUTER_FOLD_COMPLETE", f"outer_run_not_complete:{run_dir}")
        require(int(result.get("outer_evaluation_count", 0)) == 1, f"outer_evaluation_count_invalid:{run_dir}")
        require(seal.get("status") == "SEALED_COMPLETE_ONE_EVALUATION", f"outer_evaluation_seal_invalid:{run_dir}")
    ...
    oof_path = args.output_dir / "residue_v1_1_nested_oof_predictions.tsv"
