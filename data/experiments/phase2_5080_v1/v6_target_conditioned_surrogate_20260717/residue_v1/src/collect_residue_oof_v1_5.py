#!/usr/bin/env python3
"""Independently collect V1.5 outer-fold predictions and bootstrap by parent."""
...
SCHEMA_VERSION = "pvrig_v6_residue_v1_5_oof_collector"
TRAINER_SCHEMA_VERSION = "pvrig_v6_nested_residue_surrogate_v1_5"
...
def validate_bootstrap_matrix(replicates: int, seed: int) -> None:
    require(replicates == BOOTSTRAP_REPETITIONS, "bootstrap_repetitions_not_frozen")
    require(seed == BOOTSTRAP_SEED, "bootstrap_seed_not_frozen")


def collector_matrix_binding() -> dict[str, Any]:
    return {
        "collector_matrix": dict(COLLECTOR_MATRIX),
        "collector_matrix_sha256": COLLECTOR_MATRIX_SHA256,
    }
...
def parent_bootstrap(rows: Sequence[Mapping[str, Any]], *, replicates: int, seed: int) -> dict[str, Any]:
    ...
    validate_bootstrap_matrix(replicates, seed)
...
def promotion_decision(rows: Sequence[Mapping[str, Any]], bootstrap: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": POSITIVE_PROMOTION_STATUS if all(gates.values()) else NEGATIVE_PROMOTION_STATUS,
        "point_deltas": point,
        "gates": gates,
        "diagnostics": {
...
def validate_implementation_freeze(path: Path, root: Path, governance_path: Path) -> dict[str, str]:
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    require(payload.get("schema_version") == "pvrig_v6_residue_v1_5_implementation_freeze", "collector_implementation_freeze_schema_invalid")
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    validate_bootstrap_matrix(args.bootstrap_replicates, args.bootstrap_seed)
    ...
    oof_path = args.output_dir / "residue_v1_5_nested_oof_predictions.tsv"
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--bootstrap-replicates", type=int, default=BOOTSTRAP_REPETITIONS)
    value.add_argument("--bootstrap-seed", type=int, default=BOOTSTRAP_SEED)
    return value
