...
LEGACY_TEST_LAUNCHER="$UPSTREAM_ROOT/launchers/run_phase1_core_preflight_tests_v1_2.sh"
...
NEW_TEST_LAUNCHER="$ROOT/launchers/run_tests_v1_3_1.sh"
RECEIPT_BUILDER="$ROOT/src/build_v220_v1_3_1_preflight_receipt.py"
...
EXPECTED_LEGACY_TEST_LAUNCHER_SHA="592d424c103872f2dd15c56a1c6c57bfec2cac6523e6b2a0f8f4c52aaa453325"
...
root=Path(sys.argv[1]).resolve(); freeze_path=Path(sys.argv[2]).resolve(); expected=sys.argv[3]
raw=freeze_path.read_bytes(); assert hashlib.sha256(raw).hexdigest()==expected
freeze=json.loads(raw)
assert freeze['status']=='FROZEN_V1_3_1_IMPLEMENTATION_PENDING_INDEPENDENT_REVIEW_AND_NODE1_PREFLIGHT'
...
PYTHON_BIN="$PYTHON_BIN" "$LEGACY_TEST_LAUNCHER" > "$RUNTIME_ROOT/logs/LEGACY_102_TESTS.log" 2>&1
PYTHON_BIN="$PYTHON_BIN" "$NEW_TEST_LAUNCHER" > "$RUNTIME_ROOT/logs/V1_3_1_TESTS.log" 2>&1
...
"$PYTHON_BIN" "$RECEIPT_BUILDER" \
  ...
  --v1-3-1-test-log "$RUNTIME_ROOT/logs/V1_3_1_TESTS.log" \
  --expected-new-tests "$EXPECTED_NEW_TESTS" \
  --output-receipt "$RUNTIME_ROOT/NODE1_V1_3_1_PREFLIGHT_RECEIPT.json"
