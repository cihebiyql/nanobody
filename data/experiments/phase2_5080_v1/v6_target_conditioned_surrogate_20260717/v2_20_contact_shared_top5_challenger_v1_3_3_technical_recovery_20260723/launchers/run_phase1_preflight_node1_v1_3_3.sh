...
LEGACY_TEST_LAUNCHER="$ROOT/launchers/run_legacy_102_tests_python311_v1_3_3.sh"
...
NEW_TEST_LAUNCHER="$ROOT/launchers/run_tests_v1_3_3.sh"
RECEIPT_BUILDER="$ROOT/src/build_v220_v1_3_3_preflight_receipt.py"
...
assert freeze['status']=='FROZEN_V1_3_3_IMPLEMENTATION_PENDING_INDEPENDENT_REVIEW_AND_NODE1_PREFLIGHT'
...
PYTHON_BIN="$PYTHON_BIN" "$NEW_TEST_LAUNCHER" > "$RUNTIME_ROOT/logs/V1_3_3_TESTS.log" 2>&1
...
"$PYTHON_BIN" "$RECEIPT_BUILDER" \
  ...
  --v1-3-3-test-log "$RUNTIME_ROOT/logs/V1_3_3_TESTS.log" \
  --expected-new-tests "$EXPECTED_NEW_TESTS" \
  --output-receipt "$RUNTIME_ROOT/NODE1_V1_3_3_PREFLIGHT_RECEIPT.json"
