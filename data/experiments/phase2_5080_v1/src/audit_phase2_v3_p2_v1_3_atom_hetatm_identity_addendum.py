#!/usr/bin/env python3
"""Audit ATOM/HETATM-heavy identity drift without producing a selector.

The audit covers all 64 targeted exact-reuse runs and the four frozen V1.3
boundary runs retrieved read-only from Node1.  It tests terminal-OXT ATOM
normalization and closes the HETATM loophole on both protein chains.
"""
...
DEFAULT_AUDIT = EXP_DIR / "audits/phase2_v3_p2_v1_3_atom_hetatm_identity_addendum_audit.json"
DEFAULT_REPORT = EXP_DIR / "reports/PVRIG_V3_P2_DOCKING_GOLD_V1_3_ATOM_HETATM_IDENTITY_ADDENDUM_ZH.md"
AUDIT_STATUS_PASS = "PASS_V1_3_ATOM_OXT_AND_HETATM_ZERO_EVIDENCE"
AUDIT_STATUS_FAIL = "FAIL_V1_3_ATOM_OR_HETATM_IDENTITY_EVIDENCE"
CLAIM_BOUNDARY = (
    "read-only computational ATOM/HETATM identity addendum; no selector, geometry score, "
    "training label, formal Gold, or coordinate modification"
)
...
def hetatm_heavy_identity_payload(
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        if not line.startswith("HETATM") or len(line) < 54 or line[21:22] != chain:
            continue
        try:
            residue_number = int(line[22:26])
        except ValueError as error:
            raise IdentityAuditError(
                f"Invalid HETATM residue number in {path}:{line_number}"
            ) from error
        resname = line[17:20].strip()
        atom_name = line[12:16].strip()
        insertion = line[26:27].strip()
        ...
        element = line[76:78].strip().upper() if len(line) >= 78 else ""
        if not resname or not atom_name:
            raise IdentityAuditError(f"Missing HETATM identity in {path}:{line_number}")
        is_heavy = (
            element not in {"H", "D"}
            if element
        ...
        if identity in identities:
            raise IdentityAuditError(
                f"Duplicate HETATM-heavy identity in {path}:{line_number}: {identity}"
            )
