#!/usr/bin/env python3
"""Build deterministic, content-bound trust anchors for the V4-D/V4-F V2 watchers."""
...
def surrogate_files(exp: Path) -> dict[str, Path]:
    ...
    roles = {
        "v2_watcher": exp / "src/monitor_phase2_v4_d_surrogate_training_v2.sh",
        "v2_helper": exp / "src/phase2_v4_d_surrogate_watcher_helper_v2.py",
...
def v4f_files(exp: Path) -> dict[str, Path]:
    embedding = exp / "prepared/pvrig_teacher_formal_v1_candidates/model_inputs"
    roles = {
        "v2_watcher": exp / "src/monitor_phase2_v4_f_prediction_freeze_v2.sh",
        ...
        "embedding_summary": embedding / "meanpool_embeddings/embedding_summary_v3.json",
        "embedding_sequence_manifest": embedding / "sequence_manifest_v3.csv",
        "surrogate_v2_trust_anchor": exp / "audits/phase2_v4_d_surrogate_training_v2_implementation_trust_anchor.json",
    }
    for index in range(7):
        roles[f"embedding_shard_{index:05d}"] = (
            embedding / f"meanpool_embeddings/shards/shard_{index:05d}.pt"
...
def build_payload(kind: str, exp: Path) -> dict[str, object]:
    if kind == "surrogate":
        schema = "phase2_v4_d_surrogate_implementation_trust_anchor_v2"
        status = "FROZEN_BEFORE_OPEN258_TEACHER_AND_SURROGATE_TRAINING"
        anchor_kind = "v4d_surrogate_training"
        files = surrogate_files(exp)
    else:
        schema = "phase2_v4_f_prediction_implementation_trust_anchor_v2"
...
def main() -> int:
    ...
    if not args.test_only_noncanonical_root:
        expected_output = CANONICAL_EXP / (
            "audits/phase2_v4_d_surrogate_training_v2_implementation_trust_anchor.json"
            if args.kind == "surrogate"
            else "audits/phase2_v4_f_prediction_freeze_v2_implementation_trust_anchor.json"
        )
