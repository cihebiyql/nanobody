...
CANONICAL_WATCHER=$CANONICAL_EXP_DIR/src/monitor_phase2_v4_f_prediction_freeze_v2.sh
EXP_DIR=${PVRIG_EXP_DIR:-$CANONICAL_EXP_DIR}
TEST_ONLY_UNFROZEN=${V4F_TEST_ONLY_ALLOW_UNFROZEN_INPUTS:-0}
V2_TEST_ONLY=${PVRIG_V4F_WATCHER_V2_TEST_ONLY:-0}
...
SURROGATE_STATUS=${V4D_SURROGATE_STATUS:-$EXP_DIR/status/pvrig_v4_d_surrogate_training_v2/status.json}
SURROGATE_COMPLETION_RECEIPT=${V4D_SURROGATE_V2_COMPLETION_RECEIPT:-$EXP_DIR/status/pvrig_v4_d_surrogate_training_v2/surrogate_v2_completion_receipt.json}
...
STATUS_DIR=${V4F_PREDICTION_STATUS_DIR:-$EXP_DIR/status/pvrig_v4_f_prediction_freeze_v2}
LOG_DIR=${V4F_PREDICTION_LOG_DIR:-$EXP_DIR/logs/pvrig_v4_f_prediction_freeze_v2}
EXPECTED_COUNT=${V4F_EXPECTED_COUNT:-96}
TRUST_ANCHOR=${V4F_V2_TRUST_ANCHOR:-$EXP_DIR/audits/phase2_v4_f_prediction_freeze_v2_implementation_trust_anchor.json}
EXPECTED_TRUST_ANCHOR_SHA=${V4F_V2_EXPECTED_TRUST_ANCHOR_SHA:-}
EXPECTED_SURROGATE_TRUST_ANCHOR_SHA=${V4D_V2_EXPECTED_TRUST_ANCHOR_SHA:-}
...
if [[ "$V2_TEST_ONLY" != 1 ]]; then
  ...
  require_canonical SURROGATE_STATUS "$SURROGATE_STATUS" "$CANONICAL_EXP_DIR/status/pvrig_v4_d_surrogate_training_v2/status.json"
  require_canonical SURROGATE_COMPLETION_RECEIPT "$SURROGATE_COMPLETION_RECEIPT" "$CANONICAL_EXP_DIR/status/pvrig_v4_d_surrogate_training_v2/surrogate_v2_completion_receipt.json"
  ...
  require_canonical STATUS_DIR "$STATUS_DIR" "$CANONICAL_EXP_DIR/status/pvrig_v4_f_prediction_freeze_v2"
  require_canonical LOG_DIR "$LOG_DIR" "$CANONICAL_EXP_DIR/logs/pvrig_v4_f_prediction_freeze_v2"
  require_canonical TRUST_ANCHOR "$TRUST_ANCHOR" "$CANONICAL_EXP_DIR/audits/phase2_v4_f_prediction_freeze_v2_implementation_trust_anchor.json"
  [[ "$EXPECTED_TRUST_ANCHOR_SHA" =~ ^[0-9a-f]{64}$ ]] || { echo "missing V4-F V2 trust-anchor SHA" >&2; exit 2; }
  [[ "$EXPECTED_SURROGATE_TRUST_ANCHOR_SHA" =~ ^[0-9a-f]{64}$ ]] || { echo "missing surrogate V2 trust-anchor SHA" >&2; exit 2; }
fi
...
trust_gate() {
  ...
  [[ -n "$EXPECTED_TRUST_ANCHOR_SHA" ]] || {
    [[ "$V2_TEST_ONLY" == 1 ]] && return 0
    return 2
  }
  temporary=$(mktemp "$STATUS_DIR/.trust-${stage}.XXXXXX")
  if ! "$PYTHON" - "$TRUST_ANCHOR" "$EXPECTED_TRUST_ANCHOR_SHA" >"$temporary" <<'PY'
import hashlib, json, stat, sys
from pathlib import Path
anchor=Path(sys.argv[1]); expected=sys.argv[2]
metadata=anchor.lstat()
assert stat.S_ISREG(metadata.st_mode) and not stat.S_ISLNK(metadata.st_mode) and metadata.st_size>0
assert hashlib.sha256(anchor.read_bytes()).hexdigest()==expected
payload=json.loads(anchor.read_text())
assert payload["schema_version"]=="phase2_v4_f_prediction_implementation_trust_anchor_v2"
assert payload["status"]=="FROZEN_BEFORE_V4F_PREDICTION_FREEZE"
assert payload["anchor_kind"]=="v4f_prediction_freeze"
observed={}
for role, item in sorted(payload["files"].items()):
    path=Path(item["path"]); meta=path.lstat()
    assert path.is_absolute() and str(path)==str(path.resolve()), role
    assert stat.S_ISREG(meta.st_mode) and not stat.S_ISLNK(meta.st_mode), role
    assert meta.st_size==item["size"] and meta.st_size>0, role
    digest=hashlib.sha256(path.read_bytes()).hexdigest()
    assert digest==item["sha256"], role
    observed[role]={"path":str(path),"size":meta.st_size,"sha256":digest}
print(json.dumps({
 "schema_version":"phase2_v4_f_prediction_freeze_watcher_v2",
 "status":"PASS_V4F_V2_IMPLEMENTATION_TRUST_ANCHOR",
...
verify_surrogate_completion_receipt() {
  "$PYTHON" - "$SURROGATE_COMPLETION_RECEIPT" "$EXPECTED_SURROGATE_TRUST_ANCHOR_SHA" <<'PY'
import hashlib, json, stat, sys
from pathlib import Path
path=Path(sys.argv[1]); expected=sys.argv[2]
meta=path.lstat()
assert stat.S_ISREG(meta.st_mode) and not stat.S_ISLNK(meta.st_mode) and meta.st_size>0
payload=json.loads(path.read_text())
assert payload["schema_version"]=="phase2_v4_d_surrogate_v2_completion_receipt_v1"
assert payload["status"]=="PASS_V4_D_SURROGATE_V2_COMPLETE_TEST32_SEALED"
assert payload["implementation_trust_anchor_sha256"]==expected
assert payload["prospective_test_labels_read"] is False
assert payload["prospective_test_label_paths_accepted"]==0
stages=payload["stage_receipts"]
assert set(stages)=={"base_stage","embedding_stage","contact_stage"}
for name, item in stages.items():
    stage=Path(item["path"]); stage_meta=stage.lstat()
    assert stat.S_ISREG(stage_meta.st_mode) and not stat.S_ISLNK(stage_meta.st_mode), name
    assert hashlib.sha256(stage.read_bytes()).hexdigest()==item["sha256"], name
print(json.dumps({
 "status":"PASS_VERIFIED_SURROGATE_V2_COMPLETION_RECEIPT",
...
trust_gate startup || { echo "V4-F V2 implementation trust gate failed" >&2; exit 2; }
write_status WAITING_V4_D_SURROGATES "V2 implementation trust verified; waiting for verified surrogate V2 completion receipt; V4-F labels remain sealed"
while true; do
  if ! trust_gate poll; then
    write_status BLOCKED_IMPLEMENTATION_TRUST "V4-F V2 implementation trust drift"
    exit 2
  fi
  ...
  if [[ "$state" == COMPLETE_V4_D_SURROGATE_TRAINING_TEST32_SEALED ]]; then
    if ! verify_surrogate_completion_receipt >"$STATUS_DIR/surrogate_v2_completion_verification.json" 2>&1; then
      write_status BLOCKED_INVALID_SURROGATE_V2_RECEIPT "surrogate status complete but V2 completion receipt failed verification"
      exit 2
    fi
    trust_gate before_freeze || {
      write_status BLOCKED_IMPLEMENTATION_TRUST "V4-F V2 implementation drift before freeze"
      exit 2
    }
    ...
    trust_gate after_freeze || {
      write_status BLOCKED_IMPLEMENTATION_TRUST "V4-F V2 implementation drift during freeze"
      exit 2
    }
    if verify_existing >"$STATUS_DIR/verification.tmp" 2>&1; then
      receipt_sha=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["receipt_sha256"])' "$STATUS_DIR/verification.tmp")
