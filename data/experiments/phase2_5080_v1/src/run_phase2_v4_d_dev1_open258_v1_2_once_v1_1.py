#!/usr/bin/env python3
"""V1.1: consume one V1.2 launch authorization exactly once, then exec the inner launcher.
...
SELF_BASENAME = "run_phase2_v4_d_dev1_open258_v1_2_once_v1_1.py"
...
LINUX_MFD_ALLOW_SEALING = 2
LINUX_F_ADD_SEALS = 1033
LINUX_F_GET_SEALS = 1034
LINUX_F_SEAL_SEAL = 1
LINUX_F_SEAL_SHRINK = 2
LINUX_F_SEAL_GROW = 4
LINUX_F_SEAL_WRITE = 8
...
def linux_int_constant(namespace: Any, name: str, fallback: int) -> int:
    value = getattr(namespace, name, fallback)
    return value if type(value) is int else fallback


def portable_memfd_create(name: str, flags: int) -> int:
    ...
    try:
        ...
        raise OneShotLaunchError("libc_memfd_create_unavailable") from exc
    function.argtypes = [ctypes.c_char_p, ctypes.c_uint]
    function.restype = ctypes.c_int
    ctypes.set_errno(0)
    fd = int(function(name.encode("ascii"), flags))
    if fd < 0:
...
def create_sealed_exec_snapshot(raw: bytes) -> tuple[int, str]:
    ...
    allow_sealing = linux_int_constant(os, "MFD_ALLOW_SEALING", LINUX_MFD_ALLOW_SEALING)
    f_add_seals = linux_int_constant(fcntl, "F_ADD_SEALS", LINUX_F_ADD_SEALS)
    f_get_seals = linux_int_constant(fcntl, "F_GET_SEALS", LINUX_F_GET_SEALS)
    f_seal_seal = linux_int_constant(fcntl, "F_SEAL_SEAL", LINUX_F_SEAL_SEAL)
    f_seal_shrink = linux_int_constant(fcntl, "F_SEAL_SHRINK", LINUX_F_SEAL_SHRINK)
    f_seal_grow = linux_int_constant(fcntl, "F_SEAL_GROW", LINUX_F_SEAL_GROW)
    f_seal_write = linux_int_constant(fcntl, "F_SEAL_WRITE", LINUX_F_SEAL_WRITE)
    try:
        fd = portable_memfd_create("pvrig_v4d_dev1_v1_2_inner_v1_1", allow_sealing)
    except OSError as exc:
    ...
    try:
        ...
        applied = fcntl.fcntl(fd, f_get_seals)
