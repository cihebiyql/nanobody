...
SCHEMA_VERSION = "phase2_v4_h_partial_success_snapshot_v1"
...
def load_scoring_function(adaptive_script: Path, scorer_path: Path) -> Callable[[dict[str, Any], str], float]:
    spec = importlib.util.spec_from_file_location("v4h_partial_snapshot_adaptive", adaptive_script)
    ...
    *,
    ...
    for candidate_id in sorted(candidate_by_id):
        ...
        snapshot_rows.append({
            ...
            "partial_incomplete_reason": "MISSING_SUCCESS_" + "+".join(missing) if missing else "",
            "claim_boundary": CLAIM_BOUNDARY,
        })
    output_dir.mkdir(parents=True)
    provenance_path = output_dir / "successful_jobs_snapshot_v1.tsv"
    teacher_path = output_dir / "partial_candidate_teacher_snapshot_v1.tsv"
    ...
    receipt_path = output_dir / "partial_success_snapshot_v1.receipt.json"
