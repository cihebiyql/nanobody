...
DEFAULT_IMPLEMENTATION_FREEZE = (
    SCRIPT_DIR.parent
    / "audits/phase2_v4_d_cross_campaign_control_bridge_implementation_freeze.json"
)
DEFAULT_IMPLEMENTATION_FREEZE_SHA256_RECORD = DEFAULT_IMPLEMENTATION_FREEZE.with_suffix(
    ".sha256"
)
EXPECTED_PREREGISTRATION_SHA256 = (
    "e9f151a713e5aaeb691cfe10de8f88015cb62d171e3f1e88f413be07b073b97e"
)
PREREGISTRATION_SCHEMA_VERSION = (
    "phase2_v4_d_cross_campaign_control_bridge_preregistration_v3_pre_extraction_implementation_freeze"
)
...
def load_preregistration(
    ...
    *,
    ...
    require(
        ...
        == "audits/phase2_v4_d_cross_campaign_control_bridge_implementation_freeze.sha256",
...
def resolve_receptor(root: Path, row: Mapping[str, str]) -> Path:
...
def load_campaign(
    ...
    *,
    ...
    if production_spec is not None:
        expected_root = Path(str(production_spec.get("root", ""))).resolve()
    ...
    for row in control_rows:
        ...
        require(
            str(row["protocol_core_sha256"]).lower() == core,
            f"manifest_protocol_core_mismatch:{label}:{row['job_id']}",
        )
        receptor_path = resolve_receptor(resolved_root, row)
        if receptor_path not in receptors:
            receptors[receptor_path] = snapshot_file(receptor_path, f"{label}_receptor")
            validate_receptor_snapshot(receptors[receptor_path], f"{label}:{receptor_path.name}")
        controls[key] = row
...
def result_path(campaign: Campaign, row: Mapping[str, str]) -> Path:
    ...
    results_root = require_real_directory(
        campaign.root / RESULTS_RELATIVE_PATH, f"{campaign.label}_results_root"
    )
    job_root = require_real_directory(
        results_root / job_id, f"{campaign.label}_control_job_directory:{job_id}"
    )
    path = lexical_absolute(job_root / "job_result.json")
    require(path.is_relative_to(results_root), f"result_path_outside_root:{campaign.label}:{job_id}")
    return path
...
def same_control_identity(
    ...
    left_receptor = left.receptors[resolve_receptor(left.root, left_row)]
    right_receptor = right.receptors[resolve_receptor(right.root, right_row)]
    require(
...
def validate_campaign_semantics(left: Campaign, right: Campaign) -> None:
    require(
...
def expected_input_snapshots(
    ...
    for campaign in (left, right):
        for row in campaign.controls_by_key.values():
            path = result_path(campaign, row)
            snapshots[path] = snapshot_file(path, f"{campaign.label}_control_result")
    return snapshots
...
def verify_receipt(
    ...
    *,
    ...
    require(receipt.get("schema_version") == SCHEMA_VERSION, "receipt_schema_invalid")
    ...
    validate_campaign_semantics(left, right)
    snapshots = expected_input_snapshots(
        left, right, preregistration_snapshot, implementation_freeze
    )
    ...
    replayed_job_rows = replay_job_rows(left, right, snapshots)
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "PASS_V4_D_CROSS_CAMPAIGN_CONTROL_BRIDGE_RECEIPT_VERIFIED",
...
def build_bridge(
    ...
    *,
    ...
    with publication_lock(resolved_out):
        ...
        job_rows = replay_job_rows(left, right, snapshots)
        ...
        try:
            ...
            audit = {
                ...
                "pairing": {
                    ...
                    "semantic_protocol_validation": "canonical_behavior_subset_sha256_equality",
                },
                "utility": {
