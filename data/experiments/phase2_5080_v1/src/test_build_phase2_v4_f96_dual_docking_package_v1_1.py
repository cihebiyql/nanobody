...
builder=load("build_phase2_v4_f96_dual_docking_package_v1_1.py")
node1=load("prepare_phase2_v4_f96_docking_inputs_node1_v1_1.py")
node23=load("stage_run_phase2_v4_f96_dual_docking_node23_v1_1.py")


class V4F96DualDockingPackageV11Tests(unittest.TestCase):
 ...
 def test_package_builds_and_hash_closes(self):
  with tempfile.TemporaryDirectory() as raw:
   root=Path(raw);out=root/"package";freeze=root/"freeze.json";result=builder.build(out,freeze);self.assertEqual(result["status"],"PASS_V4_F96_DUAL_DOCKING_V1_1_PACKAGE_HASH_CLOSED");self.assertEqual(builder.validate_package(out)["package_receipt_sha256"],result["package_receipt_sha256"])
 ...
 def test_version_isolated_paths_and_status_helpers_execute(self):
  self.assertIn("pvrig_v4_f96_docking_input_release_v1_1_20260717",builder.NODE1.read_text())
  self.assertIn("pvrig_v4_f96_dual_redocking_v1_1_20260717",builder.NODE23.read_text())
  for template,remote_root in ((builder.NODE1_TEMPLATE,"/data1/qlyu/projects/pvrig_v4_f96_docking_input_release_v1_1_20260717"),(builder.NODE23_TEMPLATE,"/data/qlyu/projects/pvrig_v4_f96_dual_redocking_v1_1_20260717")):
   raw=template.read_text();end=raw.index("\nverify_static(){")
   with tempfile.TemporaryDirectory() as tmp:
    snippet=raw[:end].replace(remote_root,tmp)+"\nstatus TEST_STATUS TEST_REASON\n"
    completed=subprocess.run(["/bin/bash","--noprofile","--norc"],input=snippet,text=True,capture_output=True,env={"HOME":os.environ.get("HOME","/tmp"),"PATH":"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"})
    self.assertEqual(completed.returncode,0,completed.stderr)
    status=json.loads((Path(tmp)/"status/waiter_status.json").read_text());self.assertEqual(status["status"],"TEST_STATUS")

 def test_waiters_do_not_assign_to_readonly_status_root(self):
  for template in (builder.NODE1_TEMPLATE,builder.NODE23_TEMPLATE):
   text=template.read_text();self.assertIn('STATUS_ROOT="$V4F96_ROOT" python3',text);self.assertNotIn('V4F96_ROOT="$V4F96_ROOT" python3',text);self.assertIn("unset BASH_ENV ENV PYTHONPATH PYTHONHOME CDPATH",text)
