...
RECOVERY = load_module(
    "recover_phase2_v4_h_qc96_h2_h4_v1_1",
    HERE / "recover_phase2_v4_h_qc96_h2_h4_v1_1.py",
)
FROZEN = load_module(
...
class FrozenSelectionParityTests(unittest.TestCase):
    ...
    def test_real1440_selection_parity_evidence_binds_current_runner(self):
        path = (
            HERE.parent / "audits" /
            "phase2_v4_h_qc96_h2_h4_recovery_v1_1_real1440_selection_parity.json"
        )
        evidence = json.loads(path.read_text())
        self.assertEqual(
            evidence["status"],
            "PASS_REAL1440_FROZEN_SOURCE_RUNNER_VS_RECOVERY_V1_1_ROW_PARITY",
        )
        self.assertEqual(evidence["actual_frozen_config_seed"], RECOVERY.SELECTION_SEED)
        self.assertEqual(evidence["input_candidate_rows"], 1440)
        self.assertEqual(evidence["selected_rows"], 96)
        self.assertEqual(evidence["row_mismatch_count"], 0)
        self.assertTrue(evidence["selected_rows_exact_equal"])
        self.assertEqual(
            evidence["selected_ids_ordered_sha256"],
            "91e86a1ab0aba2dc37dfd2e3b4f30479fa4653690f9b005dd28c94a79f9aadf4",
        )
        self.assertEqual(
            evidence["source_hashes"]["recovery_v1_1_runner"],
            hashlib.sha256((HERE / "recover_phase2_v4_h_qc96_h2_h4_v1_1.py").read_bytes()).hexdigest(),
        )
        self.assertEqual(
            evidence["source_hashes"]["frozen_source_runner"],
            hashlib.sha256((HERE / "run_phase2_v4_h_qc96_qc_node1.py").read_bytes()).hexdigest(),
        )
