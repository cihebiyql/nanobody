...
SCRIPT = Path(__file__).with_name("reconcile_node1_ssd_deepqc_three_state_tnp_igfold100_v3_2.py")
SPEC = importlib.util.spec_from_file_location("reconcile_v3_2", SCRIPT)
...
class ReconciliationTests(unittest.TestCase):
    def test_mixed_tnp_manifest_union_fields_accepts_valid_or_na_first(self):
        valid = {"candidate_id": "a", "status": "VALID_TNP", "sha256": "1"}
        na = {"candidate_id": "b", "status": "TNP_NUMBERING_HARD_FAIL_NA", "failure_log": "b.log"}
        valid_first = MODULE.tsv_bytes([valid, na]).decode().splitlines()
        na_first = MODULE.tsv_bytes([na, valid]).decode().splitlines()
        self.assertEqual(valid_first[0], na_first[0])
        self.assertEqual(valid_first[0].split("\t"), sorted({*valid, *na}))
        self.assertEqual(len(valid_first), 3)
        self.assertEqual(len(na_first), 3)

    def test_mixed_tnp_manifest_byte_identical_replay_ignores_dict_key_order(self):
        first = [
            {"candidate_id": "a", "status": "VALID_TNP", "sha256": "1"},
            {"candidate_id": "b", "status": "TNP_NUMBERING_HARD_FAIL_NA", "failure_log": "b.log"},
        ]
        replay = [
            {"sha256": "1", "status": "VALID_TNP", "candidate_id": "a"},
            {"failure_log": "b.log", "candidate_id": "b", "status": "TNP_NUMBERING_HARD_FAIL_NA"},
        ]
        self.assertEqual(MODULE.tsv_bytes(first), MODULE.tsv_bytes(replay))
