...
SCHEMA_VERSION = "phase2_v4_d_surrogate_training_watcher_v3"
TRUST_ANCHOR_SCHEMA_VERSION = "phase2_v4_d_surrogate_implementation_trust_anchor_v3"
TRUST_ANCHOR_KIND = "v4d_surrogate_training"
...
EXPECTED_OPEN_TEACHER_BUILDER_SHA256 = (
...
REQUIRED_TRUST_ROLES = frozenset(
    {
        "v3_watcher",
        "v3_helper",
        "base_trainer",
        ...
        "split_manifest",
        "contact_schema",
        "contact_schema_receipt",
        "contact_features",
        "contact_feature_audit",
        "contact_feature_receipt",
        "contact_feature_verification",
        "embedding_manifest",
        "embedding_summary",
...
def required_regular_file(path: Path, label: str) -> os.stat_result:
    try:
        metadata = path.lstat()
    except OSError as exc:
        raise WatcherError(f"missing:{label}:{path}") from exc
    require(not stat.S_ISLNK(metadata.st_mode), f"symlink_forbidden:{label}:{path}")
    require(stat.S_ISREG(metadata.st_mode), f"not_regular:{label}:{path}")
    require(metadata.st_size > 0, f"empty:{label}:{path}")
    return metadata


def validate_trust_anchor(
    path: Path, expected_sha256: str, expected_kind: str = TRUST_ANCHOR_KIND
) -> dict[str, Any]:
    """Verify the immutable implementation/data trust root and every bound file."""

    metadata = required_regular_file(path, "trust_anchor")
    require(
        len(expected_sha256) == 64
        and all(character in "0123456789abcdef" for character in expected_sha256),
        "trust_anchor_expected_sha_invalid",
    )
    observed_anchor_sha = sha256_file(path)
    require(
        observed_anchor_sha == expected_sha256,
        f"trust_anchor_hash_mismatch:{observed_anchor_sha}:{expected_sha256}",
    )
    payload = load_json(path, "trust_anchor")
    require(
        payload.get("schema_version") == TRUST_ANCHOR_SCHEMA_VERSION,
        "trust_anchor_schema_invalid",
    )
    require(
        payload.get("status")
        == "FROZEN_BEFORE_OPEN258_TEACHER_AND_SURROGATE_TRAINING",
        "trust_anchor_status_invalid",
    )
    require(payload.get("anchor_kind") == expected_kind, "trust_anchor_kind_invalid")
    files = payload.get("files")
    require(isinstance(files, dict) and files, "trust_anchor_files_missing")
    require(set(files) == REQUIRED_TRUST_ROLES, "trust_anchor_role_set_mismatch")
    observed_files: dict[str, dict[str, Any]] = {}
    resolved_paths: set[Path] = set()
    for role, entry in sorted(files.items()):
        require(isinstance(entry, dict), f"trust_anchor_entry_invalid:{role}")
        raw_path = entry.get("path")
        require(isinstance(raw_path, str) and raw_path, f"trust_anchor_path_missing:{role}")
        bound = Path(raw_path)
        require(bound.is_absolute(), f"trust_anchor_path_not_absolute:{role}")
        require(str(bound) == str(bound.resolve()), f"trust_anchor_path_not_canonical:{role}")
        require(bound not in resolved_paths, f"trust_anchor_duplicate_path:{bound}")
        resolved_paths.add(bound)
        bound_metadata = required_regular_file(bound, f"trust_file:{role}")
        expected_size = entry.get("size")
        expected_hash = entry.get("sha256")
        require(
            isinstance(expected_size, int) and expected_size > 0,
            f"trust_anchor_size_invalid:{role}",
        )
        require(
            isinstance(expected_hash, str)
            and len(expected_hash) == 64
            and all(character in "0123456789abcdef" for character in expected_hash),
            f"trust_anchor_file_sha_invalid:{role}",
        )
        require(bound_metadata.st_size == expected_size, f"trust_anchor_size_mismatch:{role}")
        observed_hash = sha256_file(bound)
        require(observed_hash == expected_hash, f"trust_anchor_file_hash_mismatch:{role}")
        observed_files[role] = {
            "path": str(bound),
            "size": bound_metadata.st_size,
            "sha256": observed_hash,
        }
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "PASS_V3_IMPLEMENTATION_TRUST_ANCHOR",
        "anchor_kind": expected_kind,
        "anchor_path": str(path.resolve()),
        "anchor_size": metadata.st_size,
        "anchor_sha256": observed_anchor_sha,
        "file_count": len(observed_files),
        "files": observed_files,
        "file_closure_sha256": sha256_json(observed_files),
    }
...
def validate_open_teacher(
    ...
    require(
        receipt.get("schema_version") == "pvrig_v4_d_open_teacher_postprocess_receipt_v2",
        "postprocess_receipt_schema_invalid",
    )
    require(
    ...
    require(
        receipt.get("full_aggregate_streamed_only_for_open_row_closure") is True,
        "receipt_full_aggregate_boundary_invalid",
    )
    require(
        receipt.get("builder_sha256") == EXPECTED_OPEN_TEACHER_BUILDER_SHA256,
        "receipt_builder_hash_mismatch",
    )
    require(
        receipt.get("job_manifest_sha256") == EXPECTED_JOB_MANIFEST_SHA256,
        "receipt_job_manifest_hash_mismatch",
    )
    audit_inputs = audit.get("inputs") or {}
    require(
        audit_inputs.get("job_manifest_sha256") == receipt.get("job_manifest_sha256"),
        "audit_receipt_job_manifest_mismatch",
    )
    require(
        audit_inputs.get("job_results_sha256_for_evaluator_binding_only")
        == receipt.get("job_results_sha256"),
        "audit_receipt_job_results_mismatch",
    )
    require(
        audit_inputs.get("pose_scores_sha256_for_evaluator_binding_only")
        == receipt.get("pose_scores_sha256"),
        "audit_receipt_pose_scores_mismatch",
    )
    required_regular_file(evaluator_path, "open_teacher_evaluator")
    evaluator_sha = sha256_file(evaluator_path)
    require(
        evaluator_sha == receipt.get("evaluator_sha256"),
        "receipt_evaluator_hash_mismatch",
    )
    evaluator = load_json(evaluator_path, "open_teacher_evaluator")
    require(
        evaluator.get("status") == "PASS" and evaluator.get("unlockable") is True,
        "evaluator_not_releasable",
    )
    require(
        evaluator.get("evidence_mode") == "production_pose_backed",
        "evaluator_not_production_pose_backed",
    )
    require(
        int(evaluator.get("job_count", -1)) == 2022,
        "evaluator_job_count_invalid",
    )
    for field in ("job_manifest_sha256", "job_results_sha256", "pose_scores_sha256"):
        require(
            evaluator.get(field) == receipt.get(field),
            f"evaluator_receipt_hash_mismatch:{field}",
        )
    gates = evaluator.get("gates")
    require(isinstance(gates, dict) and gates, "evaluator_gates_missing")
    failed_gates = []
    for name, value in gates.items():
        if isinstance(value, dict):
            ...
            passed = value == "PASS"
        if not passed:
            failed_gates.append(str(name))
    require(not failed_gates, "evaluator_nonpass_gates:" + ",".join(failed_gates))
    return {
        ...
        "job_results_sha256": receipt.get("job_results_sha256"),
        "pose_scores_sha256": receipt.get("pose_scores_sha256"),
...
def preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.trust_anchor is None or not args.expected_trust_anchor_sha:
        require(mode == "test_fixture", "production_trust_anchor_required")
        trust = {
            "schema_version": SCHEMA_VERSION,
            "status": "TEST_ONLY_TRUST_ANCHOR_BYPASS",
            "execution_mode": mode,
        }
    else:
        trust = validate_trust_anchor(
            args.trust_anchor, args.expected_trust_anchor_sha, TRUST_ANCHOR_KIND
        )

    # Validate every production-locked, label-free dependency before checking for
    # the teacher.  This deliberately catches implementation/hash drift while the
    # watcher is still in WAITING_OPEN_TEACHER.
    ...
    files = {
        ...
        "release_receipt": args.release_receipt,
        "evaluator": args.evaluator,
        **({"trust_anchor": args.trust_anchor} if args.trust_anchor is not None else {}),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    trust_parser = subparsers.add_parser("verify-trust-anchor")
    trust_parser.add_argument("--anchor", type=Path, required=True)
    trust_parser.add_argument("--expected-sha", required=True)
    trust_parser.add_argument("--expected-kind", default=TRUST_ANCHOR_KIND)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "verify-trust-anchor":
            result = validate_trust_anchor(
                args.anchor, args.expected_sha, args.expected_kind
            )
