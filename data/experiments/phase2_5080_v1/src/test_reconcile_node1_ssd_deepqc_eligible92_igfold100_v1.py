...
SCRIPT = Path(__file__).with_name("reconcile_node1_ssd_deepqc_eligible92_igfold100_v1.py")
SPEC = importlib.util.spec_from_file_location("reconcile_v1", SCRIPT)
...
class ReconciliationTests(unittest.TestCase):
    ...
    def test_tnp_result_is_bound_to_eligible_id(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = MODULE.validate_tnp_outputs(paths, {"a": "AAAA"}, {"a"})
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0]["candidate_id"], "a")
            payload["a"]["name"] = "wrong"
            result.write_text(json.dumps(payload))
            with self.assertRaises(RuntimeError):
                MODULE.validate_tnp_outputs(paths, {"a": "AAAA"}, {"a"})
