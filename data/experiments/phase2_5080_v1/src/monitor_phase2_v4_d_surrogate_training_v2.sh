...
HELPER=${WATCHER_HELPER:-$EXP_DIR/src/phase2_v4_d_surrogate_watcher_helper_v2.py}
...
STATUS_DIR=${V4D_SURROGATE_STATUS_DIR:-$EXP_DIR/status/pvrig_v4_d_surrogate_training_v2}
LOG_DIR=${V4D_SURROGATE_LOG_DIR:-$EXP_DIR/logs/pvrig_v4_d_surrogate_training_v2}
TEST_HASH_LOCKS=${V4D_TEST_ONLY_HASH_LOCKS:-}
TRUST_ANCHOR=${V4D_V2_TRUST_ANCHOR:-$EXP_DIR/audits/phase2_v4_d_surrogate_training_v2_implementation_trust_anchor.json}
EXPECTED_TRUST_ANCHOR_SHA=${V4D_V2_EXPECTED_TRUST_ANCHOR_SHA:-}
...
if [[ "$TEST_ONLY" == 1 ]]; then
  ...
  require_canonical HELPER "$HELPER" "$CANONICAL_EXP_DIR/src/phase2_v4_d_surrogate_watcher_helper_v2.py"
  ...
  require_canonical STATUS_DIR "$STATUS_DIR" "$CANONICAL_EXP_DIR/status/pvrig_v4_d_surrogate_training_v2"
  require_canonical LOG_DIR "$LOG_DIR" "$CANONICAL_EXP_DIR/logs/pvrig_v4_d_surrogate_training_v2"
  require_canonical TRUST_ANCHOR "$TRUST_ANCHOR" "$CANONICAL_EXP_DIR/audits/phase2_v4_d_surrogate_training_v2_implementation_trust_anchor.json"
  [[ -z "$TEST_HASH_LOCKS" ]] || { echo "test hash locks forbidden in production" >&2; exit 2; }
  [[ "$EXPECTED_TRUST_ANCHOR_SHA" =~ ^[0-9a-f]{64}$ ]] || {
    echo "production launcher must provide a valid V4D_V2_EXPECTED_TRUST_ANCHOR_SHA" >&2
    exit 2
  }
fi
...
COMPLETION_RECEIPT=$STATUS_DIR/surrogate_v2_completion_receipt.json
...
trust_gate() {
  ...
  if ! "$PYTHON" - "$TRUST_ANCHOR" "$EXPECTED_TRUST_ANCHOR_SHA" <<'PY'
...
assert payload["status"]=="FROZEN_BEFORE_OPEN258_TEACHER_AND_SURROGATE_TRAINING"
for role, item in payload["files"].items():
    path=Path(item["path"]); metadata=path.lstat()
    assert path.is_absolute() and str(path)==str(path.resolve()), role
    assert stat.S_ISREG(metadata.st_mode) and not stat.S_ISLNK(metadata.st_mode), role
    assert metadata.st_size==item["size"] and metadata.st_size>0, role
    assert hashlib.sha256(path.read_bytes()).hexdigest()==item["sha256"], role
PY
  then
    return 2
  fi
...
trust_gate startup || { echo "V2 implementation trust-anchor verification failed" >&2; exit 2; }
write_state WAITING_OPEN_TEACHER "V2 implementation trust verified; waiting for hash-closed open258 teacher; test32 labels remain sealed"
while true; do
...
PY
      write_state COMPLETE_V4_D_SURROGATE_TRAINING_TEST32_SEALED \
        "V2 trust receipt plus base, frozen embedding, and contact/fusion artifacts verified; scientific open-gate statuses recorded separately"
      exit 0
    fi
  fi
  if [[ "$ONCE" == 1 ]]; then
