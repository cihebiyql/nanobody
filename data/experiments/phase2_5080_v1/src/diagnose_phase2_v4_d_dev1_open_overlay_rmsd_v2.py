...
def main() -> int:
    ...
    invalid_by_conformation_seed: Counter[tuple[str, str]] = Counter()
    invalid_by_pose_index: Counter[int] = Counter()
    invalid_by_seed: Counter[str] = Counter()
    invalid_by_job: Counter[str] = Counter()
    invalid_by_candidate: Counter[str] = Counter()
    native_model_count_by_job: dict[str, int] = {}
    retained_model_count_by_job: dict[str, int] = {}
    ...
    for job in selected_jobs:
        ...
        job_native_model_count = 0
        job_invalid_model_count = 0
        for pose_index, pose in enumerate(payload.get("pose_scores", []), start=1):
            ...
            require(model not in seen_models, f"duplicate_pose_model:{job['job_id']}:{model}")
            ...
            job_native_model_count += 1
            if rmsd > RMSD_LIMIT_A:
                invalid_by_conformation[conformation] += 1
                invalid_by_conformation_seed[(conformation, job["seed"])] += 1
                invalid_by_pose_index[pose_index] += 1
                invalid_by_seed[job["seed"]] += 1
                invalid_by_job[job["job_id"]] += 1
                invalid_by_candidate[job["entity_id"]] += 1
                job_invalid_model_count += 1
                invalid_rows.append(
                    {
                        "candidate_id": job["entity_id"],
                        "job_id": job["job_id"],
        ...
        native_model_count_by_job[job["job_id"]] = job_native_model_count
        retained_model_count_by_job[job["job_id"]] = (
            job_native_model_count - job_invalid_model_count
        )
    ...
    selected_job_by_id = {job["job_id"]: job for job in selected_jobs}
    affected_job_retention = [
        {
            "candidate_id": selected_job_by_id[job_id]["entity_id"],
            "job_id": job_id,
            "conformation": selected_job_by_id[job_id]["conformation"].lower(),
            "seed": int(selected_job_by_id[job_id]["seed"]),
            "native_scored_model_count_before": native_model_count_by_job[job_id],
            "excluded_native_overlay_gt_1a_count": invalid_by_job[job_id],
            "retained_model_count_after_exclusion": retained_model_count_by_job[job_id],
        }
        for job_id in affected_jobs
    ]
    retained_counts = [
        row["retained_model_count_after_exclusion"] for row in affected_job_retention
    ]
    missing_open_jobs.sort(key=lambda row: row["job_id"])
    raw_bindings.sort(key=lambda row: row["job_id"])

    output: dict[str, Any] = {
        "schema_version": "phase2_v4_d_dev1_open_overlay_rmsd_diagnostic_v2",
        ...
        "native_overlay_summary": {
            ...
            "above_1a_by_seed": {
                seed: invalid_by_seed.get(seed, 0) for seed in sorted(SEEDS, key=int)
            },
            "above_1a_by_conformation_seed": {
                f"{conformation}_s{seed}": invalid_by_conformation_seed.get(
                    (conformation, seed), 0
                )
                for conformation in sorted(CONFORMATIONS)
                for seed in sorted(SEEDS, key=int)
            },
            "above_1a_by_pose_index": {
                str(index): invalid_by_pose_index.get(index, 0)
                for index in sorted(invalid_by_pose_index)
            },
            "rmsd_bands": {
                "gt_1_le_2": sum(1.0 < value <= 2.0 for value in rmsd_values),
                "gt_2_le_5": sum(2.0 < value <= 5.0 for value in rmsd_values),
                "gt_5": sum(value > 5.0 for value in rmsd_values),
                "gt_10": sum(value > 10.0 for value in rmsd_values),
                "gt_50": sum(value > 50.0 for value in rmsd_values),
                "gt_100": sum(value > 100.0 for value in rmsd_values),
            },
            "affected_job_count": len(affected_jobs),
            "affected_success_job_fraction": (
                len(affected_jobs) / counters["open_success_job_files_inspected"]
            ),
            "affected_candidate_count": len(affected_candidates),
            "affected_open_candidate_fraction": len(affected_candidates) / len(open_ids),
            "affected_jobs": affected_jobs,
            "affected_candidates": affected_candidates,
            "affected_job_retention": {
                "minimum_retained_model_count": min(retained_counts),
                "maximum_retained_model_count": max(retained_counts),
                "jobs_with_retained_model_count_below_4": sum(
                    count < 4 for count in retained_counts
                ),
                "jobs_with_retained_model_count_at_least_4": sum(
                    count >= 4 for count in retained_counts
                ),
                "rows": affected_job_retention,
            },
        },
