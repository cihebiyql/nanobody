...
def ensure_decimal_evaluator_runtime(destination: Path) -> None:
    aggregate_path = destination / "scripts/aggregate_results.py"
    text = aggregate_path.read_text(encoding="utf-8")
    if "from decimal import Decimal" not in text:
        token = "from collections import Counter, defaultdict\n"
        if token not in text:
            raise RuntimeError("aggregate_results Decimal import token is absent")
        text = text.replace(token, token + "from decimal import Decimal\n", 1)

    if "def decimal_scaled(" not in text:
        token = (
            'LEGACY_THRESHOLDS = {"hotspot": 14.0, "total_occlusion": 500.0, '
            '"cdr3_occlusion": 100.0, "cdr3_fraction": 0.15}\n\n\n'
        )
        replacement = token + (
        ...
        if token not in text:
            raise RuntimeError("aggregate_results Decimal helper token is absent")
    ...
    replacements = {
        ...
        "total < 50 * scale": "total < decimal_scaled(50.0, scale)",
        "hotspot >= 10 * scale and total >= 100 * scale and cdr3 >= 20 * scale and fraction >= 0.10 * scale": (
            "hotspot >= decimal_scaled(10.0, scale) and total >= decimal_scaled(100.0, scale) "
            "and cdr3 >= decimal_scaled(20.0, scale) and fraction >= decimal_scaled(0.10, scale)"
        ),
    }
    for old, new in replacements.items():
        if old in text:
            ...
            raise RuntimeError(f"aggregate_results numeric patch token is absent: {old!r}")

    if '"stability_gate_id": stability_spec.get("gate_id")' not in text:
        token = '        "stability_gate_spec": str(stability_spec_path),\n'
        if token not in text:
            raise RuntimeError("aggregate_results stability gate payload token is absent")
        text = text.replace(
            token,
            '        "stability_gate_id": stability_spec.get("gate_id"),\n' + token,
            1,
        )
    if '"numeric_comparison": stability_spec.get("numeric_comparison", {})' not in text:
        token = '        "stability_gate_spec_sha256": sha256_file(stability_spec_path),\n'
        if token not in text:
            raise RuntimeError("aggregate_results numeric metadata token is absent")
        text = text.replace(
            token,
            token + '        "numeric_comparison": stability_spec.get("numeric_comparison", {}),\n',
            1,
        )
    aggregate_path.write_text(text, encoding="utf-8")

    gate_path = destination / "config/evaluator_stability_gate.json"
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    gate["schema_version"] = 2
    gate["gate_id"] = "pvrig_v4_d_evaluator_stability_v3_decimal_thresholds"
    gate["numeric_comparison"] = {
        "boundary_policy": "inclusive_greater_than_or_equal",
        "scaled_threshold_arithmetic": "decimal_string_product",
        "supersedes": "pvrig_v3_evaluator_stability_v2",
        "correction_reason": (
            "avoid binary-float inflation such as 100.0 * 1.1 becoming "
            "110.00000000000001"
        ),
    }
    write_json(gate_path, gate)

    test_path = destination / "tests/test_stability_gate.py"
    test_text = test_path.read_text(encoding="utf-8")
    start = test_text.find("    def test_scaled_threshold_boundary_uses_decimal_semantics")
    end_token = "    def reduced_valid_jobs(self) -> list[dict[str, str]]:\n"
    end = test_text.find(end_token)
    if end < 0:
        raise RuntimeError("stability test insertion token is absent")
    if start >= 0:
        test_text = test_text[:start] + test_text[end:]
        end = test_text.find(end_token)
    boundary_tests = '''    def test_scaled_threshold_boundary_uses_decimal_semantics(self) -> None:
        score = {
            "hotspot_overlap": {"full": {"count": 15.4}},
            "vhh_pvrl2_occlusion": {
                "residue_pair_count": 550.0,
                "by_vhh_region_pair_count": {"cdr3": 110.0},
                "cdr3_fraction": 0.165,
            },
        }
        ...
        paths = (
            ("hotspot_overlap", "full", "count", 15.399999999),
            ("vhh_pvrl2_occlusion", "residue_pair_count", None, 549.999999999),
            ("vhh_pvrl2_occlusion", "by_vhh_region_pair_count", "cdr3", 109.999999999),
            ("vhh_pvrl2_occlusion", "cdr3_fraction", None, 0.164999999),
        )
        for first, second, third, value in paths:
            candidate = json.loads(json.dumps(score))
            if third is None:
                candidate[first][second] = value
            else:
                candidate[first][second][third] = value
            with self.subTest(metric=(first, second, third)):
                self.assertNotEqual(aggregate_results.classify_geometry(candidate, 1.1), "A")

    def test_scaled_secondary_boundaries_use_decimal_semantics(self) -> None:
        class_c = {
            "hotspot_overlap": {"full": {"count": 15.4}},
            "vhh_pvrl2_occlusion": {
                "residue_pair_count": 54.999999999,
                "by_vhh_region_pair_count": {"cdr3": 0.0},
                "cdr3_fraction": 0.0,
            },
        }
        self.assertEqual(aggregate_results.classify_geometry(class_c, 1.1), "C")
        class_b = {
            "hotspot_overlap": {"full": {"count": 11.0}},
            "vhh_pvrl2_occlusion": {
                "residue_pair_count": 110.0,
                "by_vhh_region_pair_count": {"cdr3": 22.0},
                "cdr3_fraction": 0.11,
            },
        }
        self.assertEqual(aggregate_results.classify_geometry(class_b, 1.1), "B")

'''
    test_path.write_text(test_text[:end] + boundary_tests + test_text[end:], encoding="utf-8")
...
def patch_runtime(destination: Path, smoke_candidate: str) -> None:
    ...
    ensure_decimal_evaluator_runtime(destination)
    replace_text(
