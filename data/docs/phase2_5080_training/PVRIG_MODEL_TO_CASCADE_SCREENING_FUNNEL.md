...
4. 在 Node1 负载低于固定 gate 时运行 HADDOCK3，不得绕过 load gate。
...
2026-07-11 的 24 条 panel 实测：

- 24 input -> 4 fast hard-pass -> 4 full -> 4 geometry shortlist；
- cascade 墙钟时间 132 秒；
- dual-baseline finalize 后 docking imported = 1；
- `PV25-25F7D6778F87`（coordinator-only ID `zym_test_108006`）为双基线
  A/A，计算标签是 `FINAL_POSITIVE_HIGH`；
- 其保守几何指标为 hotspot overlap 15、total PVRL2 occlusion 610、
  CDR3 occlusion 106、CDR3 fraction 0.17377；
- 其余 3 条是 `FINAL_INCOMPLETE_NEEDS_DOCKING`；
- 24 条实验样本仍全部是 `PENDING_EXPRESSION_QC`；
- E6 review rows = 0。

这里的 `FINAL_POSITIVE_HIGH` 只表示双基线计算几何优先级，不是已验证
binder、blocker 或功能阳性；冻结的 24 条 prospective panel 不因该标签或
cascade 结果而增删。
