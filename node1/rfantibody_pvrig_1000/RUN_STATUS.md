...
- `run_manifest.json` 记录的 RFantibody 参考 commit：`8fe311415754e0276d1a39c87c57e69c88927a2d`
...
- 远端仍保留 200 个 backbone PDB、200 个 TRB 和 1,600 个 ProteinMPNN sequence PDB。
- 远端 PVRIG 输入、RFdiffusion 权重和 ProteinMPNN 权重的 SHA256 均与 `run_manifest.json` 一致。
- 本地的配置、输入、泄漏参考和 5 个执行/收集脚本与远端逐文件 SHA256 一致。

远端安装目录 `/data/qlyu/software/RFantibody` 当前不包含 `.git`
元数据，因此参考 commit 是本批次 manifest 保存的 provenance，不是现场从
安装目录重新执行 `git rev-parse` 得到的证据。具体输入、权重和脚本则已通过
实际文件哈希重新核对。
