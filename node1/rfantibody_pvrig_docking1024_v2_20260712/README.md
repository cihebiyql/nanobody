...
```bash
nohup env MAX_LOAD1=72 bash scripts/run_generation_controller.sh \
  > logs/generation_pipeline_controller.log 2>&1 &

nohup env MAX_LOAD1=64 bash scripts/run_downstream_controller.sh \
  > logs/downstream_pipeline_controller.log 2>&1 &

nohup env MAX_LOAD1=64 bash scripts/run_postprocess_controller.sh \
...
- RFdiffusion、RF2、NanoBodyBuilder2 使用 GPU 1、2、3、4、5、7；GPU 占用超过阈值时等待。
