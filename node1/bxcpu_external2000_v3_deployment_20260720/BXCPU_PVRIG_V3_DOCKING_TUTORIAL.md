...
在该时点，正式作业的实际并发上限是 **2 节点 × 64 核 = 128 CPU 核**。
之后集群管理员已将 `bscc-al` 的 `GrpTRES` 提高为 `node=8`。
2026-07-20 20:01 CST 复核时，array `11936121` 的 8 个分片均已处于 `RUNNING`，
分别占用 8 个独占 64 核节点，实际并发资源为 **512 CPU 核**。
