...
for ((i=0; i<COUNT; i++)); do
    ...
    dfile=$(printf '%s/state/stage2.shard%02dof%02d.delivered_job_ids.txt' "$root" "$i" "$COUNT")
    pfile=$(printf '%s/state/stage2.shard%02dof%02d.pruned_job_ids.txt' "$root" "$i" "$COUNT")
