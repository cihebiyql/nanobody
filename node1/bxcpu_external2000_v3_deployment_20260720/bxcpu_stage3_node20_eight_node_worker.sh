#!/usr/bin/env bash
# One of eight exclusive 64-core nodes for the frozen Node20 residual Stage3 transfer shard.
set -euo pipefail
umask 027

CACHE_ROOT="${PVRIG_BXCPU_CACHE:-$HOME/.local/opt}"
BUNDLE_ARCHIVE="$HOME/pvrig_v29_bxcpu_stage3_node20_v1_20260720.tar.zst"
PROJECT_NAME=pvrig_v29_bxcpu_stage3_node20_v1_20260720
...
SAFE_MANIFEST="$LOCAL_PROJECT/manifests/stage3_jobs.tsv"
TOTAL_JOBS=$("$LOCAL_ENV/bin/python" - "$LOCAL_PROJECT/status/READY.json" <<'PY'
import json, sys
print(int(json.load(open(sys.argv[1]))["job_count"]))
PY
)
[[ "$TOTAL_JOBS" -gt 0 ]] || { echo "empty Stage3 job set" >&2; exit 65; }
[[ $(wc -l < "$SAFE_MANIFEST") -eq $((TOTAL_JOBS + 1)) ]] || {
    echo "unexpected Stage3 manifest row count" >&2
    exit 65
}
[[ -f "$LOCAL_PROJECT/reports/reference_normalization_summary.json" ]] || { echo "missing V3 score reference" >&2; exit 65; }
[[ -f "$LOCAL_PROJECT/scripts/run_job.py" ]] || { echo "missing Stage2 run_job.py" >&2; exit 65; }
...
expected=$(((TOTAL_JOBS + SHARD_COUNT - SHARD_INDEX) / SHARD_COUNT))
...
marker="$PUBLISH_ROOT/markers/stage3_node20_8shard_${SHARD_INDEX}.done"
