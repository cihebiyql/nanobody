#!/usr/bin/env bash
# One of eight exclusive 64-core nodes for the frozen 10,500-job Stage2 transfer shard.
set -euo pipefail
umask 027

CACHE_ROOT="${PVRIG_BXCPU_CACHE:-$HOME/.local/opt}"
BUNDLE_ARCHIVE="$HOME/pvrig_v29_bxcpu_stage2_10500_v1_20260720.tar.zst"
PROJECT_NAME=pvrig_v29_bxcpu_stage2_10500_v1_20260720
...
SAFE_MANIFEST="$LOCAL_PROJECT/manifests/stage2_jobs.tsv"
[[ $(wc -l < "$SAFE_MANIFEST") -eq 10501 ]] || { echo "unexpected Stage2 manifest" >&2; exit 65; }
...
marker="$PUBLISH_ROOT/markers/stage2_10500_8shard_${SHARD_INDEX}.done"
