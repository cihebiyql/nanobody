...
- 预审计的 200 个 backbone 中，198 个在 8X6B 对齐后对 PVRL2 有任意遮挡；52 个同时满足：VHH-PVRL2 总遮挡残基对计数 `>=500`、CDR3-PVRL2 遮挡残基对计数 `>=100`、CDR3 占比 `>=0.15`。这是基于重原子距离截断的几何接触代理，不是埋藏表面积 `A^2`。
...
本轮已完成结构漏斗和诊断性 docking，结论是 **conditional-go**：正式 blocker 高置信通道不通过，但诊断性几何分析有用。

- RF2 blind pose-recovery：78/78 有完整输出，但严格通过为 `0/78`；68 条为低交互置信，10 条为目标对齐后姿势未恢复。
- NanoBodyBuilder2：对 30 条诊断候选建模，`30/30` 序列完全匹配，`30/30` 主链几何质控通过。
- HADDOCK3：`30/30` 候选成功，共保留 260 个 cluster-selected models，无非零退出。
- 双基线模型级共识：260 个模型中只有 5 个在 8X6B 和 9E6Y 上同时达到 `BLOCKER_LIKE_A`，分属 5 个候选；没有任一候选拥有至少 2 个 A/A 模型。
- 最终标签：30/30 均为 `FINAL_DIAGNOSTIC_ONLY_RF2_NOT_RECOVERED`，`FINAL_POSITIVE_HIGH=0`。

这一结果不能证明候选不结合，但说明当前候选的 RFantibody 结合方向没有被独立 RF2 盲恢复。后续 full-interface-guided HADDOCK 只能用于弱排序、淘汰和下一轮设计诊断，不能把候选“救回”为已验证 blocker。详细报告见 `reports/PVRIG_RFANTIBODY_VALIDATION_FINAL_ZH.md`。
