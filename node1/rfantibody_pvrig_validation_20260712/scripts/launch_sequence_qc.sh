...
for path in \
  "$LOCAL_ROOT/inputs/pvrig_rfantibody_1000.canonical.fasta" \
  "$LOCAL_ROOT/manifests/qc_input_audit.json" \
...
copy_file \
  "$LOCAL_ROOT/inputs/pvrig_rfantibody_1000.canonical.fasta" \
  "$REMOTE_ROOT/inputs/pvrig_rfantibody_1000.canonical.fasta"
copy_file \
  "$LOCAL_ROOT/manifests/qc_input_audit.json" \
  "$REMOTE_ROOT/manifests/qc_input_audit.json"
copy_file \
  "$LOCAL_ROOT/config/pipeline_config.json" \
...
ssh.exe "$REMOTE_HOST" "chmod +x '$REMOTE_ROOT/scripts/run_sequence_qc_node1.sh' && '$REMOTE_ROOT/scripts/run_sequence_qc_node1.sh'"
