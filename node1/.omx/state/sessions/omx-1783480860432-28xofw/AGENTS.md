...
- When the host is Windows or the shell is PowerShell, emit Windows PowerShell 5.1-compatible commands unless the task explicitly requires `pwsh`.
- Never mix in bash-only syntax such as `&&`, `||`, `export`, `rm -rf`, `cp`, `mv`, or `cat <<EOF`.
- For multi-step or failure-sensitive commands, start with `$ErrorActionPreference = 'Stop'`.
- Use single quotes for literal strings and double quotes only when interpolation is required.
- Quote Windows paths and prefer `-LiteralPath` for filesystem operations.
- Prefer `Join-Path` over manual path concatenation.
- Build JSON from hashtables or objects and serialize with `ConvertTo-Json -Compress`; never hand-write escaped JSON strings.
- Invoke external executables with `&` and pass arguments as separate tokens instead of one large composed command string.
- Prefer native PowerShell cmdlets in reusable docs and examples, and prefer `py -3` for Python entrypoints on Windows.
- If a compact reusable instruction block is needed, use `~/.codex/prompts/windows-powershell-safe.md`.
...
<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1783480860432-28xofw | 2026-07-08T03:21:09.978Z
