#!/usr/bin/env python3
"""Run Boltz-2 and Chai-1 Top100 shards without HADDOCK restraints."""
...
def main() -> int:
    ...
    if len(gpus) != 8:
        raise ValueError("eight GPU IDs required")
    logs = project / "logs"
    ...
    state: dict[str, object] = {
        "schema_version": "pvrig.top100.independent_complex.controller.v1",
        "pid": os.getpid(),
    ...
    try:
        ...
        if boltz_count != 100:
            raise RuntimeError(f"expected 100 Boltz PDBs, found {boltz_count}")
        ...
        for shard, gpu in enumerate(gpus):
            ...
            chai_commands.append(
                (
                    ...
                    [
                        str(CHAI_PYTHON),
                        str(script_root / "run_chai_top100_shard.py"),
                        "--manifest",
                        str(
                            project / "manifests"
                            / "top100_independent_complex_manifest.tsv"
                        ),
                        "--project", str(project),
                        "--shard", str(shard),
                    ],
        ...
        if chai_count != 200:
            raise RuntimeError(f"expected 200 Chai CIFs, found {chai_count}")
