...
ROOT="${PVRIG_RUN_ROOT:-/data/qlyu/projects/pvrig_top7500_dualpanel_screen_v1_20260724}"
MDROOT="$ROOT/run/md"
...
assert len({r["system_id"] for r in rows})==20
PY
...
json.dump({"state":"RUNNING","pid":os.getppid(),"started_at":datetime.now(timezone.utc).isoformat(),
           ...
           "gpu_limit":4,"cpu_limit":32,"slots_per_gpu":4,
           "ntomp_per_trajectory":2},open(sys.argv[1],"w"),indent=2)
PY
mv "$STATUS_FILE.tmp" "$STATUS_FILE"

mapfile -t gpu_list < <(tail -n +2 "$MANIFEST" | cut -f6 | sort -n -u)
[[ "${#gpu_list[@]}" -le 4 ]] || { echo "manifest exceeds four GPUs" >&2; exit 64; }
...
python3 - "$MANIFEST" "$PRODUCTION_BASE" "$STATUS_FILE.tmp" <<'PY'
import csv,json,sys
from datetime import datetime,timezone
...
json.dump({"state":state,"updated_at":datetime.now(timezone.utc).isoformat(),
           ...
           "gpu_limit":4,"cpu_limit":32,"slots_per_gpu":4,
           "ntomp_per_trajectory":2},open(status,"w"),indent=2)
