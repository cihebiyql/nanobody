...
def main() -> int:
    ...
    expected_boltz_inputs = {Path(row["boltz_input"]) for row in manifest_rows}
    observed_boltz_inputs = set(boltz_inputs.rglob("*.yaml"))
    if observed_boltz_inputs != expected_boltz_inputs:
        stale = sorted(str(path) for path in observed_boltz_inputs - expected_boltz_inputs)
        missing = sorted(str(path) for path in expected_boltz_inputs - observed_boltz_inputs)
        raise RuntimeError(
            f"Boltz input freeze mismatch; stale={stale}, missing={missing}"
        )
    expected_chai_inputs = {Path(row["chai_input"]) for row in manifest_rows}
    observed_chai_inputs = set(chai_inputs.glob("*.fasta"))
    if observed_chai_inputs != expected_chai_inputs:
        stale = sorted(str(path) for path in observed_chai_inputs - expected_chai_inputs)
        missing = sorted(str(path) for path in expected_chai_inputs - observed_chai_inputs)
        raise RuntimeError(
            f"Chai input freeze mismatch; stale={stale}, missing={missing}"
        )
    protocol = {
