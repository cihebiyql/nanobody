#!/usr/bin/env python3
"""Normalize and score control-panel Boltz/Chai poses against 8X6B and 9E6Y."""
...
def main() -> int:
    ...
    for candidate in sorted(manifest, key=lambda row: int(row["top100_rank"])):
        ...
        for tool, pose_index, source in sources:
            ...
            row: dict[str, Any] = {
                ...
                "base_molecule": candidate["base_molecule"],
    ...
    pose_path = reports / "CONTROL_INDEPENDENT_COMPLEX_POSE_SCORES.tsv"
    candidate_path = reports / "CONTROL_INDEPENDENT_COMPLEX_CANDIDATE_SUMMARY.tsv"
    uncertain_path = reports / "CONTROL_INDEPENDENT_COMPLEX_HIGH_UNCERTAINTY.tsv"
    ...
    summary_payload = {
        "schema_version": "pvrig.control.independent_complex.scoring.v1",
        "state": "COMPLETE",
        ...
        "threshold_note": (
            ...
            "are interpreted here using five experimental positive blockers and "
            "four computational disruptive controls."
        ),
        "claim_boundary": (
    ...
    summary_path = reports / "CONTROL_INDEPENDENT_COMPLEX_SCORING_SUMMARY.json"
