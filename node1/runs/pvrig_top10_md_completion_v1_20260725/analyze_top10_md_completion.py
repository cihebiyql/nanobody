#!/usr/bin/env python3
"""Analyze a Top10 completion short-MD panel as descriptive pose-persistence evidence."""
...
def main() -> int:
    ...
    if not systems or not jobs or len(jobs) != 3 * len(systems):
        raise ValueError("expected a non-empty panel with three MD jobs per system")
    ...
    receipt = {
        "schema_version": "pvrig.top10.md_completion.analysis.v1",
        "state": "TOP10_MD_COMPLETION_ANALYSIS_COMPLETE",
    ...
    receipt_path = report_dir / "TOP10_MD_COMPLETION_ANALYSIS_COMPLETE.json"
    receipt_path.write_text(
        json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(json.dumps({"candidates": len(candidates), "trajectories": len(trajectory_rows)}))
    return 0
